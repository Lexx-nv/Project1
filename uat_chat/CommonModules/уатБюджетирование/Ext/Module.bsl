...
Процедура ПроверкаКурсовВалютСценарии(ТаблицаВалют,Отказ,Заголовок) Экспорт
	...
	Для Каждого СтрокаВалюта ИЗ ТаблицаВалют Цикл
		...
		Если СтрокаВалюта.ИспользоватьКурсыСценария Тогда
			...
			СтруктураКурсов=РегистрыСведений.уатКурсыВалютПоСценариям.ПолучитьПоследнее(СтрокаВалюта.ДатаКурса,СтруктураОтбор);
...
Функция КурсВалютыПоСценарию(Валюта, ДатаКурса, Сценарий) Экспорт
	...
	Если Сценарий.ИспользоватьКурсыСценария Тогда
        ...
        Структура = РегистрыСведений.уатКурсыВалютПоСценариям.ПолучитьПоследнее(ДатаКурса, СтруктураОтбора);
	Иначе
...
// Процедура добавляет в текст запроса обороты за указанный интервал
//
...
// Функция формирует текст запроса по оборотам за указанный интервал
//
Функция СформироватьТекстЗапросаПоОборотам(СтруктураИспользуемыхИзмерений,Сценарий,ДатаНач,ДатаКон,ОтборСтатьи) Экспорт
	...
	|ВЫРАЗИТЬ(ЗапросПоИсточникам.СтатьяОборотов КАК Справочник.уатСтатьиОборотовПоБюджетам) КАК СтатьяОборотов,
	...
	СтруктураПараметров.Вставить("ПустойНоменклатура",?(Сценарий.ДетализацияПланирования=Перечисления.уатДетализацияПланирования.НоменклатурнаяГруппа,
...
// Функция формирует текст запроса по остаткам на заданную дату
//
Функция СформироватьТекстЗапросаПоОстаткам(СтруктураИспользуемыхИзмерений,Сценарий,ДатаОстатка,ОтборСчета) Экспорт
	...
	ТекстИтог="ВЫБРАТЬ
	|ВЫРАЗИТЬ(ЗапросПоИсточникам.Счет КАК ПланСчетов.уатБюджетирование) КАК Счет,
	|ВЫРАЗИТЬ(ЗапросПоИсточникам.Валюта КАК Справочник.Валюты) КАК Валюта,
	|ВЫРАЗИТЬ(ЗапросПоИсточникам.ЦФО КАК Справочник.Подразделения) КАК ЦФО,
	|ВЫРАЗИТЬ(ЗапросПоИсточникам.Проект КАК Справочник.Проекты) КАК Проект,
	|ВЫРАЗИТЬ(ЗапросПоИсточникам.Контрагент КАК Справочник.Контрагенты) КАК Контрагент,
...
// Функция формирует текст запроса по остаткам на границы заданного интервала
//
Функция СформироватьТекстЗапросаПоОстаткамПериод(СтруктураИспользуемыхИзмерений,Сценарий,ДатаНач,ДатаКон,Периодичность,ОтборСчета,Префикс="") Экспорт
	...
	ТекстИтог="ВЫБРАТЬ
	|ВЫРАЗИТЬ(ЗапросПоИсточникам"+Префикс+".Счет КАК ПланСчетов.уатБюджетирование) КАК Счет,
	...
	|СГРУППИРОВАТЬ ПО 
	|ВЫРАЗИТЬ(ЗапросПоИсточникам"+Префикс+".Счет КАК ПланСчетов.уатБюджетирование),
...
// Процедура добавляет в текст запроса остатки на заданную дату
//
...
// Процедура добавляет в текст запроса остатки на границы заданного интервала
//
