...
// Создаем COM соединение с COM объектом Word.Application, создает в нем
...
Процедура ЗаполнитьПараметрыНижнегоКолонтитула(знач ПечатнаяФорма,
	...
	Для Каждого ПараметрЗначение Из ДанныеОбъекта Цикл
		Если ТипЗнч(ПараметрЗначение.Значение) <> Тип("Массив") Тогда
			ВыполнитьЗаменуВНижнемКолонтитуле(ПечатнаяФорма.COMСоединение,
...
// Выполняет замену в нижнем колонтитуле.
//
Функция ВыполнитьЗаменуВНижнемКолонтитуле(COMСоединение, ТекстДляПоиска, ТекстЗамены)
...
Процедура ЗаполнитьПараметрыВерхнегоКолонтитула(знач ПечатнаяФорма,
	...
	Для Каждого ПараметрЗначение Из ДанныеОбъекта Цикл
		Если ТипЗнч(ПараметрЗначение.Значение) <> Тип("Массив") Тогда
			ВыполнитьЗаменуВВерхнемКолонтитуле(ПечатнаяФорма.COMСоединение,
												ПараметрЗначение.Ключ,
												ПараметрЗначение.Значение);
		КонецЕсли;
	КонецЦикла;
...
// Выполняет замену в верхнем колонтитуле.
//
...
Процедура ЗаполнитьПараметры(знач ОбластьПечатнойФормы,
	...
	Для Каждого ПараметрЗначение Из ДанныеОбъекта Цикл
		Если ТипЗнч(ПараметрЗначение.Значение) <> Тип("Массив") Тогда
			ВыполнитьЗамену(ОбластьПечатнойФормы.Document,
							ПараметрЗначение.Ключ,
							ПараметрЗначение.Значение);
		КонецЕсли;
	КонецЦикла;
...
// Выполняет замену строки ТекстДляПоиска на строку ТекстЗамены.
//
...
// Получает массив настроек параметров страницы.
//
