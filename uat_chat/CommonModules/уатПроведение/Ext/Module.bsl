...
Процедура ИнициализироватьДополнительныеСвойстваДляПроведения(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	|   " + ?(СтруктураДополнительныеСвойства.ДляПроведения.МетаданныеДокумента.Реквизиты.Найти("Организация") <> Неопределено, "_Документ_.Организация КАК Организация," , "") + "
	...
	СтруктураКурсаРегл = уатОбщегоНазначенияТиповые.ПолучитьКурсВалюты(Константы.ВалютаРегламентированногоУчета.Получить(),ДокументСсылка.Дата);
	...
	Если уатРаботаСМетаданными.уатЕстьКонстанта("ВалютаУправленческогоУчета") тогда
		СтруктураКурсаУпр = уатОбщегоНазначенияТиповые.ПолучитьКурсВалюты(Константы.ВалютаУправленческогоУчета.Получить(),ДокументСсылка.Дата);
...
// Выполняет движения регистра накопления Заказы на ТС
//
Процедура ОтразитьЗаказыНаТС(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаЗаказовНаТС = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗаказовНаТС;
	
	Если Отказ ИЛИ ТаблицаЗаказовНаТС.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвиженияУслуги = Движения.уатЗаказыНаТС;
	ДвиженияУслуги.Записывать = Истина;
	ДвиженияУслуги.Загрузить(ТаблицаЗаказовНаТС);
...
//
Функция ПолучитьТаблицуПартийДляСписания(ТаблицаНоменклатуры, СпособСписанияПартий, МоментВремени = Неопределено, СтруктураДополнительныхПараметров = Неопределено) Экспорт
	...
	Иначе
		...
		Если СпособСписанияПартий = Перечисления.СпособыОценки.ЛИФО Тогда
			мЗапрос.Текст = СтрЗаменить(мЗапрос.Текст,"уатПартииТоваровНаСкладахОстатки.Партия.МоментВремени","уатПартииТоваровНаСкладахОстатки.Партия.МоментВремени Убыв");
		КонецЕсли;
		
        мЗапрос.УстановитьПараметр("Партия",Неопределено);
		МассивРезультатов = мЗапрос.ВыполнитьПакет();
		
		ТаблицастатковПартий = МассивРезультатов[2].Выгрузить();
		...
		ТаблицаПартий.Колонки.Добавить("Партия"      , Новый ОписаниеТипов("ДокументСсылка.уатОприходованиеТоваров, ДокументСсылка.уатСливГСМ, ДокументСсылка.уатПоступлениеТоваровУслуг, ДокументСсылка.уатОперацииСТехническимиЖидкостями"));
		...
		Для каждого ТекСтрокаТабНоменклатуры ИЗ ТаблицаНоменклатуры Цикл
			...
			Для каждого ТекСтрокаОстатковПартий ИЗ ТаблицастатковПартий Цикл
				...
				Если ТекСтрокаТабНоменклатуры.Склад = ТекСтрокаОстатковПартий.Склад 
				   ...
				   И ТекСтрокаОстатковПартий.КоличествоОсталось > 0 тогда
					...
					ИначеЕсли ТекСтрокаОстатковПартий.КоличествоОсталось <> 0 тогда
						НовСтрока.Стоимость    = Окр(ТекСтрокаОстатковПартий.СтоимостьОстаток    / ТекСтрокаОстатковПартий.КоличествоОсталось * СписатьКоличество,2);
						НовСтрока.СтоимостьУпр = Окр(ТекСтрокаОстатковПартий.СтоимостьУпрОстаток / ТекСтрокаОстатковПартий.КоличествоОсталось * СписатьКоличество,2);
					КонецЕсли;
