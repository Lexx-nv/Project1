...
Функция ПолучитьСтруктуруОтбора(Отбор) Экспорт
	...
	Для каждого ЭлементОтбора Из Отбор Цикл
		...
		СтруктураЭлементаОтбора.Вставить("Значение"     , ЭлементОтбора.Значение);
		СтруктураЭлементаОтбора.Вставить("ЗначениеПо"   , ЭлементОтбора.ЗначениеПо);
		СтруктураЭлементаОтбора.Вставить("ЗначениеС"    , ЭлементОтбора.ЗначениеС);
		СтруктураЭлементаОтбора.Вставить("Имя"          , ЭлементОтбора.Имя);
		...
		СтруктураЭлементаОтбора.Вставить("ПутьКДанным"  , ЭлементОтбора.ПутьКДанным);
		СтруктураЭлементаОтбора.Вставить("ТипЗначения"  , ЭлементОтбора.ТипЗначения);
		СтруктураОтбора.Вставить("Отбор" + Строка(Отбор.Индекс(ЭлементОтбора)), СтруктураЭлементаОтбора);
	КонецЦикла; 
...
// Процедура заполняет данными отбор из структуры.
//
// Параметры
//  Отбор - Отбор
...
#Если Клиент Тогда
	...
	// Функция  делает попытку сохранить  значения настроек отчета
	//
	// Параметры:
	//	СтруктураНастройки - структура сохраняемых настроек
	// Возвращаемое значение:
	//  Результат попытки : Истина ,если настройки восстановлены, иначе - Ложь
	//	
...
// Функция  делает попытку получить значения настройки отчета
...
// Функция  делает попытку получить значения настроек отчета
//
// Параметры:
//	СтруктураНастройки - структура настроек
//  ПолучитьНастройкиВсехПользователей  - флаг восстановления пользовательских настроек
//  ПолучитьГрупповыеНастройки  - флаг восстановления настроек для группы пользователей
//  ПолучитьОбщиеНастройки  - флаг восстановления общих настроек
// Возвращаемое значение:
//  Таблица значений -Результат запроса по значению настроек
//
...
// Функция  делает попытку удалить значения настройки отчета
//
// Параметры:
//	СтруктураНастройки - структура настроек
// Возвращаемое значение:
...
// Процедура устанавливает видимость панели отчета
//
// Параметры:
//	ЭлементыФормы - ссылка на коллекцию элементов формы
//  ВариантОтображения - отображаемая страница панели
//
...
Процедура ДействияПриИзмененииОтображения(Отчет, ФормаОтчета, ВыбраннаяКнопка) Экспорт
	...
	Если Не ВыбраннаяКнопка.Пометка Тогда
		...
		Если Отчет.Метаданные().Имя = "СтатистикаКадров" Тогда
			// если отчет уже формировался - отображаем полученные результаты по-новому
...
// Процедура добавляет в текст запроса тест описания поля свойств
//
// Параметры:
//	Текст - тест запроса для коррекции
//  ТекстПоляСвойств - добавляемый текст
//  ЗаменятьСвойстваОбщие - текст комментария, на мето которого нужно добавить текст
//
...
Процедура ДобавитьВТекстПостроителяКонтактнуюИнформацию(ТаблицаВидовОбъектовКИ, Текст, СтруктураПредставлениеПолей, мСоответствиеНазначений, 
	...
	Для Каждого СтрокаТаблицыВидовОбъектов ИЗ ТаблицаВидовОбъектовКИ Цикл
		...
		// Текст выборки всех видов контактной информации для заданного вида объекта: 
		//Произвольные + Заданные в справочнике "ВидыКонтактнойИнформации"
...
//                            ДоступноеПоле значением свойства или категории
...
Процедура УстановитьСвязьПолейБыстрогоОтбораНаФорме(ЭлементыФормы, Отбор, СтруктураСвязиЭлементовСДанными = Неопределено, ПутьКОтбору = "ОтчетОбъект.ПостроительОтчета.Отбор") Экспорт
	...
	Для Сч = 0 По Отбор.Количество()-1 Цикл
		...
		Если СтруктураНаборовСвязываемыхЭлементов.Свойство(ПолеОтбора.Имя) Тогда
			//Тогда имеет место комплект (возможно, неполный) элементов управления для него
...
// измерении, встречающемся в строках и столбцах, но тогда отчет будет не читабелен
...
Функция ПроверитьПовторыИзмеренийПостроителя(ПостроительОтчета) Экспорт
	...
	Для Инд=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1  Цикл
		...
		//  у них будет неправильное автоматически созданное имя, оканчивающееся на число
...
// Это делается для избежания ложного впечатления о том, что что-то как-то отсортировали
