...
//Где - строка
//НаЧто - строка
Функция уатЗаменитьОдниСимволыДругими(Что,Где,НаЧто) Экспорт
	Рез = Где;
...
//
Функция уатСтрокаНаписанаРусскимиИлиЛатинскими(Знач СтрокаПараметр) Экспорт
...
КонецФункции // уатРегламентированнаяОтчетность.уатСтрокаНаписанаРусскимиИлиЛатинскими()
...
//  Режим		   - Число : 0 - дополнять строку слева, 1 -дополнять строку справа
//
// Возвращаемое значение:
//  Строка дополненная указанным символом до указанной длины
//
Функция уатДополнитьСтроку(Знач Стр, Длина, Чем=" ", Режим = 0) Экспорт
...
//		например:
//				уатПреобразоватьЦифруВРимскуюНотацию(7,"I","V","X") = "VII"
//
Функция уатПреобразоватьЦифруВРимскуюНотацию(Цифра,РимскаяЕдиница,РимскаяПятерка,РимскаяДесятка) Экспорт
...
#Если Клиент Тогда
	...
	//		например:
	//				уатПреобразоватьЧислоВРимскуюНотацию(17) = "ХVII"
	//
	Функция уатПреобразоватьЧислоВРимскуюНотацию(АрабскоеЧисло, ИспЛатинскиеБуквы = Ложь) Экспорт
		
		РимскоеЧисло="";
		cRab = уатДополнитьСтроку(АрабскоеЧисло,3);
		...
		РимскоеЧисло = РимскоеЧисло + уатПреобразоватьЦифруВРимскуюНотацию(nSt,c100,c500,c1000);
		РимскоеЧисло = РимскоеЧисло + уатПреобразоватьЦифруВРимскуюНотацию(nDs,c10,c50,c100);
		РимскоеЧисло = РимскоеЧисло + уатПреобразоватьЦифруВРимскуюНотацию(nEd,c1,c5,c10);
	...
	//		например:
	//				уатПреобразоватьЧислоВАрабскуюНотацию("ХVII") = 17
	//
	Функция уатПреобразоватьЧислоВАрабскуюНотацию(РимскоеЧисло) Экспорт
...
// ВызваноИзОтчета - откуда вызывается процедура, Истина из отчета при восстановлении, Ложь - извне
//
Процедура уатКонвертацияОтчетнойФормы(СохраненныйДок = Неопределено, ЭтаФормаД = Неопределено, ВерсияФормы = Неопределено, ПоказателиОтчета = Неопределено) Экспорт
	Перем ТаблицаСтраницРаздела;
	Перем ДанныеМногостраничныхРазделов;
	Перем ДанныеМногострочныхРазделов;
	
	Если СохраненныйДок = Неопределено Тогда
		ВызваноИзОтчета = Истина;
	Иначе
		ВызваноИзОтчета = Ложь;
	КонецЕсли;
	
	Если НЕ ВызваноИзОтчета Тогда
			...
			Если ТаблицаСтраницРаздела.Количество() = 0 Тогда
				...
				Если НЕ МногострочнаяЧастьОтсутствует Тогда
					...
					Для Каждого ЭлементСтруктуры Из ДанныеМногострочныхРазделов Цикл
						...
						Если уатОпределитьПринадлежностьМногострочнойЧастиКЛистуДекларации(ИмяГруппы, ВыбраннаяФорма, 1) = ИмяСтраницы Тогда
...
//
Процедура уатИзменитьИменаПоказателейВСохраненномОтчете(СохраненныйДок) Экспорт
...
//
Функция уатОпределитьПринадлежностьМногострочнойЧастиКЛистуДекларации(ИдГруппы, ИмяФормы, НомерКонвертации)
...
//
Процедура уатроДобавитьКолонкуАвтоматическогоПредставленияВСтруктуруМногостраничныхРазделов() Экспорт
	...
	Выборка = Документы.уатРегламентированныйОтчет.Выбрать();
...
Функция уатСтраховойНомерПФРСоответствуетТребованиям(СтраховойНомер) Экспорт
...
#Если Клиент Тогда
	...
	Функция уатПроверитьНомерУдостоверенияЛичности(ДокументКодИМНС, Знач Номер, СтрОшибка) Экспорт
		
		Если ПустаяСтрока(уатЗаменитьОдниСимволыДругими("0123456789",Номер,"          "))=0 Тогда
	...
	Функция уатПроверитьСериюУдостоверенияЛичности(ДокументКодИМНС, Знач Серия , СтрОшибка) Экспорт
		...
		Если ТипДока = 1 Тогда // паспорт СССР и свид о рожд
			...
			Если ПустаяСтрока(Ч1) Тогда
				...
				Возврат Ложь;
			ИначеЕсли  ПустаяСтрока(уатЗаменитьОдниСимволыДругими("IVXLC1УХЛС",Ч1,"          "))=0 Тогда
				СтрОшибка = "Числовая часть серии документа должна указываться символами 1 У Х Л С  или  I V X L C";
				Возврат Ложь;
			ИначеЕсли уатЗаменитьОдниСимволыДругими("IVXLC",Ч1,"1УХЛС") <> уатПреобразоватьЧислоВРимскуюНотацию(уатПреобразоватьЧислоВАрабскуюНотацию(уатЗаменитьОдниСимволыДругими("IVXLC",Ч1,"1УХЛС"))) Тогда
				СтрОшибка = "Числовая часть серии документа указана некорректно!";
				Возврат Ложь;
			ИначеЕсли (СтрДлина(Ч2) <> 2) ИЛИ (ПустаяСтрока(уатЗаменитьОдниСимволыДругими("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ",Ч2,"                                 "))=0) Тогда
		...
		ИначеЕсли ТипДока=2 Тогда // серия - две буквы: военный билет, ...
			...
			Если (СтрДлина(Серия) <> 2) ИЛИ (ПустаяСтрока(уатЗаменитьОдниСимволыДругими("АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ",Серия,"                                 "))=0) Тогда
		...
		ИначеЕсли ТипДока=3 Тогда // серия - две цифры: загранпаспорта 
			...
			Если (СтрДлина(Серия) <> 2) ИЛИ (ПустаяСтрока(уатЗаменитьОдниСимволыДругими("0123456789",Серия,"          "))=0) Тогда
		...
		ИначеЕсли ТипДока=4 Тогда // серия - две группы цифр: новый паспорт
			...
			Если (СтрДлина(ПервЧасть) <> 2) ИЛИ
				(ПустаяСтрока(уатЗаменитьОдниСимволыДругими("0123456789",ПервЧасть,"          "))=0) Тогда
				СтрОшибка = "Первая группа символов серии документа должна содержать две цифры!";
				Возврат Ложь;
			КонецЕсли;
			
			Если (СтрДлина(ВторЧасть) <> 2) ИЛИ
				(ПустаяСтрока(уатЗаменитьОдниСимволыДругими("0123456789",ВторЧасть,"          "))=0) Тогда
...
// Возвращает код региона из адресного классификатора по его Названию
Функция уатКодРегионаПоНазванию(Название) Экспорт
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Выборка.Следующий();
		Возврат Выборка.КодРегионаВКоде;
...
#Если Клиент Тогда
	...
	Функция уатПредставлениеАдресаДляВыгрузки(Знач АдресВФормате9Запятых) Экспорт
		...
		КодРегиона = Формат(уатКодРегионаПоНазванию(СоставляющиеАдреса[2]), "ЧЦ=2; ЧН=; ЧВН=");
	...
	Функция уатИННСоответствуетТребованиям(Знач ИНН, ВладелецИНН) Экспорт
		
		ИНН = СокрЛП(ИНН);
		ДлинаИНН =  СтрДлина(ИНН);
		
		Если НЕ уатТолькоЦифрыВСтроке(ИНН) Тогда
			Возврат Ложь;
		КонецЕсли;
	...
	// Во всех остальных случаях следует использовать функцию 
	// уатРегламентированнаяОтчетность.уатИННСоответствуетТребованиям(Знач ИНН, ВладелецИНН)
	// Параметры:
	//		ИНН - строка - проверяемый индивидуальный номер налогоплательщика
	Функция уатИННСоответствуетТребованиямБезТипа(Знач ИНН) Экспорт
		...
		Возврат уатИННСоответствуетТребованиям(ИНН, ВладелецИНН);
	...
	Функция уатКППСоответствуетТребованиям(Знач КПП) Экспорт
		
		Если НЕ уатТолькоЦифрыВСтроке(КПП) Тогда
			Возврат Ложь;
		КонецЕсли;
	...
	Функция уатОГРНСоответствуетТребованиям(Знач ОГРН) Экспорт
		Попытка
	...
	Функция уатСтрокаНаписанаПоРусски(Знач СтрокаПараметр) Экспорт
	...
	Функция уатОтветственныеЛицаОрганизаций(Организация, ДатаСреза, Исполнитель = Неопределено) Экспорт
		...
		Если Организация <> Неопределено тогда
			...
			Если Исполнитель <> Неопределено тогда
				...
				ЗапросПоИсполнителям.УстановитьПараметр("ГоловнаяОрганизация",         уатГоловнаяОрганизация(Организация));
	...
	//
	Функция уатПредставлениеАдреса(Знач АдреснаяСтрока, Способ = 0, ПредставлениеПустого = "<<Адрес не задан>>") Экспорт
		
		Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтрЗаменить(АдреснаяСтрока,",","")) Тогда
			Возврат ПредставлениеПустого;
		КонецЕсли;
		
		СтруктураАдрес = уатРазложитьАдрес(АдреснаяСтрока);
		Если уатАдресСоответствуетТребованиям(СтруктураАдрес) Тогда
			Адрес = "";
			Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Страна) Тогда
			...
			Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Индекс)и(Способ<>1) тогда
				Адрес = Адрес + СтруктураАдрес.Индекс;
			КонецЕсли;	 
			Адрес =Адрес + ?(НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Регион),", "+СтруктураАдрес.Регион,""); 
			Адрес =Адрес + ?(НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Район),", "+СтруктураАдрес.Район,""); 
			Адрес =Адрес + ?(НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Город),", "+СтруктураАдрес.Город,""); 
			Адрес =Адрес + ?(НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.НаселенныйПункт),", "+СтруктураАдрес.НаселенныйПункт,""); 
			Адрес =Адрес + ?(НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Улица),", "+СтруктураАдрес.Улица,""); 
			Адрес =Адрес + ?(НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Дом),", д."+СтруктураАдрес.Дом,""); 
			Адрес =Адрес + ?(НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Корпус),", корп. "+СтруктураАдрес.Корпус,""); 
			Адрес =Адрес + ?(НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураАдрес.Квартира),", кв. "+СтруктураАдрес.Квартира,""); 
	...
	//
	Функция уатАдресСоответствуетТребованиям(Знач СтруктураАдрес) Экспорт
		...
		ИначеЕсли СтрДлина(СокрЛП(СтруктураАдрес.Индекс)) <> 6  Тогда   // в индексе ровно 6 символов
			Возврат Ложь;
		ИначеЕсли Не уатТолькоЦифрыВСтроке(СтруктураАдрес.Индекс) Тогда   // индекс состоит из цифр
			Возврат Ложь;
		...
		ИначеЕсли СтрДлина(СтруктураАдрес.Район) > 35 Тогда   // наим.района не > 35 символов
			Возврат Ложь;
		ИначеЕсли НЕ уатСтрокаНаписанаПоРусски(СтруктураАдрес.Район) Тогда   // наим.района написано не русскими буквами
			Возврат Ложь;
		ИначеЕсли СтрДлина(СтруктураАдрес.Город) > 35 Тогда   // наим.города не > 35 символов
			Возврат Ложь;
		ИначеЕсли НЕ уатСтрокаНаписанаПоРусски(Город) Тогда   // наим.города написано не русскими буквами
			Возврат Ложь;
		ИначеЕсли СтрДлина(СтруктураАдрес.НаселенныйПункт) > 45 Тогда   // наим.нас.пункта не > 45 символов
			Возврат Ложь;
		ИначеЕсли НЕ уатСтрокаНаписанаПоРусски(НаселенныйПункт) Тогда   // наим.нас.пункта написано не русскими буквами
			Возврат Ложь;
		ИначеЕсли СтрДлина(СтруктураАдрес.Улица) > 45 Тогда   // наим.улицы не > 45 символов
			Возврат Ложь;
		ИначеЕсли НЕ уатСтрокаНаписанаПоРусски(Улица) Тогда   // наим.улицы написано не русскими буквами
			Возврат Ложь;
...
Функция уатПолучитьТекстФайлаИзРегистра(ДокументСсылка) 
	...
	|ИЗ
	|	РегистрСведений.уатАрхивДанныхРегламентированнойОтчетности КАК АрхивДанныхРегламентированнойОтчетности
	|
	|ГДЕ
...
Функция уатПолучитьТекстФайла(ДокументОбъект,Отказ) Экспорт
	
	Если ДокументОбъект.Проведен тогда
		//Для проведенного документа берём сохраненный ранее текст файла 
		Отказ = Ложь;
		Возврат уатПолучитьТекстФайлаИзРегистра(ДокументОбъект.Ссылка);
...
Функция уатСобратьАдрес(Знач СтруктураАдрес) Экспорт
...
#Если Клиент Тогда
	...
	Функция уатРазложитьСериюДокумента(Знач ВидДокумента, Знач СерияДокумента) Экспорт
		
		Часть1 = "";
		Часть2 = "";
		
		Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВидДокумента) тогда
			...
			Если (КодДока = "01") или (КодДока = "03")  Тогда
				...
				Часть1 = ?(Разделитель = 0, СерияДокумента, уатЗаменитьОдниСимволыДругими("1УХЛС", ВРег(СокрЛП(Лев(СерияДокумента, Разделитель-1))), "IVXLC"));
	...
	Функция уатПредставлениеМестаРождения(Знач СтрокаМестоРождения) Экспорт
		
		СтруктураМестоРождения = уатРазложитьМестоРождения(СтрокаМестоРождения, Ложь);
		
		Если СтруктураМестоРождения.Особое = 1 Тогда
			
			Представление	=	"особое" +
			?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураМестоРождения.НаселенныйПункт),		"",	"  "	+	СокрЛП(СтруктураМестоРождения.НаселенныйПункт))
			+?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураМестоРождения.Район),	"",	"  "	+	СокрЛП(СтруктураМестоРождения.Район))
			+?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураМестоРождения.Область),	"",	"  "	+	СокрЛП(СтруктураМестоРождения.Область))
			+?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураМестоРождения.Страна),	"",	"  "	+	СокрЛП(СтруктураМестоРождения.Страна));
			
		Иначе
			
			Представление	= "" + ?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураМестоРождения.НаселенныйПункт),		"",	"Населенный пункт: " + СокрЛП(СтруктураМестоРождения.НаселенныйПункт))
			+?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураМестоРождения.Район),	"",	", район:  " + СокрЛП(СтруктураМестоРождения.Район))
			+?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураМестоРождения.Область),	"",	", область: "	+	СокрЛП(СтруктураМестоРождения.Область))
			+?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтруктураМестоРождения.Страна),	"",	", страна: "	+	СокрЛП(СтруктураМестоРождения.Страна));
	...
	Функция уатРазложитьАдрес(Знач СтрокаАдрес) Экспорт
	...
	Функция уатРазложитьМестоРождения(Знач СтрокаМестоРождения, ВерхнийРегистр = Истина) Экспорт
		
		Особое = 0;НаселенныйПункт	= "";Район	= "";Область	= "";Страна	= "";
		
		//МассивМестоРождения	=	уатОбщегоНазначенияТиповые.РазложитьСтрокуВМассивПодстрок(Врег(СтрокаМестоРождения));
...
//==================================================================================
	...
	Функция уатТребованиеЗаписиДокументаУдовлетворено(ДокументОбъект,ТекстВопросаПодтверждения) Экспорт
	...
	Функция уатПолучитьПутьВыгрузки(ПутьДляВыгрузки = Неопределено) Экспорт
		
		ФормаПараметровСохранения = уатроПолучитьОбщуюФорму("НастройкаПараметровСохраненияРегламентированногоОтчета");
...
//==================================================================================
	...
	Процедура уатПоказатьФайлДокумента(ДокументОбъект, ЗаголовокОкна) Экспорт
		
		Вопрос = "Перед формированием файла необходимо записать документ. Записать?";
		Если НЕ уатТребованиеЗаписиДокументаУдовлетворено(ДокументОбъект,Вопрос) тогда
			Возврат;
		КонецЕсли;	 
		
		ЕстьОшибки = Ложь;
		ТекстФайла = уатПолучитьТекстФайла(ДокументОбъект,ЕстьОшибки);
...
//
	Процедура уатЗаписатьФайлДокументаНаДискету(ДокументОбъект, ТекущийПользователь, ИмяФайла) Экспорт
...
//
	Процедура уатРаспечататьФайлДокумента(ДокументОбъект) Экспорт
...
//Возвращает счетчик выгруженных файлов по организации за период, обозначенный ДатойИзПериода
//
	Функция уатПолучитьСчетчикВыгруженныхФайлов(Организация, ТипСчетчика = Неопределено, Знач ДатаОтчета = Неопределено, ИФНС = "") Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ИЗ
	                      |	РегистрСведений.уатСчетчикиВыгрузок КАК СчетчикиВыгрузок
	                      |ГДЕ
	...
	Запрос.УстановитьПараметр("Тип", ?(ТипСчетчика = Неопределено, Перечисления.уатТипыСчетчиковВыгрузки.СчетчикФайлов, ТипСчетчика));
	...
	КонецЕсли;
	Возврат 1;
	//Отбор = Новый Структура("Тип, Организация", ?(ТипСчетчика = Неопределено, 
	//	Перечисления.уатТипыСчетчиковВыгрузки.СчетчикФайлов, ТипСчетчика), Организация);
	//РезФ = РегистрыСведений.уатСчетчикиВыгрузок.СрезПоследних(ДатаОтчета, Отбор);
...
//Возвращает счетчик выгруженных документов по организации за период, обозначенный ДатойИзПериода
//	
// Параметры:
//  Организация  - Организация по которой необходимо получить счетчик
//  ТипСчетчика  - 
//
// Возвращаемое значение:
//  Значение счетчика
	Функция уатПолучитьСчетчикВыгруженныхДокументов(Организация, ДатаДокумента = Неопределено) Экспорт
		
		Если Организация = Неопределено Тогда
			Возврат 0;
		КонецЕсли;
		
		Отбор = Новый Структура("Тип, Организация", Перечисления.уатТипыСчетчиковВыгрузки.СчетчикДокументовВФормате300, Организация);
		РезД = РегистрыСведений.уатСчетчикиВыгрузок.СрезПоследних(КонецГода(?(ДатаДокумента = Неопределено, РабочаяДата, ДатаДокумента)), Отбор);
	...
	Процедура уатУстановитьСчетчикВыгруженныхДокументов(Организация, СчДок, ДатаДокумента = Неопределено) Экспорт
		
		МенЗап = РегистрыСведений.уатСчетчикиВыгрузок.СоздатьМенеджерЗаписи();
		МенЗап.Период = НачалоГода(?(ДатаДокумента = Неопределено, РабочаяДата, ДатаДокумента));
		МенЗап.Организация = Организация;
		МенЗап.Тип = Перечисления.уатТипыСчетчиковВыгрузки.СчетчикДокументовВФормате300;
	...
	Процедура уатУстановитьСчетчикВыгруженныхФайлов(Организация, СчФайлов, ТипСчетчика = Неопределено, ПериодУстановки = Неопределено, ИФНС = "") Экспорт
		МенЗап = РегистрыСведений.уатСчетчикиВыгрузок.СоздатьМенеджерЗаписи();
		МенЗап.Период = НачалоГода(?(ПериодУстановки = Неопределено, РабочаяДата, ПериодУстановки));
		МенЗап.Организация = Организация;
		МенЗап.Тип = ?(ТипСчетчика = Неопределено, Перечисления.уатТипыСчетчиковВыгрузки.СчетчикФайлов, ТипСчетчика);
	...
	Функция уатВерсияФорматаВыгрузки(РеглДок, НаДату = Неопределено) Экспорт
		
		Попытка
			Возврат уатРеглОтчеты(РеглДок.ИсточникОтчета).ПолучитьФорму().ПолучитьВерсиюФорматаВыгрузки(НаДату);
			//Возврат уатФормаРеглОтчета(РеглДок.ИсточникОтчета).ПолучитьВерсиюФорматаВыгрузки(НаДату);
		Исключение
	...
	//
	// Параметры:
	//	ДокументСсылка.РегламентированныйОтчет - отчет, который требуется выгрузить.
	//
	// Возвращаемое значение:
	//	Булево - Истина, если отчет удалось выгрузить; Ложь - в противном случае.
	//
	Функция уатПроверитьРегламентированныеОтчеты(Доки) Экспорт
		...
		Попытка
			...
			//	КонецЕсли;
			//КонецЦикла;
			НовыйДок = Документы.уатВыгрузкаРегламентированныхОтчетов.СоздатьДокумент();
...
// функция формирует версию программы
//
Функция уатСформироватьВерсПрог() Экспорт
	...
	КонецЕсли;
	Возврат Лев("1С:ПРЕДПРИЯТИЕ " + СокрЛП(РедакцияПриложения) + " УАТ Проф " + СокрЛП(уатОбщегоНазначения.уатНомерВерсииКонфигурации()), 40);
...
Функция уатПустоеЗначение(Реквизит) Экспорт
...
Функция уатЭтоКодПоказателяП(КодПок) Экспорт
...
Процедура уатВывестиПоказатель(Текст, ЗначениеЯчейки, ИмяЯчейки, ПризнакФормата, ФорматВыгрузки) Экспорт
	
	Если уатПустоеЗначение(ЗначениеЯчейки) И ПризнакФормата <> "Д" Тогда
	...
	Иначе
		...
		ИначеЕсли ПризнакФормата = "Д" Тогда 
			Если уатПустоеЗначение(Дата(ЗначениеЯчейки)) = 1 Тогда
				Текст.ДобавитьСтроку(ИмяЯчейки + ":");
			Иначе
...
Функция уатВычислитьЗначениеПоКорню(Табл, Корень) Экспорт
...
//Функция уатсобирает значения из отдельных "ячеек", например, ИНН1 из ИНН1_1 + ИНН1_2 + ...
Функция уатСвернутьДанныеТабличногоПоля(Данные) Экспорт
	...
	Для Каждого Стр Из ТП Цикл
		...
		Если уатЭтоКодПоказателяП(Имя) Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого Стр Из ТП2 Цикл
		...
		Если Стр.Кол > 1 Тогда
			ЗначениеСводногоПоказателя = уатВычислитьЗначениеПоКорню(ТП, Стр.Имя);
...
#Если Клиент Тогда
	...
	Функция уатВывестиСведенияСлужебнойЧасти30(Текст, Организация = Неопределено, Знач ДатаДок = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач КодИнсп = Неопределено, ТипИнф = "ОТЧЕТНОСТЬНП", ВерсияФормата = "3.00") Экспорт
		...
		Текст.ДобавитьСтроку("ВерсПрог:" + уатСформироватьВерсПрог());
	...
	Функция уатВывестиСведенияОбОтправителе30(Текст, Организация = Неопределено, Знач ОргНазв = Неопределено, Знач ОргТелефон = Неопределено, Знач ОргБухгалтер = Неопределено, ВыгрузкаДекларацииПоАлкоголю = Ложь) Экспорт
		...
		Если Организация <> Неопределено Тогда
			...
			Если ОргТелефон = Неопределено Тогда
				ЗапрашиваемыеПараметры = Новый СписокЗначений;
				ЗапрашиваемыеПараметры.Добавить("ТелОрганизации", "ТелОрганизации");
				ОргТелефон = СокрЛП(уатПолучитьСведенияОбОрганизации(Организация, РабочаяДата, ЗапрашиваемыеПараметры).ТелОрганизации);
			КонецЕсли;
			...
			Если ОргБухгалтер = Неопределено Тогда
				...
				ОргСведения = уатПолучитьСведенияОбОрганизации(Организация, РабочаяДата, Сведения);
				ОргБухгалтер = "";
				ОргСведения.Свойство("ФИОБух", ОргБухгалтер);
				ОргБухгалтер = СокрЛП(ОргБухгалтер);
		...
		Если КатегорияНП = 1 ИЛИ ВыгрузкаДекларацииПоАлкоголю Тогда
			...
			Текст.ДобавитьСтроку("ТелОтпр:"    + ?(ПустаяСтрока(ОргТелефон), "-", СокрЛП(ВРЕГ(Лев(ОргТелефон, 20)))));
			Текст.ДобавитьСтроку("ДолжнОтпр:ГЛАВНЫЙ БУХГАЛТЕР");
			Текст.ДобавитьСтроку("ФИООтпр:"   + уатСтрЗаменитьПробелы(ОргБухгалтер));
			Текст.ДобавитьСтроку("@@@"); 
		Иначе                  
			// отправитель - ФЛ 
			Текст.ДобавитьСтроку("ФИООтпрФЛ:" + уатСтрЗаменитьПробелы(уатПолучитьСведенияОбОрганизации(Организация, РабочаяДата, "ФИО").ФИО));
			Текст.ДобавитьСтроку("ТелОтпр:"   + ?(ПустаяСтрока(ОргТелефон), "-", СокрЛП(ВРЕГ(Лев(ОргТелефон, 20))))); 
			Текст.ДобавитьСтроку("@@@"); 
		КонецЕсли;
	...
	Функция уатВывестиОбщиеСведенияИнформационнойЧасти30(Текст, Организация = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач ДатаДок = Неопределено) Экспорт
		...
		СчДок = уатПолучитьСчетчикВыгруженныхДокументов(Организация, ДатаДок) + 1;
	...
	//Выводит общие сведения информационной части (формат 3.00)
	Функция уатВывестиОбщиеСведенияИнформационнойЧасти201(Текст, Организация = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач ДатаДок = Неопределено) Экспорт
	...
	Функция уатПроверитьНаКорректность(НазваниеОтчета, Стр, Значение, ВыводитьСообщения, НомерСтраницы = Неопределено, ФормаВыводаСообщений = Неопределено) Экспорт
		...
		Если ТипЗнч(Значение) = Тип("Строка") И уатЭтоКодПоказателяП(Стр.КодПоказателяПоСоставу) Тогда
			Возврат 0;
		КонецЕсли;
		
		Если ДопОписание = "ИНН" Тогда
			Если НЕ уатИННСоответствуетТребованиямБезТипа(Значение) Тогда
				Если ФормаВыводаСообщений <> Неопределено Тогда
					...
					НовСтр.Строка = уатСтрокаПоКодуПоказателя(Стр.КодПоказателяПоСоставу);
					НовСтр.Графа = уатГрафаПоКодуПоказателя(Стр.КодПоказателяПоСоставу);
		...
		ИначеЕсли ДопОписание = "КПП" Тогда
			...
			Если НЕ уатКППСоответствуетТребованиям(Значение) Тогда
				Если ФормаВыводаСообщений <> Неопределено Тогда
					НовСтр = ФормаВыводаСообщений.ТаблицаСообщений.Добавить();
					НовСтр.Раздел = Стр.ИмяПоляТаблДокумента;
					НовСтр.Строка = уатСтрокаПоКодуПоказателя(Стр.КодПоказателяПоСоставу);
		...
		ИначеЕсли ДопОписание = "ОГРН" Тогда
			...
			Если НЕ уатОГРНСоответствуетТребованиям(Значение) Тогда
				Если ФормаВыводаСообщений <> Неопределено Тогда
					НовСтр = ФормаВыводаСообщений.ТаблицаСообщений.Добавить();
					НовСтр.Раздел = Стр.ИмяПоляТаблДокумента;
					НовСтр.Строка = уатСтрокаПоКодуПоказателя(Стр.КодПоказателяПоСоставу);
...
Функция уатРазбитьПоПодразделам(ИмяРаздела, мТаблицаСоставПоказателей, МногострочныеИсключения = Неопределено) Экспорт
...
// (ошибка - отсутствие значения у обязательного к заполнению показателя) в таблицу сообщений, 
// определенную в модуле формы, из которой вызвана функция
// НомерСтраницы - номер страницы, на которой находится проверяемый показатель
...
// ФормаВыводаСообщений - форма, из которой была вызвана функция
Функция уатПроверитьНаПустоеЗначение(НазваниеОтчета, Стр, Значение, ВыводитьСообщения, НомерСтраницы = Неопределено, НомерСтроки = Неопределено, ПроверкаЗаполненияСтр = 0, ФормаВыводаСообщений = Неопределено) Экспорт
	...
	Если уатПустоеЗначение(Значение) Тогда
		Если ВыводитьСообщения Тогда
			Раздел = уатРазделПоКодуПоказателя(Стр.КодПоказателяПоСоставу);
			Стрк = уатСтрокаПоКодуПоказателя(Стр.КодПоказателяПоСоставу);
			Графа = уатГрафаПоКодуПоказателя(Стр.КодПоказателяПоСоставу);
...
Функция уатУдобочитаемоеПредставлениеПоКодуПоказателя(КодПоказателя) Экспорт
	
	Если уатЭтоКодПоказателяП(КодПоказателя) Тогда
...
Функция уатПредставлениеДокументаРеглОтч(Док) Экспорт
...
Функция уатФИОНаписаноВерно(Знач СтрокаПараметр) Экспорт
...
// Если оба реквизита не определенны, функция возвращает Неопределено
Функция уатПолучитьРеквизит(Реквизит1, Реквизит2) Экспорт
...
Функция уатСкопированнаяСтруктура(Структура1) Экспорт
...
Функция уатПолучитьКоличествоПоказателей(Текст) Экспорт
...
Функция уатНайтиКолПокФОтч(Текст) Экспорт
...
//
Функция уатСтрЗаменитьПробелы(ИсхСтрока) Экспорт
...
Функция уатЕстьСоответствующаяОбластьГруппСтрок(СледСтрока, ГруппыСтрок)
...
Функция уатМаксимальноеКоличествоОбластейМногострочнойЧасти(МногострочныеЧасти)
	Максимум = 0;
...
Функция уатЭтоОднаИзМногострочныхЧастей(ЭлСпКоорд, МногострочныеЧасти, ШагСтроки = 1)
...
Функция уатПолучитьКартуЛистовРаздела(Форма, ИмяРаздела)
...
#Если Клиент Тогда
	...
	Функция уатПолучитьНизМногострочнойЧасти(Форма, Раздел, ИмяОбласти)
		ГруппыСтрок = уатПолучитьКартуИменованныхГруппСтрок(Форма, Раздел);
		...
		Пока Не уатЕстьСоответствующаяОбластьГруппСтрок(Низ, ГруппыСтрок) Цикл
			Низ = Низ + 1;
		Конеццикла;
		Возврат Низ - 1;
	...
	Функция уатПолучитьОбластиМногострочнойЧасти(Форма, Раздел, ИмяОбласти)
		...
		Шаг = уатПолучитьШагСтрокиДляТабличногоПоля(Обл);
		...
		Пока Не уатЕстьСоответствующаяОбластьГруппСтрок(СледСтрока, ГруппыСтрок) Цикл
	...
	Функция	уатПолучитьКартуМногострочныхСекцийЛиста(Форма, ИмяТабличногоПоля, Верх, Низ)
	...
	// раздела с именем ИмяЛиста. Сформированные листы хранятся в списке значений мПечатныеФормы
	Функция уатКоличествоЛистовПоИмениЛиста(мПечатныеФормы, ИмяЛиста)
	...
	Процедура уатВывестиНаПечатьЛистОбщая(Форма, СтруктураПараметровЛиста) Экспорт
		...
		Перем ИмяОбластиТаблПоля; // имя области табличного документа, выводимой на печать;
		...
		КартаСтраницРаздела = уатПолучитьКартуЛистовРаздела(Форма, ИмяРаздела);
		НомЛиста = 0;
		...
		Для Каждого Стр Из КартаСтраницРаздела Цикл
			...
			Состояние(Форма.Заголовок + ". Формирование печатной формы " + ИмяЛиста + ". Лист №" + (уатКоличествоЛистовПоИмениЛиста(Форма.мПечатныеФормы, ИмяЛиста) + 1));
			
			СтруктураПараметровЛиста.Вставить("МногострочнаяЧасть", уатПолучитьКартуМногострочныхСекцийЛиста(Форма, ИмяТабличногоПоля, Стр.Верх, Стр.Низ));
			
			ПечатныеЛисты = уатВывестиНаПечатьЛистРаздела(Форма, СтруктураПараметровЛиста);
	...
	Функция уатУдалосьПроставитьНомерСтраницы(ТаблДок,Знач НомерЛиста)
	...
	// после нумерации выводит листы в общую форму ПечатьРегламентированныхОтчетов.
	// Параметры:
	...
	Процедура уатПроставитьНомераЛистов(Форма, ВидПечати, ПризнакНумеруемыхФорм = Истина) Экспорт;
		
		Если ПризнакНумеруемыхФорм Тогда
			Если Форма.мСчетчикСтраниц <> Истина Тогда
				НомерЛиста = 1;
				Для Каждого Эл Из Форма.мПечатныеФормы Цикл
					Если НЕ уатУдалосьПроставитьНомерСтраницы(Эл.Значение, НомерЛиста) Тогда
		...
		Если НЕ уатНаложитьДвухмерныйШтрихКод(Форма) Тогда
		...
		ПредПросмотр = уатроПолучитьОбщуюФорму("ПечатьРегламентированныхОтчетов", Форма);
	...
	// данному разделу.
	Функция уатПолучитьКартуИменованныхГруппСтрок(Форма, Раздел)
		Карта = Новый ТаблицаЗначений;
		Карта.Колонки.Добавить("Верх");
		Карта.Колонки.Добавить("Низ");
		ТаблПоле = Форма.ЭлементыФормы["ПолеТабличногоДокумента" + Раздел];
	...
	Функция уатПолучитьПустуюСтрокуМногострочнойЧасти(МногострочныеЧасти, НомСтр)
	...
	Функция уатВывестиНаПечатьЛистРазделаБезМногострочныхЧастей(Форма, СтруктураПараметровЛиста)
		Перем ИмяЛиста;           // имя выводимого на печать листа;
		Перем Ориентация;         // ориентация страницы;
		Перем ИмяТабличногоПоля;  // имя табличного поля, который выводится на печать;
		Перем ИмяОбластиТаблПоля; // имя области табличного документа, выводимой на печать;
...
// процедура выполняет обнуление итоговых показателей на текущем листе 
// табличного документа.
//
// Параметры:
//  ТаблДок      - табличный документ на котором необходимо обнулить показатели
	Процедура уатОбнулитьИтоговыеПоказателиНаТекущемЛисте(ТаблДок)
	...
	Функция уатВывестиНаПечатьЛистРаздела(Форма, СтруктураПараметровЛиста)
		Перем ИмяЛиста;           // имя выводимого на печать листа;
		Перем Ориентация;         // ориентация страницы;
		Перем ИмяТабличногоПоля;  // имя табличного поля, который выводится на печать;
		Перем ИмяОбластиТаблПоля; // имя области табличного документа, выводимой на печать;
		...
		Если ИмяОбластиМногострочнойЧасти = Неопределено Тогда
			СписокПечатныхФорм.Добавить(уатВывестиНаПечатьЛистРазделаБезМногострочныхЧастей(Форма, СтруктураПараметровЛиста));
		...
		Для Каждого Область Из ТабличноеПоле.Области Цикл
			...
			Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
				...
				Если Найти(ВРЕГ(Область.Имя), "МНОГОСТРОЧНАЯЧАСТЬ") <> 0 Тогда
					СписокВыводимыхНаПечатьОбластей.Добавить(уатПолучитьНизМногоСтрочнойЧасти(Форма, ИмяРаздела, Область.Имя), Область.Имя);
				Иначе
		...
		Если ТипЗнч(ИмяОбластиМногострочнойЧасти) = Тип("Строка") Тогда
			МногострочныеЧасти.Вставить(ИмяОбластиМногострочнойЧасти, уатПолучитьОбластиМногоСтрочнойЧасти(Форма, ИмяРаздела, ИмяОбластиМногострочнойЧасти));
		Иначе
			Для Каждого Эл Из ИмяОбластиМногострочнойЧасти Цикл
				МногострочныеЧасти.Вставить(Эл, уатПолучитьОбластиМногоСтрочнойЧасти(Форма, ИмяРаздела, Эл));
			КонецЦикла;
		КонецЕсли;
		...
		НомСтр = 0;
		Для Каждого Эл Из МногострочныеЧасти Цикл
			ПустыеСтрокиМногострочныхЧастей[НомСтр] = уатПолучитьПустуюСтрокуМногострочнойЧасти(Эл, 0);
			НомСтр = НомСтр + 1;
		КонецЦикла;
		...
		Пока уатМаксимальноеКоличествоОбластейМногострочнойЧасти(МногострочныеЧасти) > 0 Цикл
			...
			Пока ПечатнаяФорма.ПроверитьВывод(Строки) Цикл
				...
				Если уатМаксимальноеКоличествоОбластейМногострочнойЧасти(МногострочныеЧасти) >= Ном Тогда
					НомерМнЧ = 0;
					Для Каждого Эл Из МногострочныеЧасти Цикл
						Попытка
			...
			Для Сч = 0 По СписокВыводимыхНаПечатьОбластей.Количество() - 1 Цикл
				...
				Если Найти(ВРЕГ(СписокВыводимыхНаПечатьОбластей.Получить(Сч).Представление), "ОГРАНИЧИТЕЛИ") <> 0 
					...
					Продолжить;
				КонецЕсли;
				Рез = уатЭтоОднаИзМногострочныхЧастей(СписокВыводимыхНаПечатьОбластей.Получить(Сч), МногострочныеЧасти, уатПолучитьШагСтрокиДляТабличногоПоля(ТабличноеПоле.ПолучитьОбласть(СписокВыводимыхНаПечатьОбластей.Получить(Сч).Представление)));
			...
			Если флОбластьОграничителиУмещаетсяНаСтраницу Тогда
				...
				Пока флОбластьОграничителиУмещаетсяНаСтраницу Цикл
					...
					Иначе
						...
						Если уатМаксимальноеКоличествоОбластейМногострочнойЧасти(МногострочныеЧасти) > 0 Тогда
							уатОбнулитьИтоговыеПоказателиНаТекущемЛисте(ИтоговыйТаблДок);
						КонецЕсли;
	...
	//
	Функция уатПолучитьШагСтрокиДляТабличногоПоля(ТабличноеПоле)
...
//
	Процедура уатЗаписатьДанныеФИОВРегистр(Ссылка, Фамилия, Имя, Отчество) Экспорт
...
//==================================================================================
	...
	//
	Процедура уатроВставитьДополнительнуюСтраницу(Форма, ИмяТекущейСтраницыПанели = Неопределено ) Экспорт
		...
		уатроСохранитьДанныеМногостраничногоРазделаВАктивнойСтранице(Форма, ИмяТекущейСтраницыПанели, Истина);
		...
		Если СтрТаблицаСтраницы <> Неопределено Тогда
			...
			КолМногострочныхЧастей = уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "МногострочностьВРазделе");
			...
			// Если многострочность есть, то она удалится и запустится Расчет на стадии
		...
		уатроПоказатьСтатикСтраницРаздела(Форма, ИмяТекущейСтраницыПанели);
	...
	//
	Процедура уатроДоступностьКнопокЛистания(Форма, НомерТекущейСтраницы, КоличествоСтраниц, ИмяТекущейСтраницыПанели) Экспорт
	...
	// Функция уатВычислитьПоказателиТитульногоЛистаИСведенияОФизЛицах
	// Процедура УниверсальнаяВыгрузкаПредписанныхПоказателей
	// Функция ПроверитьПредписанныеПоказатели
	//
	Процедура уатроОбновитьСтруктуруМногостраничныхРазделов(Форма, ИмяСтраницы = Неопределено) Экспорт
		...
		Для Каждого Страница Из Форма.мСтруктураМногостраничныхРазделов Цикл
			...
			Иначе
				...
				уатроСохранитьДанныеМногостраничногоРазделаВАктивнойСтранице(Форма, ИмяТекущейСтраницыПанели, Ложь);
			КонецЕсли;
	...
	//
	Процедура уатроОбнулитьПеременныеМодуляХранящиеЗначенияКвадратногоПредставления(Форма) Экспорт
	...
	//
	Процедура уатроОпределитьПредставлениеДопСтраниц(Форма, ИмяСтраницы = Неопределено, ТолькоУАктивнойСтраницы = Неопределено) Экспорт
		Перем ТаблицаСтраницРаздела;
		...
		Для Каждого Страница Из Форма.мСтруктураМногостраничныхРазделов Цикл
			...
			Если ТаблицаСтраницРаздела.Количество() > 0 Тогда
				...
				Для Каждого СтрокаТаблицы Из ТаблицаСтраницРаздела Цикл
					...
					ИмяПредставления = уатроПолучитьСвойствоРаздела(Форма, ИмяТекСтраницы, "ИмяПредставления");
					
					Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ИмяПредставления) Тогда
					...
					Если (НЕ ТолькоАктивная) Или ((ТолькоАктивная) И (СтрокаТаблицы.АктивнаяСтраница))Тогда
						...
						ТекущееПредставлениеСтраницы = ?(уатПустоеЗначение(ТекущееПредставлениеСтраницы), "Лист N " + Строка(НомерСтраницы), ТекущееПредставлениеСтраницы + ". Лист N " + Строка(НомерСтраницы));
	...
	//
	Процедура уатроОткрытьФормуСпискаДопСтраниц(Форма, ИмяСтраницы = Неопределено) Экспорт
		Перем ТаблицаСтраницРаздела;
		...
		//ФормаВыбораДопСтраницы = Обработки.ОперацииРегламентированнойОтчетности.ПолучитьФорму("ФормаСпискаДопСтраниц",
		//	Форма, "дляПереходаНаСтраницу");
		ФормаВыбораДопСтраницы = уатроПолучитьОбщуюФорму("ФормаСпискаДопСтраниц", Форма, "дляПереходаНаСтраницу");
		...
		СтруктураДанных.Вставить("ПерестановкаСтраницЗапрещена", уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "ПерестановкаСтраницЗапрещена", Ложь));
		...
		Если ВыбранноеЗначение <> Неопределено Тогда
			...
			// Так как признак активной страницы не зависит от Модифицированности формы, то,
			//	не анализируя этот показатель, всегда вставляем в структуру многостраничных разделов полученную таблицу.
			...
			уатроОпределитьПредставлениеДопСтраниц(Форма, ИмяТекущейСтраницыПанели, Ложь);
			...
			уатроПоказатьСтраницуМногостраничногоРазделаПриВосстановлении(Форма, ИндексВыбСтраницы, ИмяТекущейСтраницыПанели, Истина, КолМногострочныхЧастей);
			
			// Если на форме доп. страниц изменился порядок страниц или представления,
			//	тогда возведем признак модифицированности и в родительском отчете
	...
	//
	Процедура уатроВывестиЭлементыВариантаЗаполнения(Форма, ТекущаяЯчейка) Экспорт
		Перем Комментарий;
		...
		Если Найти(ИмяПоказателя, ":") > 0 Тогда
			...
			уатроДоступностьЭлементовВариантаЗаполнения(Форма,, ТекущаяЯчейка);
			Возврат;
		КонецЕсли;
		...
		ТаблицаВариантыЗаполнения = уатроПолучитьТаблицуВариантовЗаполнения(Форма, ИмяТекущейСтраницыПанели);
		...
		Если ВариантЗаполнения = 1 ИЛИ 
			...
			уатроДоступностьЭлементовВариантаЗаполнения(Форма, ВариантЗаполнения);
	...
	//
	Процедура уатроДоступностьЭлементовВариантаЗаполнения(Форма, Вариант = Неопределено, ТекущаяОбласть = Неопределено) Экспорт
		...
		Иначе
			...
			КонецЕсли;
			Попытка
				// попытки сделаны для того, чтобы если автозаполнение только 4
			...
			Исключение
				// в случае, если на листе не предусмотрен автоматическое заполнение, кроме признака 4
			КонецПопытки;
	...
	//
	Функция уатроОчистить(Форма, ЗадаватьВопросПередОчисткой = Истина) Экспорт
		...
		// мСвойстваРазделовДекларации, т.к. на одной странице могут быть несколько
		// табличных полей, например Прибыль, раздел Лист03_РазделыБВ
		
		// Переделано из процедуру в функцию
		// возвращает истина - если прошла очистка и ложь - если отказались от очистки
		...
		Если МногостраничностьЕсть Тогда
			...
			Для Каждого Страница Из Форма.мСтруктураМногостраничныхРазделов Цикл
				...
				Если (ТаблицаТекущейСтраницы.Количество() > 1)
					...
					Пока Не (ИндексСтраницы > (ТаблицаТекущейСтраницы.Количество()-1)) Цикл
						...
						// что данные активной страницы были очищены ранее.
				...
				Если (ИмяТекущейСтраницыПанели <> "Лист03_РазделыБВ1")
					...
					ПанельРаздела = уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "НазваниеПанелиТабличногоПоляРаздела");
			...
			уатроОбновитьСтруктуруМногостраничныхРазделов(Форма);
	...
	//
	Процедура уатроПоказатьСтатикСтраницРаздела(Форма, ИмяТекущейСтраницыПанели) Экспорт
		Перем ТаблицаСтраницРаздела;
		...
		уатроДоступностьКнопокЛистания(Форма, НомерТекущейСтраницы, КоличествоСтраниц, ИмяТекущейСтраницыПанели);
	...
	//
	Процедура уатроПоказатьСтраницуМногостраничногоРаздела(Форма, Шаг, ИмяТекущейСтраницыПанели, ПоказатьПоИндексу = Ложь) Экспорт
		Перем ТаблицаСтраницРаздела;
		...
		Если КолМногострочныхЧастей > 0 Тогда
			...
			СохранятьМногострКакТЗ = уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "СохранятьМногострКакТЗ");
		...
		ПредставлениеДанных = уатроПолучитьСвойствоРаздела(Форма, ИмяТекТабличногоПоля, "ПредставлениеДанных");
		...
		Для Каждого Элемент Из СтруктураДанныхСледующаяСтраница Цикл
			...
			// Старая версия формы, нужно по возможности собрать данные
	...
	// КолМногострочныхЧастей - по умолчанию -1. Определяет, работать -ли при показатель с многострочными частями
	...
	//
	Процедура уатроПоказатьСтраницуМногостраничногоРазделаПриВосстановлении(Форма, Шаг, ИмяТекущейСтраницыПанели, УчестьАктивнуюСтраницу = Ложь, КолМногострочныхЧастей = -1) Экспорт
		Перем ТаблицаСтраницРаздела;
		...
		ПредставлениеДанных = уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "ПредставлениеДанных");
		
		// Вариант -1 означает, что процедура запущена из режима восстановить сохраненные данные.
		// В этом случае операции по разметке табличного поля для многострочных разделов не требуются.
		...
		// Специально для декларации по прибыли, не отображать общими средствами
	...
	//
	Функция уатроПолучитьСвойствоРаздела(Форма, Знач ИмяСтраницы, Знач ИмяСвойства, ВозвращаемоеЗначение = Неопределено) Экспорт
	...
	//  деклараций старого периода всегда равно 15.2 и параметр Неопределено.
	// Возвращаемое значение:
	//   таблица значений, содержащая информацию о вариантах автозаполнения ячеек.
	//
	Функция уатроПолучитьТаблицуВариантовЗаполнения(Форма, ИмяСтраницыПанели, ПолучитьЭталонную = Ложь, ТипЗнчСуммы = Неопределено) Экспорт
	...
	// ИспользоватьНомер - В случае, если параметр повторяется на листе несколько раз, например ИНН или 
	//                КПП, по умолчанию включен
	// ЛистыМногостраничные - если лист многостраничных, то проставлять во всех экземплярах листа, иначе, 
	//                только в активном
	//
	Процедура уатроПроставитьНаВсехЛистах(Форма, Знач Значение, Знач ПрефиксНаЛисте, ЛистИсключение = Неопределено, ТолькоДляЛиста = Неопределено, ТолькоАктивныйЛистМногостраничнойЧасти = Ложь, ИспользоватьНомер = Истина, ЛистыМногостраничные = Истина) Экспорт
		...
		Если Найти(ПрефиксНаЛисте, "КПП") > 0 Тогда
			...
			Попытка
				...
				Если уатЭтоПБОЮЛ(Форма.Организация) Тогда
					Значение = "         ";
				КонецЕсли;
		...
		Пока ПереборЗнач > 0 Цикл
			...
			Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ТолькоДляЛиста) Тогда
			...
			Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ЛистИсключение) Тогда
			...
			Если (НЕ ЛистыМногостраничные) Или (РезВыбор) Тогда
				...
				Для Инд = 1 По Колво Цикл
					...
					Исключение
						уатроПроставитьВКвадратыЗначения(Форма, ИмяДляРасчета, ПрефиксНаЛисте + ?(ИспользоватьНомер, Строка(Инд) + "_", ""), СтрДлина(Значение), Значение);
					КонецПопытки;
				КонецЦикла;
				
			ИначеЕсли ЛистыМногостраничные И (НЕ РезВыбор) Тогда
				
				уатроОбновитьСтруктуруМногостраничныхРазделов(Форма, ИмяДляРасчета);
				...
				Для Каждого Страница Из ТаблицаСтраницРаздела Цикл
					...
					Для Инд77 = 1 По Колво Цикл
						...
						Если Страница.АктивнаяСтраница Тогда
							...
							Исключение
								уатроПроставитьВКвадратыЗначения(Форма, ИмяДляРасчета, ПрефиксНаЛисте + ?(ИспользоватьНомер, Строка(Инд77) + "_", ""), СтрДлина(Значение), Значение);
							КонецПопытки;
	...
	// сохраняет данные в активном листе
	// Особенность:
	// Процедура только сохраняет данные, не передвигая страница на начальную
	// или какую либо другую часть многостраничного раздела
	...
	//
	Процедура уатроСохранитьДанныеМногостраничногоРазделаВАктивнойСтранице(Форма, ИмяТекущейСтраницы, ОчищатьРазделЕслиЕстьМногострочность) Экспорт
		Перем ТаблицаСтраницРаздела;
		...
		Если СтрТаблицаСтраницы = Неопределено Тогда
			уатроПоказатьСтатикСтраницРаздела(Форма, ИмяТекущейСтраницы);
			Возврат;
		КонецЕсли;
		...
		// В каждой декларации функция уатСобратьДанныеТекущегоТаблПоля - разные !
		СтруктураДанныхТекущейТаблицы = Форма.СобратьДанныеТекущегоТаблПоля(ТекТабличноеПоле);
		
		КолМногострочныхЧастей = уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницы, "МногострочностьВРазделе");
		Если ТипЗнч(КолМногострочныхЧастей) = Тип("ТаблицаЗначений") Тогда
			КолМногострочныхЧастей = КолМногострочныхЧастей.Количество();
		КонецЕсли;
		...
		СохранятьМногострКакТЗ = уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницы, "СохранятьМногострКакТЗ");
		...
		Если (КолМногострочныхЧастей > 0) И (НЕ СохранятьМногострКакТЗ) Тогда
			...
			// Если на листе больше одной многострочной части, то алгоритм сбора будет другим
		...
		уатроОпределитьПредставлениеДопСтраниц(Форма, ИмяТекущейСтраницы, Истина);
	...
	//
	Процедура уатроУдалитьДополнительнуюСтраницу(Форма, ИмяТекущейСтраницыПанели = Неопределено) Экспорт
		Перем ТаблицаСтраницРаздела;
		...
		Если СтрТаблицаСтраницы <> Неопределено Тогда
			...
			Если КолвоДопСтраниц > 0 Тогда
				...
				КолвоМногострочныхБлоков = уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "МногострочностьВРазделе");
		...
		Если уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "ПредставлениеДанных") Тогда
			Форма.ЗаполнениеЗначенийВКвадратиках(ИмяТекущейСтраницыПанели);
		КонецЕсли;
	...
	//
	// Функция уат определяем, входит ли переданный показатель в многострочную часть
	// Параметры:
	...
	//
	Функция уатроПоказательВходитВМногострочнуюЧасть(Форма, ИмяПоказателя) Экспорт
	...
	//
	// Процедура считывает св-ва всех разделов и если раздел является автозаполняемым, 
	// то добавляется 3 параметра в элементы форм
	//
	// Параметры:
	//  Форма       - форма
	Процедура уатроЗаполнитьЭлементыФормыДляСистемыАвтоЗаполнения(Форма) Экспорт
	...
	//
	Функция уатроЯвляетсяЧислом(Значение) Экспорт
	...
	//
	Функция уатроЯвляетсяДатой(Знач Значение) Экспорт
	...
	//
	Функция уатроКолвоЛистоВРазделе(Форма, ИмяСтраницыПанели) Экспорт
...
//
Процедура уатроПроставитьВКвадратыЗначения(Форма, Знач Лист, Знач Показатель, Знач Размерность, Знач Значение, ДополнитьЗначение = Ложь, НольКакНоль = Ложь) Экспорт
	...
	Если (ДополнитьЗначение) Или ((уатроЯвляетсяЧислом(Значение)) И (Значение = 0)) Тогда

		Если ((уатроЯвляетсяЧислом(Значение)) И (Значение = 0)) Тогда

			Если НольКакНоль Тогда
				ЗначениеДопСтрока = уатДополнитьСтроку("0", Размерность," ");
			Иначе
				ЗначениеДопСтрока = уатДополнитьСтроку("", Размерность," ");
			КонецЕсли;

		Иначе

			ЗначениеДопСтрока = Формат(Значение, СтрокаФорматаЯчейки);
			ЗначениеДопСтрока = уатДополнитьСтроку(ЗначениеДопСтрока, Размерность," ");
...
КонецПроцедуры // ПроставитьВКвадратыЗначения(Лист, Показатель, Размерность, Значение)
	...
	//
	Функция уатроРазделТекущейОбласти(Форма, ТекущаяОбласть) Экспорт
	...
	//
	Функция уатроПолучитьСмысловуюЧастьИмениЯчейки(ИмяЯчейки, Определение = Истина) Экспорт
		...
		Для Инд = 1 По СтрДлина(ИмяЯчейки) Цикл
			...
			Если НЕ уатроЯвляетсяЧислом(СимволСтроки) Тогда
	...
	//
	Процедура уатроУстановитьЗначениеПоказателя(Форма, ИмяПоказателя, Знач ЗначениеПоказателя) Экспорт
		...
		Иначе
			ИмяСтраницыПанели = уатроОпределитьСтраницуПанелиПоКодуПоказателя(Форма, ИмяПоказателя);
		КонецЕсли;
		
		ТаблицаВариантыЗаполнения = уатроПолучитьТаблицуВариантовЗаполнения(Форма, ИмяСтраницыПанели);
		Если Не ТаблицаВариантыЗаполнения = Неопределено Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				Если (НЕ уатроЯвляетсяЧислом(ЗначениеПоказателя)) И (ВариантЗаполнения = 2) Тогда
					ЗначениеПоказателя = 0;
				КонецЕсли;
				
				Если уатроЯвляетсяЧислом(ЗначениеПоказателя) Тогда
	...
	//
	Функция уатроОпределитьСтраницуПанелиПоКодуПоказателя(Форма, КодПоказателя) Экспорт
		Перем ИмяСтраницы;
	...
	//  Форма          - ссылка на форму отчета.
	//
	Процедура уатроПоказатьСтраницыОтчета(Форма) Экспорт
		...
		//	// Выводим в названии кнопки "Настройка страниц" доп. символ '*'.
		//	КнопкиНастройки.Текст = ИсходныйТекст + " (*)";
		//КонецЕсли;
	...
	//
	Функция уатОсновнаяФормаОрганизацияНеЗаполненаВывестиТекст() Экспорт
	...
	//
	Процедура уатроПриИзмененииВидаДокументаИКорректировки(Форма, РазделДокумента = Неопределено, ВидДок = Неопределено, НомКоррект = Неопределено) Экспорт
	...
	// 
	Процедура уатроДоступностьКнопокДобавитьУдалить(Форма, ТекущаяОбласть = Неопределено, ИмяТекущейСтраницыПанели = Неопределено) Экспорт
		...
		Если Не ТекущаяОбласть = Неопределено Тогда
			...
			Если КнопкаДобавить <> Неопределено И КнопкаУдалить <> Неопределено Тогда
				Если уатроРазделТекущейОбласти(Форма, ТекущаяОбласть ) = Неопределено Тогда
	...
	//
	Процедура уатроПриАктивизацииОбластиРегламентированногоОтчета(Форма, Элемент, ИмяТекущейСтраницыПанели = Неопределено) Экспорт
		...
		Попытка
			Если Найти(Элемент.ТекущаяОбласть.Имя, "НомСтр") > 0 Тогда
				Если ТипЗнч(Форма.мСтруктураВзаимныхПереходов) = Тип("Структура") Тогда
					...
					// Снимем флаг, присвоим Неопределено структуре взаимных переходов.
		...
		Если КолвоМногострочныхБлоков > 0 Тогда
			...
			уатроДоступностьКнопокДобавитьУдалить(Форма, ТекущаяОбласть, ИмяТекущейСтраницыПанели);
			
		КонецЕсли;
		
		Если уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "РазделЯвляетсяАвтозаполняемым") Тогда
			
			уатроВывестиЭлементыВариантаЗаполнения(Форма, Элемент.ТекущаяОбласть);
	...
	//
	Функция уатВыбраннаяОбластьРегламентированногоОтчетаНеВерна(Форма, Область, СтандартнаяОбработка) Экспорт
	...
	//
	Функция уатроЗаполнитьСведенияОбОрганизации(Форма, Знач СписокСведений, Дополнения = "", РазделДляВывода = "Титульный") Экспорт
		...
		ОргСведения = уатПолучитьСведенияОбОрганизации(Форма.Организация, Форма.ДатаПодписи, СписокСведений);
		Результат = "";
		ПБОЮЛ = уатЭтоПБОЮЛ(Форма.Организация);
		...
		Для Каждого ПолученныйРезультат Из ОргСведения Цикл
			...
			Если Параметр = "ИННРук" Тогда
				уатроПроставитьВКвадратыЗначения(Форма, "Титульный", "ИННРук", 12, ?(ПБОЮЛ, "            ", Результат));
			ИначеЕсли Параметр = "ИННБух" Тогда
				уатроПроставитьВКвадратыЗначения(Форма, "Титульный", "ИННГБ" , 12, ?(ПБОЮЛ, "            ", Результат));
			ИначеЕсли Параметр = "ИННУпПред" Тогда
				уатроПроставитьВКвадратыЗначения(Форма, "Титульный", "ИННУП" , 12, ?(ПБОЮЛ, "            ", Результат));
			ИначеЕсли Параметр = "ОКВЭД" Тогда
				...
				Если ОКВЭДВариант = 1 Тогда
					...
					уатроПроставитьВКвадратыЗначения(Форма, РазделДляВывода, "ОКВЭД" , 6, ОбработанныйРезультат);
			...
			ИначеЕсли Параметр = "ИННЮЛ" Тогда
				Если НЕ уатПустоеЗначение(Результат) Тогда
					Результат = уатДополнитьСтроку(Результат, 12, "0");
				Иначе
					Результат = уатДополнитьСтроку(Результат, 12, " ");
				КонецЕсли;
				
				уатроПроставитьНаВсехЛистах(Форма, Результат, "ИНН", "", ,,, Истина);
				
			ИначеЕсли Параметр = "КППЮЛ" Тогда
				Результат = уатДополнитьСтроку(Результат, 9, " ");
				уатроПроставитьНаВсехЛистах(Форма, Результат, "КПП", "", ,,, Истина);
			ИначеЕсли Параметр = "ОГРН" Тогда
				...
				Иначе
					...
					ОГРН = уатДополнитьСтроку(Результат, ?(ТипНП = 3, 15, 13), " ");
				КонецЕсли;
				
				уатроПроставитьВКвадратыЗначения(Форма, "Титульный", ОГРНКоордината, СтрДлина(ОГРН), ОГРН);
			ИначеЕсли Параметр = "КодНО" Тогда
				Результат = уатДополнитьСтроку(Результат, 4, " ");
				уатроПроставитьВКвадратыЗначения(Форма, "Титульный", "КодИМНС", 4, Результат);
	...
	//
	Функция уатроОбластьЯвляетсяЯчейкойСоставногоПоля(Форма, ИмяЯчейки, БыстраяПроверка = Истина) Экспорт
		
		СмысловаяЧасть = уатроПолучитьСмысловуюЧастьИмениЯчейки(ИмяЯчейки);
		...
		ИмяСтраницыПанели = уатроОпределитьСтраницуПанелиПоКодуПоказателя(Форма, ИмяЯчейки);
		...
		// Получаем текущий счетчик
		//ТекущийСчетчик = Сред(ИмяЯчейки,АдресПоследнегоРазделителя+1,СтрДлина(ИмяЯчейки)-АдресПоследнегоРазделителя);
		ТекущийСчетчик = уатроПолучитьСмысловуюЧастьИмениЯчейки(ИмяЯчейки,Ложь);
		...
		Если уатроЯвляетсяЧислом(ТекущийСчетчик) Тогда
	...
	//
	Процедура уатроСвойстваГрупповыхЗначений (Форма, ИмяЯчейки, Операция, ЦветФона = Неопределено, ИмяСтраницы = Неопределено) Экспорт
		...
		Пока МожноРаботать Цикл
			...
			Если Форма.ЭлементыФормы[ИмяПоляТаблДокумента].Области.Найти(Координата) <> Неопределено Тогда
				...
				Исключение
					МожноРаботать = Ложь; // далее нет ячеек 
					Прервать;
				КонецПопытки;
	...
	//
	Процедура уатроИзменитьТаблицуВариантовЗаполнения(Форма, ПолныйРучнойВвод = Ложь) Экспорт
		...
		Пока ПереборЗнач > 0 цикл
			...
			КолСтрВРазделе = уатроПолучитьКоличествоСтраницРаздела(Форма, ИмяСтраницы); // кол-во страниц в разделе
			...
			ТаблицаВариантыЗаполнения = уатроПолучитьТаблицуВариантовЗаполнения(Форма, ИмяСтраницы);
	...
	//
	Функция уатроПолучитьКоличествоСтраницРаздела(Форма, ИмяТекущейСтраницыПанели) Экспорт
		Перем ТаблицаСтраницРаздела;
	...
	//
	Процедура уатроОчиститьВариантыЗаполнения(Форма, Знач Раздел = Неопределено) Экспорт
		
		Перем МассивВариантовЗаполненияСтраниц;
		
		// Данная процедура используется только в новых декларациях, где установлены показатели Е как 14.2
		// Через процедуру УстановитьФормат
		...
		Для Каждого СтрокаУровня1 Из Форма.мСвойстваРазделовДекларации.Строки Цикл
			...
			Если Форма.мСтруктураВариантыЗаполнения.Свойство(НазваниеСтраницы, МассивВариантовЗаполненияСтраниц) Тогда
				...
				ТаблицаВариантыЗаполнения = уатроПолучитьТаблицуВариантовЗаполнения(Форма, НазваниеСтраницы, Истина, мОписаниеТиповЧислоЕ);
		...
		Если ИзмененияЕсть И Форма.ФлажокОтклАвтоРасчет Тогда
			уатроИзменитьТаблицуВариантовЗаполнения(Форма, НЕ Форма.ФлажокОтклАвтоРасчет);
		КонецЕсли;
	...
	//
	Функция уатроВернутьЗначениеПараметраСЛиста(Форма, СтраницаПанели, ОсновнаяЧасть) Экспорт
	...
	//  Нет.
	//
	Процедура уатВвестиКодОГРННаТитульномЛистеРегламентированногоОтчета(Форма, ОбластьИмя) Экспорт
		
		Если Найти(ОбластьИмя, "ОГРН") > 0 Тогда
			Если Найти(ОбластьИмя, "ОГРНИП") > 0 Тогда
				ПБОЮЛ = Истина;
				ОГРН = уатроВернутьЗначениеПараметраСЛиста(Форма, "Титульный", "ОГРНИП");
			Иначе
				ОГРН = уатроВернутьЗначениеПараметраСЛиста(Форма, "Титульный", "ОГРН");
				ПБОЮЛ = Ложь;
			КонецЕсли;
			ОГРН = СокрЛП(ОГРН);
			
			Если ВвестиСтроку(ОГРН, "Введите ОГРН " + ?(ПБОЮЛ, "предпринимателя", "организации"), ?(ПБОЮЛ, 15, 13)) Тогда
				Если (ОГРН = "") Или (уатТолькоЦифрыВСтроке(ОГРН)) И (СтрДлина(ОГРН) = ?(ПБОЮЛ, 15, 13)) Тогда
					уатроПроставитьВКвадратыЗначения(Форма, "Титульный", ?(ПБОЮЛ, "ОГРНИП", "ОГРН"), ?(ПБОЮЛ, 15, 13), ОГРН);
					Форма.Модифицированность = Истина;
				Иначе
	...
	//
	Функция уатВернутьЛогическийЗнакВСтроке(Знач Формула) Экспорт
	...
	//
	Функция уатИмпортДанныхРегламентированнойОтчетностиИзКонтейнераXML(ИмяФайла) Экспорт
		...
		Пока ЧтениеXML.Прочитать() Цикл
			Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				ИначеЕсли Узел = "СведенияОтчета" Тогда
					СведенияОтчетаРезультат = Новый Структура;
					Для каждого Элемент Из СведенияОтчета Цикл
						СведенияОтчетаРезультат.Вставить(Элемент.Ключ, ЧтениеXML.ПолучитьАтрибут(Элемент.Ключ));
					КонецЦикла;
				Иначе
					...
					Если НайденнаяСтрока <> Неопределено Тогда
						...
						НовСтр.СтруктураДанных = уатСобратьДанныеИзXMLУзла(ЧтениеXML);
					Иначе
						...
						НовСтрТаблицаДанных.СтруктураДанных = уатСобратьДанныеИзXMLУзла(ЧтениеXML);
			...
			ИначеЕсли (ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента) И (ЧтениеXML.Имя = "РегламентированныйОтчет") Тогда
				...
				Результат.Вставить("СведенияОтчета", уатПолучитьКопиюСтруктуры(СведенияОтчетаРезультат));
	...
	//
	Функция уатПолучитьКопиюСтруктуры(Знач СтруктураРезультата)
		Результат = Новый Структура;
	...
	//
	Функция уатСобратьДанныеИзXMLУзла(ЧтениеXML)
	...
	//
	Функция уатПолучитьДанныеРегламентированнойОтчетностиЭтогоОтчета(ДанныеКонтейнера, СтруктураИдентификацииДанных, УчитыватьПериодичность = Истина) Экспорт
		...
		Для каждого Отчет Из ДанныеКонтейнера Цикл
			...
			НовСтр.ДанныеОтчета = уатПолучитьКопиюСтруктуры(Отчет.Значение);
	...
	//
	Функция уатВернутьСтруктуруТочногоОтбораДанныхИзКонтейнераXML(Форма) Экспорт
		...
		ИначеЕсли Форма.мПериодичность = Перечисления.Периодичность.Квартал Тогда
			...
			// в УПП 1.1. нет перечисления Полугодие.
			//ИначеЕсли Форма.мПериодичность = Перечисления.Периодичность.Полугодие Тогда
	...
	//
	Функция уатСформироватьАдрес9зпт(КодРегионаКакЧисло, СтруктураДанных) Экспорт
	...
	Функция уатЗаменитьТекстРегионаНаКодРегиона(Знач АдресВФормате9зпт) Экспорт
		...
		Регион = Параметры.Получить(2).Значение;
		
		Если НЕ уатТолькоЦифрыВСтроке(Регион) Тогда
			Регион = Формат(уатКодРегионаПоНазванию(Регион), "ЧЦ=2; ЧН=; ЧВН=");
		КонецЕсли;
		
		Возврат Параметры.Получить(0).Значение + "," + Параметры.Получить(1).Значение + "," + Регион + "," + Параметры.Получить(3).Значение + "," +
		Параметры.Получить(4).Значение + "," + Параметры.Получить(5).Значение + "," + Параметры.Получить(6).Значение + "," + Параметры.Получить(7).Значение + "," +
		Параметры.Получить(8).Значение + "," + Параметры.Получить(9).Значение;
	...
	//  ВыводитьСообщение - в случае запуска процедуры из-под отчета, если ошибок не обнаружено, 
	//                то этот флаг позволяет задавать вопрос
	//                 и предложение о просмотре формы контрольных соотношений.
	//
	Процедура уатПроверитьКонтрольныеСоотношенияВОтчете(Форма, СохраненныйДок, ВызовИзЦУО, ВыводитьСообщение = Ложь) Экспорт
		...
		Если НЕ ВызовИзЦУО Тогда
			...
			ОбработкаКС = уатПолучитьОбработкуОшибокКонтрольныхСоотношений(Форма, СохраненныйДок);
		...
		Иначе
			...
			ОбработкаКС = уатПолучитьОбработкуОшибокКонтрольныхСоотношений(Форма, СохраненныйДок);
			Если ОбработкаКС.мТаблицаОшибокКС.Количество() > 0 Тогда
				Сообщить("В отчете " + уатПредставлениеДокументаРеглОтч(СохраненныйДОк.Ссылка) + Символы.ПС + 
	...
	//
	Функция уатПолучитьОбработкуОшибокКонтрольныхСоотношений(Форма, СохраненныйДок)
	...
	//
	Функция уатПолучитьСписокПраздниковРФ(КалендарныйГод) Экспорт
	...
	//
	Процедура уатУстановитьПримечаниеНаТитульныхЛистахГруппыАлкогольныхДеклараций(Форма) Экспорт
		// Сохраним текущий активный экземпляр листа.
		уатроСохранитьДанныеМногостраничногоРазделаВАктивнойСтранице(Форма, "Титульный", Ложь);
		...
		ИначеЕсли (Форма.мСтруктураМногостраничныхРазделов.Титульный[0].Данные.ТипОрг2 = "V")
			...
			// Если декларация открыта не организацией и не обособленным подразделением(или обособленным),
			//	то скроем флаг и очистим все примечания.
	...
	//
	Функция уатПолучитьСтруктуруТаблицыРО(ТаблицаИсточник) Экспорт
	...
	//
	Функция уатПолучитьКварталРО(ЗаданнаяДата) Экспорт
	...
	//
	Функция уатПолучитьТаблицуМногостраничногоРаздела() Экспорт
...
//  СтрокаВводаИзТаблицы - Строка. Если "" (пусто), тогда стандартный текст, если Неопределено, тогда не позволять,
//		иначе - текст.
...
//  СтруктураТребованийРучногоВвода - структура с ключами: СтрокаТолькоИзЧисел, КонтролируемаяДлинаСтроки, проверка
//		ручного ввода.
...
Функция уатВвестиЗначениеПоказателяРеглОтчета(Форма, Знач Раздел, Знач ИмяЯчейкиВывода, Знач ПервоначальноеЗначение, Знач РазмерностьЦ, Знач РазмерностьД, Знач ЭтоСумма, Знач ТекстВвода, Знач ТекстВыбора, ТаблицаВыбора, Знач СтрокаВводаИзТаблицы, СтандартнаяОбработка, Знач КолвоКвадратиков = Неопределено, КолонкаПоиска = "Код", СтруктураТребованийРучногоВвода = Неопределено, НольКакНоль = Ложь) Экспорт
		...
		Если ТипЗнч(ПервоначальноеЗначение) = Тип("Строка") Тогда
			...
			Пока Вводить Цикл
				...
				ИначеЕсли МеханизмВводаИНН Тогда
					...
					// Первое условие, когда в переменной РазмерностьД задано 0, это означает, что вводится ИНН может как 12-ти,
					//	так и 10-ти значный.
					// Применяется в декларации по НДС.
					// В случае, если РазмерностьД указано одно из значений 10 или 12, то требуется вводить однозначно или 10-ти,
					//	или 12-ти значный ИНН.
					Если (((уатИННСоответствуетТребованиямБезТипа(ПервоначальноеЗначение)) Или (ПустаяСтрока(ПервоначальноеЗначение))) И РазмерностьД = 0)
					Или (((РазмерностьД = 10) И (уатИННСоответствуетТребованиям(ПервоначальноеЗначение, Ложь))) Или ПустаяСтрока(ПервоначальноеЗначение))
					Или (((РазмерностьД = 12) И (уатИННСоответствуетТребованиям(ПервоначальноеЗначение, Истина))) Или ПустаяСтрока(ПервоначальноеЗначение)) Тогда
						Вводить = Ложь;
						...
						// Добавим симовлы 0, для заполнения ячеек с левой стороны.
							ПервоначальноеЗначение = уатДополнитьСтроку(ПервоначальноеЗначение, КолВоКвадратиков, СимволЗаполнения);
						КонецЕсли;
					Иначе
				...
				ИначеЕсли МеханизмВводаКПП Тогда
					ПроверкаВвода = Ложь;
					Если (уатКППСоответствуетТребованиям(ПервоначальноеЗначение)) Или (ПустаяСтрока(ПервоначальноеЗначение))Тогда
						Вводить = Ложь;
					Иначе
				...
				Если ПроверкаВвода Тогда
					Если (ПервоначальноеЗначение = "") Или (уатТолькоЦифрыВСтроке(ПервоначальноеЗначение)) И (СтрДлина(ПервоначальноеЗначение) = КритичнаяДлина) Тогда
						Вводить = Ложь;
					Иначе
				...
				Иначе
					...
					Если СтруктураТребованийРучногоВвода <> Неопределено Тогда
						...
						Если (СтруктураТребованийРучногоВвода.СтрокаТолькоИзЧисел) И (НЕ уатТолькоЦифрыВСтроке(ПервоначальноеЗначение)) Тогда
	...
	Иначе
		уатроПроставитьВКвадратыЗначения(Форма, Раздел, ИмяЯчейкиВывода, ?(ЭтоСумма, КолВоКвадратиков, РазмерностьЦ), Значение, ЭтоСумма, НольКакНоль);
	КонецЕсли;
...
КонецФункции // ВвестиЗначениеПоказателяРеглОтчета
	...
	//
	Процедура уатроУстановитьЦветФонаЯчейки(Форма, ИмяЯчейки = Неопределено, ВариантЗаполнения = Неопределено, ИмяТекущейСтраницыПанели = "") Экспорт
		Перем ЦветФона;
		Перем Комментарий;
		...
		Если Не ИмяЯчейки = Неопределено Тогда
			...
			ЗначениеВКвадратах = уатроОбластьЯвляетсяЯчейкойСоставногоПоля(Форма, ИмяЯчейки);
			
			Если ВариантЗаполнения = 4 Тогда
				
				Если ЗначениеВКвадратах Тогда
					// если ЗНАЧЕНИЕ КВАДРАТИЧНОЕ
					уатроСвойстваГрупповыхЗначений(Форма, ИмяЯчейки,"УстановитьЗащиту");
				Иначе
			...
			Иначе
				...
				Если ЗначениеВКвадратах Тогда
					// если ЗНАЧЕНИЕ КВАДРАТИЧНОЕ
					уатроСвойстваГрупповыхЗначений(Форма, ИмяЯчейки,"СнятьЗащиту");
				Иначе
			...
			Если ЗначениеВКвадратах Тогда
				уатроСвойстваГрупповыхЗначений(Форма, ИмяЯчейки,"ПроставитьЦветРаскраски", ЦветФона);
			Иначе
		...
		Иначе
			...
			Для Каждого Строка Из ТаблицаВариантыЗаполнения Цикл
				...
				КолвоСтрокВМногострочнойЧасти = уатроПоказательВходитВМногострочнуюЧасть(Форма, ИмяПоказателя);
				
				Если КолвоСтрокВМногострочнойЧасти > 0 Тогда
					Многострочность = Истина;
					БазоваяЧасть = ИмяПоказателя;
					ОписанияБлоков = уатроПолучитьСвойствоРаздела(Форма, ИмяТекущейСтраницыПанели, "МногострочностьВРазделе");
				...
				Для Инд = 1 по КолвоСтрокВМногострочнойЧасти Цикл
					...
					Если ВариантЗаполнения = 4 Тогда
						...
						Если ЗначениеВКвадратах Тогда
							// если ЗНАЧЕНИЕ КВАДРАТИЧНОЕ
							уатроСвойстваГрупповыхЗначений(Форма, ИмяПоказателя, "УстановитьЗащиту");
						Иначе
					...
					Иначе
						...
						Если ЗначениеВКвадратах Тогда
							// если ЗНАЧЕНИЕ КВАДРАТИЧНОЕ
							уатроСвойстваГрупповыхЗначений(Форма, ИмяПоказателя,"СнятьЗащиту");
						Иначе
					...
					Если ЗначениеВКвадратах Тогда
						уатроСвойстваГрупповыхЗначений(Форма, ИмяПоказателя,"ПроставитьЦветРаскраски", ЦветФона);
					Иначе
	...
	//
	Процедура уатроУстановитьФорматВывода(Форма) Экспорт
		...
		Если ЕдиницаИзмерения = Перечисления.уатПорядкиОкругленияОтчетности.Окр1 Тогда
			Форма.мДелитель = 1;
			РазмерностьПрописьюТекст = "в рублях";
			Форма.мСтрокаФормата = "ЧЦ = 17; ЧДЦ = " + ТочностьЕдиницыИзмерения + "; ЧРД=,; ЧН=-; ЧС = 0";
			
		ИначеЕсли ЕдиницаИзмерения = Перечисления.уатПорядкиОкругленияОтчетности.Окр1000 Тогда
		...
		ИначеЕсли ЕдиницаИзмерения = Перечисления.уатПорядкиОкругленияОтчетности.Окр1000000 Тогда
		...
		Для Каждого СтрПоказатели Из Форма.мТаблицаСоставПоказателей Цикл
			...
			Если (ТипДанныхПоказателя = "Е") И (НЕ ПустаяСтрока(КодПоказателяПоСтруктуре)) Тогда
				...
				Если НЕ ДанныеВСтруктуреПоРазделуОтражены Тогда
					...
					// Установим признак, что переменные отображены, для удаления отображения при проходе по следующей записи
					//	и показателя.
			...
			Если (ТипДанныхПоказателя = "Е") И (ПустаяСтрока(КодПоказателяПоСтруктуре)) Тогда
				...
				Если ПризнМногострочность <> "/" Тогда
					...
					ФорматируемаяОбласть.ТипЗначения = Форма.мОписаниеТиповЧислоЕ;
				...
				Иначе
					...
					ОписанияБлоков = уатроПолучитьСвойствоРаздела(Форма, ИмяПоляТаблДокумента, "МногострочностьВРазделе");
	...
	// Получает и проставляет ИНН и КПП на всех страницах, включая многостраничные разделы, а также листы,
	//	где ИНН и КПП указывается несколько раз.
	//
	// Параметры:
	//  Форма          - ссылка на форму декларации
	//  НачальноеЗаполнение - при первоначальном открытии декларации, без обновления страниц в структурах.
	//					По умолчанию Истина. В случае, если Ложь, тогда ИНН и КПП будут проставлены
	//					и в структуре многостраничных разделов всех экземпляров.
	//
	Процедура уатПроставитьИННКППНаВсехЛистахРегОтчета(Форма, НачальноеЗаполнение = Истина) Экспорт
		
		Результат = уатПолучитьСведенияОбОрганизации(Форма.Организация, Форма.ДатаПодписи, "ИННЮЛ, КППЮЛ");
		РезультатИНН = Результат.ИННЮЛ;
		РезультатКПП = Результат.КППЮЛ;
		
		Если НЕ уатПустоеЗначение(РезультатИНН) Тогда
			РезультатИНН = уатДополнитьСтроку(РезультатИНН, 12, "0");
		Иначе
			РезультатИНН = уатДополнитьСтроку(РезультатИНН, 12, " ");
		КонецЕсли;
		
		РезультатКПП = уатДополнитьСтроку(РезультатКПП, 9, " ");
		...
		Если НачальноеЗаполнение Тогда
			Для каждого Страница Из Форма.мСвойстваРазделовДекларации.Строки Цикл
				...
				КолвоСтраниц = уатроКолвоЛистоВРазделе(Форма, ИмяСтраницы);
				Для Инд = 1 По КолвоСтраниц Цикл
					Стр = Строка(Инд) + "_";
					уатроПроставитьВКвадратыЗначения(Форма, ИмяСтраницы, "ИНН" + Стр, 12, РезультатИНН);
					уатроПроставитьВКвадратыЗначения(Форма, ИмяСтраницы, "КПП" + Стр, 9, РезультатКПП);
				КонецЦикла;
			КонецЦикла;
		Иначе
			уатроПроставитьНаВсехЛистах(Форма, РезультатИНН, "ИНН", "", ,,, Истина);
			уатроПроставитьНаВсехЛистах(Форма, РезультатКПП, "КПП", "", ,,, Истина);
		КонецЕсли;
	...
	// Параметры:
	//  Форма          - ссылка на форму отчета
	//  ИмяГруппыБлока - имя группы, в том случае, если необходимо вставить строки в процедурах 
	//                автозаполнения, без фокусировок.
	//  СпециальноеДобавление - режим добавления строк по нестандартному алгоритму, когда в 
	//                правой части располагаются данные.
	//  ИмяКолонкиНомера - имя колонки, в которой необходимо проставлять номер при вставки строки.
	//
	Процедура уатроВставитьДополнительнуюСтроку(Форма, ИмяГруппыБлока = Неопределено, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено) Экспорт
		Перем СтруктураГруппы;
	...
	//
	Процедура уатроУдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено) Экспорт
		...
		ИмяГруппы      = уатроРазделТекущейОбласти(Форма, ТекущаяОбласть);
		...
		Если СтруктураГруппы.Количество() > ИсходноеКоличествоСтрокГруппы Тогда
			...
			МногострочностьВРазделе = уатроПолучитьСвойствоРаздела(Форма, ТекущаяСтраница, "МногострочностьВРазделе");
	...
	//
	Процедура уатСохранитьЗначениеВМногостраничномБлоке(Форма, Знач ИмяГруппы, Знач НомерСтрокиНаФорме, Знач ИмяКолонки, Знач ЗначениеПоказателя) Экспорт
		Перем СтруктураГруппы;
		Форма.мСтруктураМногострочныхРазделов.Свойство(ИмяГруппы, СтруктураГруппы);
	...
	//
	Процедура уатОчиститьПоказателиОтчетаЗаполняемыеИзИБ(Форма, НазванияМногостраничныхРазделовДляУдаления = "") Экспорт
		
		Если НЕ ПустаяСтрока(НазванияМногостраничныхРазделовДляУдаления) Тогда
			НазванияМногостраничныхРазделов = Новый Структура(НазванияМногостраничныхРазделовДляУдаления);
		Иначе
			НазванияМногостраничныхРазделов = Новый Структура;
		КонецЕсли;
		...
		Для каждого СтрокаУровня1 из Форма.мСвойстваРазделовДекларации.Строки Цикл
			...
			Если НазванияМногостраничныхРазделов.Свойство(НазваниеСтраницы) Тогда
				...
				КонецЦикла;
				// Отобразим статик.
				уатроПоказатьСтатикСтраницРаздела(Форма, НазваниеСтраницы);
				// Варианты заполнения установим в один массив.
				уатроОчиститьВариантыЗаполнения(Форма, НазваниеСтраницы);
				Продолжить;
				
			КонецЕсли;
			
			ВариантыАвтозаполнения = уатроПолучитьТаблицуВариантовЗаполнения(Форма, НазваниеСтраницы);
			...
			Если ВариантыАвтозаполнения <> Неопределено Тогда
				...
				Для каждого Пок Из ВариантыАвтозаполнения Цикл
					...
					Если (Пок.ВариантЗаполнения = 3) Или (Пок.ВариантЗаполнения = 4) Тогда
						...
						Колво = уатроПоказательВходитВМногострочнуюЧасть(Форма, КодПоказателя);
						Если Колво = 0 Тогда
	...
	//
	Процедура уатроВывестиРазделВТабличныйДокумент(Форма, ИмяГруппы, СтруктураГруппы) Экспорт
		Перем ИсходноеКоличествоСтрокГруппы;
		...
		Если ТекущееКоличествоСтрокГруппы > ИсходноеКоличествоСтрокГруппы Тогда
			...
			НастройкиМнгЧ = уатроПолучитьСвойствоРаздела(Форма, ТекТабличноеПолеИмя, "МногострочностьВРазделе").Скопировать();
			...
			// При превышении реального количества строк над исходным количеством (по шаблону),
			// автоматически добавляем необходимое количество строк.
			...
			Для НомНовойСтроки = 1 По ТекущееКоличествоСтрокГруппы Цикл
				...
				уатЗаполнитьСтрокуДаннымиИзТаблицы(СтруктураГруппы, Строка1, СтруктураГруппы[НомНовойСтроки - 1], НомСтр);
	...
	// Процедура заполняет строку данными таблицы
	//
	// Параметры:
	//	ТаблицаРаздела - таблица - источник
	//	Строка1 - строка для заполнения
	//	СтрокаТаблицы - строка таблицы- источника
	//	НомерНаФорме - номер на форме
	//
	Процедура уатЗаполнитьСтрокуДаннымиИзТаблицы(ТаблицаРаздела, Строка1, СтрокаТаблицы, НомерНаФорме) Экспорт
		// Заполняем строку.
	...
	//
	Процедура уатУдалитьДопСтрокиМногострочнойЧастиРегОтчета(Форма, ИдГруппы, ОбновлятьБлок = Истина) Экспорт
		...
		Если ТекущееКоличествоСтрок > ИсходноеКоличествоСтрокГруппы Тогда
			...
			НастройкиМнгЧ = уатроПолучитьСвойствоРаздела(Форма, ТекТабличноеПолеИмя, "МногострочностьВРазделе").Найти(ИдГруппы, "ИдГруппы");
	...
	//
	Процедура уатроОчиститьТабличноеПоле(Форма, ВыбТабличноеПоле) Экспорт
		
		ВыбТабличноеПолеИмя = ВыбТабличноеПоле.Имя;
		// Проверяем многострочные блоки в разделе.
		НастройкиМнгЧ = уатроПолучитьСвойствоРаздела(Форма, ВыбТабличноеПолеИмя, "МногострочностьВРазделе");
		...
		Если ЕстьМногострочныеБлоки Тогда
			Для каждого Блок Из НастройкиМнгЧ Цикл
				...
				уатУдалитьДопСтрокиМногострочнойЧастиРегОтчета(Форма, МногострочныйБлок, Ложь);
			КонецЦикла;
		КонецЕсли;
		...
		ПредставлениеДанных = уатроПолучитьСвойствоРаздела(Форма, ВыбТабличноеПолеИмя, "ПредставлениеДанных");
		Если ПредставлениеДанных Тогда
	...
	//
	Процедура уатроОбновитьСтруктуруМногострочногоРаздела(Форма, ИмяГруппы) Экспорт
		Перем СтруктураГруппы;
	...
	//
	Процедура уатроОбновитьСтруктуруМногострочныхРазделов(Форма) Экспорт
		// Заполняет таблицу значений многострочного раздела
...
//
Функция уатПолучитьВариантЗаполненияПоказателяРегОтчета(МакетСоставаПоказателей, Ном) Экспорт
	...
	Если ПозицияРазделителя = 0 Тогда
		Если (стрВариантЗаполнения = "2" ИЛИ стрВариантЗаполнения = "3") Тогда
			Возврат "";
		Иначе
...
//
Процедура уатСформироватьСоставКнопкиОчиститьДляРегОтчетов(Форма) Экспорт
	...
	Если уатСвойствоОпределено(Форма, "мБезОткрытияФормы") Тогда
	...
	МногостраничностьВРазделе = уатроПолучитьСвойствоРаздела(Форма, ИмяСтраницы, "МногостраничностьВРазделе", Ложь);
	ПредставлениеРаздела = уатПолучитьПредставлениеРазделаРегОтчета(Форма, ИмяСтраницы);
	СведенияОФЛ = (Найти(НРег(ПредставлениеРаздела), "физ") > 0) И (Найти(НРег(ПредставлениеРаздела), "лиц") > 0);

	ЧтоОчищать = "Отчет";
	Если уатСвойствоОпределено(Форма, "мЧтоОчищать") Тогда
	...
	Если ИмяСтраницы <> "Титульный" Тогда
		...
		//								"Очистить сведения о физическом лице" + ДополнениеКТексту,
		//								Новый Действие("КоманднаяПанельФормыВыбратьВариантОчисткиОтчета"));
		//Иначе
		ЭлементМеню.Кнопки.Добавить("ОчиститьТекущийРаздел" + ИмяСтраницы, ТипКнопкиКоманднойПанели.Действие,
...
//
Функция уатПолучитьПредставлениеРазделаРегОтчета(Форма, ИмяСтраницы)
...
//
Процедура уатМеханизмыОчисткиРегламентированныхОтчетов(Форма, Кнопка) Экспорт
	...
	ЗапускатьОтдельнуюОбработкуПослеОчисткиЛиста = уатроПолучитьСвойствоРаздела(Форма, ИмяСтраницы, "ЗапускатьОтдельнуюОбработкуПослеОчисткиЛиста", Ложь);
	Многостраничность = уатроПолучитьСвойствоРаздела(Форма, ИмяСтраницы, "МногостраничностьВРазделе", Ложь);
	УдалятьсяВсеДопЭкземпляры = Ложь;
	ПредставлениеРазделаОтчета = НРег(уатПолучитьПредставлениеРазделаРегОтчета(Форма, ИмяСтраницы));
	...
	Если Найти(КнопкаИмя, "ОчиститьТекущийРаздел") > 0 Тогда
		...
		Если Ответ = КодВозвратаДиалога.Да Тогда
			...
			Если (Многостраничность) И (КолВоСтраницВРазделе > 1) Тогда
				уатОчиститьПоказателиОтчетаЗаполняемыеИзИБ(Форма, ИмяСтраницы);
	...
	Если ОчисткаПроизошла Тогда
		...
		// Например, в этой процедуре может быть перерасчет каких-то листов,
		...
		Если ЗапускатьОтдельнуюОбработкуПослеОчисткиЛиста Тогда
			// Параметр УдалятьсяВсеДопЭкземпляры означает: если Истина, то удаляются все экземпляры
			//	доп. страниц раздела, иначе всегда Ложь.
		...
		Если уатроПолучитьСвойствоРаздела(Форма, ИмяСтраницы, "РазделЯвляетсяАвтозаполняемым") Тогда
			Форма.УстановитьЦветФонаЯчейки(,, ИмяСтраницы);
		КонецЕсли;
...
// Процедура сворачивает панель дл ввода данных на форме регламентированного отчета
//Форма 2007г. кв.1
//
//Параметры:
//	Форма - Форма регл. отчета
//
Процедура уатПриНажатииНаКнопкуПереходаВРежимВводаДанных(Форма) Экспорт
...
// Выводит отчет на печать (машиночитаемая форма)
//Возможно только для конфигурации в составе "Бухгалтерия предприятия"
//
//Параметры:
//	Форма - Форма регл. отчета,
//	ИмяКнопки - имя кнопки печати в отчете
//
Функция уатВывестиМашиночитаемуюФорму(Форма, ИмяКнопки) Экспорт
	
	Если НЕ уатРаботаСМетаданными.ЕстьСправочник("РегистрацияВИФНС") тогда
		Сообщить("Данная функциональность доступна только при объединении с конфигурацией ""Управление производственным предприятием""", СтатусСообщения.Внимание);
		Возврат Ложь;
	КонецЕсли;
	
	Возврат РегламентированнаяОтчетность.ВывестиМашиночитаемуюФорму(Форма, ИмяКнопки);
...
//
Процедура уатроПроставитьСведенияОПредставителеОрганизации(Форма, ФлагТолькоФЛ, СхемаПолученияКодИФНС = "0") Экспорт
	...
	ФИОУпПред = уатПолучитьСведенияОбОрганизации(Форма.Организация, Форма.ДатаПодписи, "ФИОУпПред").ФИОУпПред;
	...
	Если СхемаПолученияКодИФНС = "0" Тогда
		КодИФНС = уатроВернутьЗначениеПараметраСЛиста(Форма, "Титульный", "КодИМНС");
	КонецЕсли;
	...
	СтруктураПредставителя = РегламентированнаяОтчетность.ПолучитьСведенияОПредставителеОрганизации(Форма.Организация, КодИФНС, Форма.ДатаПодписи);
...
// Процедура формирует набор кнопок печати отчета
//
//Параметры:
//	Форма - форма регл. отчета,
//	ДействиеВстроеннаяПечать - ссылка на кнопку для встроенной печати,
//	ДействиеПечатьМЧБ - ссылка на кнопку для ч/б печати,
//	ПринудительнаяПрорисовкаКнопокВстроеннойПечати - принудительная прорисовка кнопок
//	
Процедура уатПрорисоватьПодменюПечать(Форма, ДействиеВстроеннаяПечать = Неопределено, ДействиеПечатьМЧБ = Неопределено, ПринудительнаяПрорисовкаКнопокВстроеннойПечати = Ложь) Экспорт
	...
	Если ПринудительнаяПрорисовкаКнопокВстроеннойПечати ИЛИ ДействиеВстроеннаяПечать <> Неопределено Тогда
		...
		Если ПринудительнаяПрорисовкаКнопокВстроеннойПечати ИЛИ (уатСвойствоОпределено(Форма, "мПечатьБезШтрихкодаРазрешена") И Форма.мПечатьБезШтрихкодаРазрешена = Истина) Тогда
...
// Функция проверяет доступность печати на принтер
//
//Возвращаемое значение:
// Флаг доступности
//
Функция уатПринтерДоступен() Экспорт
...
//
Процедура уатСтандартныеОперацииПоУстановкамМасок(Форма) Экспорт
	...
	Для Каждого СтрокаУровня1 Из Форма.мСвойстваРазделовДекларации.Строки Цикл
		...
		КоличествоЛистов = уатроКолвоЛистоВРазделе(Форма, ИмяСтраницы);
...
КонецПроцедуры // СтандартныеОперацииПоУстановкамМасок
...
//  Нет.
//
Процедура уатУстановитьКодОКВЭДНаТитульномЛисте(Форма, СтандартнаяОбработка) Экспорт
	мТитульный = Форма.ЭлементыФормы.ПолеТабличногоДокументаТитульный;
	...
	СтандартнаяОбработка = Ложь;
	ВводЗначений = уатроПолучитьОбщуюФорму("ФормаВводаОКВЭД", Форма);
	СтруктураПараметров = Новый Структура;
	...
	Если ВводЗначений.ОткрытьМодально() Тогда
		...
		уатроПроставитьВКвадратыЗначения(Форма, "Титульный", "ОКВЭД", 6, РезультатВвода2);
...
// Функция проверяет возможность автосохранения отчета
//
//Возвращаемое значение:
// Флаг возможности автосохранения
//
Функция уатАвтосохранениеВозможно(Форма) Экспорт
	Возврат Форма.мЗаписьЗапрещена <> Истина И Форма.мСохраненныйДок <> Неопределено И Форма.мСохраненныйДок <> Новый("ДокументОбъект.уатРегламентированныйОтчет") И Форма.Модифицированность;
КонецФункции

// Функция проверяет наличия свойства объекта
//
//Параметры:
//	Объект - объект, в свойствах которого осуществляется поиск,
//	ИмяСвойства - имя искомого свойства
//
//Возвращаемое значение:
// Флаг наличия свойства у объекта
//
Функция уатСвойствоОпределено(Объект, ИмяСвойства) Экспорт
...
Функция уатПродолжитьПриНесоответствииВерсийФорм(Знач ВерсияФормы, Знач мВерсияФормы) Экспорт
...
Функция уатОбъектОтчетСУчетомКэша(РеглОтч)
	Перем ОтчетВКэше;
...
Функция уатПризнакИспользованияВнешнегоОтчетаСУчетомКэша(РеглОтч)
	Перем ОтчетВКэше;
...
// Параметры:
//		Форма - объект Форма.
Процедура уатДобавитьНадписьВнешнийОтчет(Форма) Экспорт
...
Функция уатРеглОтчеты(Наим, СоздаватьИНеЗапоминать = Ложь) Экспорт
	Перем ВременныйФайл;
	...
	Если НайденныйЭлемент = Справочники.уатРегламентированныеОтчеты.ПустаяСсылка() Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если уатПризнакИспользованияВнешнегоОтчетаСУчетомКэша(НайденныйЭлемент) <> Истина ИЛИ НайденныйЭлемент.ВнешнийОтчетХранилище = Неопределено Тогда // используется внутренний отчет
	...
	Иначе // используется внешний отчет
		...
		Если НЕ СтруктураРегламентированныхОтчетов.Свойство(Наим, ВременныйФайл) ИЛИ ВременныйФайл = Неопределено Тогда
			...
			Попытка
				...
				уатОбъектОтчетСУчетомКэша(НайденныйЭлемент).Записать(ВременныйФайл);
...
Функция уатФормаРеглОтчета(Наим, ВыбраннаяФорма = Неопределено, Владелец = Неопределено, КлючУникальности = Неопределено) Экспорт
	Перем ВременныйФайл;
	...
	Иначе // используется внешний отчет
		...
		Если НЕ СтруктураРегламентированныхОтчетов.Свойство(Наим, ВременныйФайл) ИЛИ ВременныйФайл = Неопределено Тогда
			...
			//	ВнешняяОбработка = ВнешниеОбработки.Создать(ВременныйФайл);
			//	СтруктураРегламентированныхОтчетов.Вставить(Наим, ВременныйФайл);
			//	ПараметрыСеанса.уатРегламентированныеОтчеты = Новый ХранилищеЗначения(СтруктураРегламентированныхОтчетов);
			//	Возврат ВнешняяОбработка;
			//Исключение
			//	Сообщить("Не удалось запустить внешний отчет " + СокрП(Наим) + ".", СтатусСообщения.ОченьВажное);
			//	Возврат Неопределено;
		...
		Иначе
			...
			Попытка
				...
				Исключение
				КонецПопытки;
				уатДобавитьНадписьВнешнийОтчет(ВнешняяОбработка);
				Возврат ВнешняяОбработка;
			Исключение
				Возврат Неопределено;
			Конецпопытки;
...
Функция уатЭтоВнешнийОтчет(Наим) Экспорт
	
	НайденныйЭлемент = Справочники.уатРегламентированныеОтчеты.НайтиПоРеквизиту("ИсточникОтчета", Наим);
	Если НайденныйЭлемент = Справочники.уатРегламентированныеОтчеты.ПустаяСсылка() Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если уатПризнакИспользованияВнешнегоОтчетаСУчетомКэша(НайденныйЭлемент) <> Истина ИЛИ НайденныйЭлемент.ВнешнийОтчетХранилище = Неопределено Тогда // внутренний отчет
		Возврат Ложь;
	Иначе // внешний отчет
		Возврат Истина;
	КонецЕсли;
...
// 				 Ложь - в противном случае.
Функция уатОтчетКэшировался(РеглОтч) Экспорт
	Перем ОтчетВКэше;
...
Функция уатЗаблокироватьДокумент(мСохраненныйДок) Экспорт
...
Процедура уатПередОткрытиемОсновнойФормыРегламентированногоОтчета(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.ВладелецФормы <> Неопределено Тогда
		Если Не Форма.ВладелецФормы.ЭтоНовый() Тогда
			...
			УжеОткрытаяФорма = уатФормаРеглОтчета(Форма.ЭтотОбъект.Метаданные().Имя, СокрЛП(ВыбраннаяФорма), , Форма.мСохраненныйДок.Ссылка);
			...
			Если НЕ уатЗаблокироватьДокумент(Форма.мСохраненныйДок) Тогда
				Возврат;
			КонецЕсли;
			...
			Попытка
				...
				Если НЕ уатПродолжитьПриНесоответствииВерсийФорм(ВерсияФормыДанных, ВыбФормаОтчета.мВерсияФормы) Тогда
					Возврат;
				КонецЕсли;
			Исключение
			Конецпопытки;
			...
			уатДобавитьНадписьВнешнийОтчет(ВыбФормаОтчета);
...
//
Процедура уатОперацииПриВосстановленииРегламентированногоОтчета(Форма) Экспорт
...
//
// Параметры:
//	Форма - форма с которой следует скрыть картинку-индикатор.
//
Процедура уатСкрытьКартинкуСохранение(Форма) Экспорт
...
Процедура уатПослеЗаписиРегламентированногоОтчета(Форма) Экспорт
	
	уатЗаблокироватьДокумент(Форма.мСохраненныйДок);
	уатСкрытьКартинкуСохранение(Форма);
...
//			Ложь - в противном случае.
//
Функция уатСуществуетДокументСАналогичнымиРеквизитами(Форма, КодИФНС)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("мСохраненныйДок", ?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(Форма.мСохраненныйДок), Неопределено, Форма.мСохраненныйДок.Ссылка));
	...
	|ИЗ
	|	Документ.уатРегламентированныйОтчет КАК РегламентированныйОтчет
	|ГДЕ
...
//			Ложь - в противном случае.
//
Функция уатБылиИзмененыКлючевыеРеквизитыОтчета(Форма, КодИФНС)
...
Функция уатПриЗаписиРегламентированногоОтчета(Форма, КодИФНС = "не применимо", Автосохранение = Ложь, Вариант = Неопределено) Экспорт
	...
	Если Форма.мЗаписьЗапрещена = Истина Тогда
		...
		Возврат Ложь;
	КонецЕсли;
	Если уатБылиИзмененыКлючевыеРеквизитыОтчета(Форма, КодИФНС) И уатСуществуетДокументСАналогичнымиРеквизитами(Форма, КодИФНС) Тогда
	...
	Если Вариант <> Неопределено И ((Форма.мСохраненныйДок = Неопределено) ИЛИ (Форма.мСкопированаФорма <> Неопределено) ИЛИ (Вариант <> Форма.мВариант)) Тогда
		...
		Если Форма.мСохраненныйДок = Неопределено ИЛИ Форма.мСкопированаФорма <> Неопределено Тогда
			...
			// создаем новый документ вида РегламентированныйОтчет
			Форма.мСохраненныйДок  = Документы.уатРегламентированныйОтчет.СоздатьДокумент();
			уатЗаблокироватьДокумент(Форма.мСохраненныйДок);
		...
		ИначеЕсли Вариант <> Форма.мВариант Тогда
			...
			Ответ = уатроПолучитьОбщуюФорму("ВопросПриИзмененииВидаДокументаНомераКорректировки").ОткрытьМодально();
			Если Ответ = КодВозвратаДиалога.Да Тогда
...
//
Процедура уатПриЗакрытииРегламентированногоОтчета(Форма) Экспорт
...
//
Процедура уатПередЗакрытиемРегламентированногоОтчета(Форма, Отказ, СтандартнаяОбработка) Экспорт
...
// Функция формирует представление периода на основании года и месяца
//
// Параметры:
...
// 
Функция уатПредставлениеПериодаДокументаВыгрузкаРегламентированныхОтчетов(Док) Экспорт
...
//		ИмяЯчейки - имя области на табличном поле, соответствующей показателю, с которым связана ошибка;
//		ФИО - ФИО физлица (для случая, когда ошибка обнаружена при проверке раздела ФизЛица).
Процедура уатроСообщитьВТаблицуСообщений(Форма, Описание, Раздел, ИмяЯчейки, ФИО = "") Экспорт
...
// Функция возвращает номер страницы раздела "ФизЛица", соответствующей переданному 
// в качестве параметра строке - ФИО физлица.
//
// Параметры:
...
Функция уатроНомерСтраницыПоФИОФизЛица(Форма, ФИО) Экспорт
...
Процедура уатроАктивизироватьЯчейку(Форма, Ячейка) Экспорт
	...
	Если Ячейка.Раздел = "ФизЛица" Тогда
		Попытка
			НС = уатроНомерСтраницыПоФИОФизЛица(Форма, Ячейка.Страница);
...
//  ИмяСлагаемого - пользовательское наименование составляющей суммы показателя 
//  Сумма         - число;
//  Счет          - ссылка на план счетов. Счет
//  КорСчет       - 
//  Сдвиг         - сдвиг от основного периода в годах с минусом. Т.е. сдвиг 2 от периода 2004 год 
//                означает 2002 год
//  ПраваяГраница - правая граница периода. Если Сдвиг задает сдвиг от основного периода, то ПраваяГраница 
//                не учитывается.
...
//
// Функция уат не имеет параметров.
// Возвращаемое значение: Число - интервал автосохранения регламентированных отчетов.
Функция уатЗначениеИнтервалаАвтосохраненияРегламентированнойОтчетности() Экспорт
...
//
Процедура уатВключитьОбработчикАвтосохранения(Форма) Экспорт
	
	Форма.мИнтервалАвтосохранения = уатЗначениеИнтервалаАвтосохраненияРегламентированнойОтчетности();
...
//
Процедура уатОтключитьОбработчикАвтосохранения(Форма) Экспорт
...
//
Функция уатПравоДоступаКРегламентированномуОтчету(ИдентификаторОтчета) Экспорт
	
	Если уатЭтоВнешнийОтчет(ИдентификаторОтчета) <> Истина Тогда // это внутренний отчет
...
// Функция формирует файл с электронным представлением регламентированного отчета.
//
// Параметры:
...
//
Функция уатСформироватьТекстВыгрузки(Форма, ВремФайл) Экспорт
...
//
Функция уатНаложитьДвухмерныйШтрихКод(Форма)
	...
	Если НЕ уатСформироватьТекстВыгрузки(Форма, ВремФайл) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	ЭтоФормаБаланса = уатЭтоФормаБаланса(Форма);
	...
	Инд = 1;
	КоличествоБлоковНаЛисте = ?(ЭтоФормаБаланса, 6, 3);
	КоличествоБлоков = 0;
...
//
Функция уатПечататьРаздел(Форма, СтрокаРаздела, ЧтоИскать = Неопределено) Экспорт
	Перем КлючФизЛица;
	...
	Исключение
		Возврат 0;
	КонецПопытки;
	// если не выбран обязательный раздел, то сообщаем об этом пользователю,
	//	добавляем раздел в список печатаемых и выходим
...
//
Функция уатБиблиотекаФормированияДвухмерногоШтрихКодаУстановлена() Экспорт
...
//
Функция уатроПолучитьОбщуюФорму(ИмяФормы, Владелец = Неопределено, КлючУникальности = Неопределено) Экспорт
	...
	Если Метаданные.Обработки.уатОбщиеОбъектыРегламентированнойОтчетности.Формы.Найти(ИмяФормы) <> Неопределено Тогда
		Возврат Обработки.уатОбщиеОбъектыРегламентированнойОтчетности.ПолучитьФорму(ИмяФормы, Владелец, КлючУникальности);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
//
Функция уатЗначениеСодержитЗначение(ГдеИскать, ЧтоИскать)
...
//
Процедура уатроПоискВРегламентированномОтчете(Форма) Экспорт
	
	ФормаПоиска = уатроПолучитьОбщуюФорму("НастройкиПоиска", Форма);
	...
	ФормаРезультатыПоиска = уатроПолучитьОбщуюФорму("РезультатыПоиска", Форма);
...
// Табл - таблица значений, описывающая показатели листа отчета
// Корень - строка - "корень", значение для которого требуется получить 
Функция уатПоискВычислитьЗначениеПоКорню(Табл, Корень)
...
//Функция уатсобирает значения из отдельных "ячеек", например, ИНН1 из ИНН1_1 + ИНН1_2 + ...
Функция уатПолучитьХарактеристикиЗначенийТабличногоПоля(Данные)
	...
	Для Каждого Стр Из ТП Цикл
		Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(Стр.Корень) Тогда
			Стр.Корень = Стр.Имя;
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Стр Из ТП2 Цикл
		Если Стр.Кол > 1 И Стр.Корень <> "П" Тогда
			...
			КонецЦикла;
			Данные.Вставить(НовоеИмя, уатПоискВычислитьЗначениеПоКорню(ТП, Стр.Корень));
		КонецЕсли;
	КонецЦикла;
...
//
Процедура уатроНайтиЗначение(Форма, СтруктураПоиска, НеСворачивать = Ложь) Экспорт
	...
	Если СтруктураПоиска.ЧтоИскать.ИскатьВТексте Тогда
		...
		Для Стр = 1 По ТаблДок.ВысотаТаблицы Цикл
			Для Кол = 1 По ТаблДок.ШиринаТаблицы Цикл
				...
				Если уатЗначениеСодержитЗначение(Обл.Текст, СтруктураПоиска.ЧтоИскать) Тогда
	...
	Иначе
		...
		Если СтрДлина(СтруктураПоиска.ЧтоИскать.ИскомоеЗначение) > 1 И НЕ НЕСворачивать Тогда
			ЗначащиеЯчейки = уатПолучитьХарактеристикиЗначенийТабличногоПоля(ЗначащиеЯчейки);
		КонецЕсли;
		...
		Для Каждого Обл Из ЗначащиеЯчейки Цикл
			Если уатЗначениеСодержитЗначение(Обл.Значение, СтруктураПоиска.ЧтоИскать) Тогда
...
//
Функция уатИнициализацияОтчета(Док) Экспорт
	
	Отчет = уатРеглОтчеты(Док.ИсточникОтчета).ПолучитьФорму(, , Новый УникальныйИдентификатор);
	...
	Если Отчет.ЭтотОбъект.Метаданные().Формы.Найти(СокрП(ВыбраннаяФорма)) <> Неопределено Тогда
		ВыбФормаОтчета = Отчет.ПолучитьФорму(СокрП(ВыбраннаяФорма), , Новый УникальныйИдентификатор);
	Иначе
		// Если не удалось найти форму с таким названием (могла быть переименована),
		// то по умолчанию выдаем текущую (действующую) форму
		ВыбраннаяФорма = Отчет.мТаблицаФормОтчета[0].ФормаОтчета;
		ВыбФормаОтчета = Отчет.ПолучитьФорму(ВыбраннаяФорма, , Новый УникальныйИдентификатор);
	КонецЕсли;
...
//
Функция уатНапечататьОтчет(Док, ВидПечати = "ПоказатьБланк", ПечататьДвухмерныйШтрихКод = Неопределено, ПроверятьСоотношенияПриПечатиИВыгрузки = Неопределено) Экспорт
	
	Попытка
		Отчет = уатИнициализацияОтчета(Док);
		Если Отчет = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли;
		Отчет.Инициализация(Истина);
		Если ПечататьДвухмерныйШтрихКод <> Неопределено Тогда
			Попытка
...
//	Булево - Истина, если отчет удалось распечатать; Ложь - в противном случае.
//
Функция уатНапечататьОтчеты(Док, ВидПечати = "ПоказатьБланк", ПечататьДвухмерныйШтрихКод = Неопределено, Форма = Неопределено, ПроверятьСоотношенияПриПечатиИВыгрузки = Неопределено) Экспорт
	...
	Для Каждого Стр Из Док Цикл	
		...
		Попытка
			Отчет = уатИнициализацияОтчета(Стр.Значение);
			Если Отчет = Неопределено Тогда
				Возврат Ложь;
			КонецЕсли;
			Отчет.Инициализация(Истина);
			...
			Отчет = Неопределено;
		Исключение
			Сообщить(уатПредставлениеДокументаРеглОтч(Стр.Значение) + ". Не удалось напечатать отчет! Вероятно, отчетом не предусмотрена возможность печати без открытия формы.", СтатусСообщения.Важное);
		КонецПопытки;
	КонецЦикла;
...
//	Булево - Истина, если отчет удалось выгрузить; Ложь - в противном случае.
//
Функция уатВыгрузитьОтчеты(Доки) Экспорт
...
//	Булево - Истина, если отчет удалось выгрузить; Ложь - в противном случае.
//
Функция уатВыгрузитьОтчет(Док) Экспорт
...
//
Функция уатПредставлениеВидаДокумента(Вид) Экспорт
...
//
Функция уатПолучитьТаблицуОтчетовДействующихВВыбранныйПериод(Форма) Экспорт
	
	// Объявим таблицу результата.
	РезультирующаяТаблица = Форма.мТаблицаФормОтчета.Скопировать();
	РезультирующаяТаблица.Очистить();
	
	// Осуществим перебор по таблице содержащей формы отчетов и периоды действий.
...
//
Процедура уатВыборФормыРегламентированногоОтчетаПоУмолчанию(Форма) Экспорт
	
	ТаблицаФормОтчета = уатПолучитьТаблицуОтчетовДействующихВВыбранныйПериод(Форма);
...
//
Функция уатКоличествоФормСоответствующихВыбранномуПериоду(Форма) Экспорт
...
//
Функция уатВыбратьФормуОтчетаИзДействующегоСписка(Форма) Экспорт
	
	Результат = Неопределено;
	
	// Получим таблицу соответствующих периоду форм.
	ТаблицаВыбораФормы = уатПолучитьТаблицуОтчетовДействующихВВыбранныйПериод(Форма);
	...
	Для Каждого Стр Из ТаблицаВыбораФормы Цикл
		Если уатПустоеЗначение(Стр.ДатаКонецДействия) Тогда
...
// Проверяет является ли переданная форма формой баланса.
//
Функция уатЭтоФормаБаланса(Форма)
...
//Транспортный налог 29 января 2007 г. 16:56:21
Функция уатПолучитьСведенияОбОрганизации(Организация, ДатаЗначения, СписокПоказателей) Экспорт   // замена
	...
	Если ТипЗнч(СписокПоказателей) = Тип("Строка") Тогда
		...
		Пока ВхождениеЗапятой <> 0 Цикл
			...
			Если ПустаяСтрока(ИмяПоказателя) Тогда
				Продолжить;
			КонецЕсли;
			СписокПоказателей.Добавить(ИмяПоказателя, ИмяПоказателя);
			ВхождениеЗапятой = Найти(СписокПоказателейСтрока, ",");
		КонецЦикла;
	...
	Для Каждого ЭлементСписка Из СписокПоказателей Цикл
		...
		Если ИмяПоказателя = "ФИОРук" Тогда
			...
			Если Метаданные.НайтиПоПолномуИмени("РегистрСведений.ОтветственныеЛицаОрганизации") <> Неопределено Тогда
			...
			ИначеЕсли Метаданные.НайтиПоПолномуИмени("РегистрСведений.ОтветственныеЛицаОрганизаций") <> Неопределено Тогда
				Данные = Новый Структура("СтруктурнаяЕдиница",Организация, 
		...
		ИначеЕсли ИмяПоказателя = "ФИОБух" Тогда
			...
			ИначеЕсли Метаданные.НайтиПоПолномуИмени("РегистрСведений.ОтветственныеЛицаОрганизаций") <> Неопределено Тогда
				Данные = Новый Структура("СтруктурнаяЕдиница",Организация, 
				"ОтветственноеЛицо", Перечисления.ОтветственныеЛицаОрганизаций.ГлавныйБухгалтер);
		...
		ИначеЕсли ИмяПоказателя = "ОКАТО" Тогда
			Значение = Организация.КодПоОКАТО;
		...
		ИначеЕсли ИмяПоказателя = "ОГРН" Тогда
			...
			Исключение
				Значение = "";
				ВвестиЗначение(Значение, "Введите ОГРН - государственный регистрационный номер организации", "Строка");			
			КонецПопытки; 
		...
		ИначеЕсли ИмяПоказателя = "НаимНО" Тогда
			...
			Исключение
				Значение = "";
				ВвестиЗначение(Значение, "Введите Полное наименование ИФНС", "Строка");			
			КонецПопытки; 
		Иначе
...
//
Функция уатГоловнаяОрганизация(Организация)    // добавлено
Возврат Организация;	
КонецФункции // ГоловнаяОрганизация(Организация)()

// функция проверяет является ли организация ПБОЮЛ
//
Функция уатЭтоПБОЮЛ(Организация)   Экспорт   //  замена
Возврат Ложь;	
КонецФункции // ГоловнаяОрганизация(Организация)()
...
//   Булево
//
Функция уатТолькоЦифрыВСтроке(Знач СтрокаПроверки, УчитыватьЛидирующиеНули = Истина, УчитыватьПробелы = Истина) Экспорт

Если ТипЗнч(СтрокаПроверки) <> Тип("Строка") Тогда
	Возврат Ложь;
КонецЕсли;

Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтрокаПроверки) Тогда
	Возврат Истина;
КонецЕсли; 
...
//
Функция уатПредставлениеАдресаВФормате9Запятых(Знач АдресВФормате9Запятых, АнализироватьРегион = Ложь) Экспорт
...
Если СокрЛП(СоставляющиеАдреса[2]) <> "" Тогда
	...
	Если АнализироватьРегион Тогда
		Если уатТолькоЦифрыВСтроке(Регион) Тогда
			Регион = уатПолучитьНазваниеРегионаПоКоду(Регион);
		КонецЕсли;
	КонецЕсли;
...
Функция уатПолучитьНазваниеРегионаПоКоду(КодРег) Экспорт

Если уатПустоеЗначение(КодРег) Тогда
	Возврат "";
КонецЕсли;
