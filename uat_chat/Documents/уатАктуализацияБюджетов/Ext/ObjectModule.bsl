...
Функция ПроверкаКонтролируемыхЗначений(СтруктураДанныхШапки, СтрокаОборотовДляКонтроля)
	...
	Иначе
		...
		Для каждого СтрокаПроверки Из ТабРезультата Цикл
			...
			|ИЗ
			|	РегистрНакопления.уатКонтролируемыеЗначенияБюджетов.Обороты(&ДатаНачала, &ДатаНачала, , 
...
Функция СформироватьЗапросПоДаннымДокумента(СтруктураДанныхШапки)
	...
	|	ИЗ
	|	Документ.уатАктуализацияБюджетов.ПереносТранзакций КАК ПереносТранзакцийДокумент
	...
	|		ИЗ
	|			Документ.уатАктуализацияБюджетов.ПереносТранзакций КАК ОперацииДата
	|		ГДЕ
	...
	|		ИЗ
	|			Документ.уатАктуализацияБюджетов.ПереносТранзакций КАК АктуализацияБюджетовПереносТранзакций
	|		ГДЕ
	...
	|ИЗ
	|		РегистрСведений.уатГраницыПериодовБюджетирования КАК ГраницыПериодов				
	|				
	|СГРУППИРОВАТЬ ПО
	...
	| И (ПереносТранзакцийДокумент.ЦФО = ГраницыПланирования.ЦФО ИЛИ ГраницыПланирования.ЦФО = Значение(Справочник.Подразделения.ПустаяСсылка)) 
	| И (ПереносТранзакцийДокумент.Проект = ГраницыПланирования.Проект ИЛИ ГраницыПланирования.Проект = Значение(Справочник.Проекты.ПустаяСсылка))	
	|ГДЕ
...
Функция СформироватьПроводкиПоДаннымДокумента(СтруктураДанныхШапки)
	...
	|ИЗ
	|	Документ.уатАктуализацияБюджетов.ПереносТранзакций КАК ПереносТранзакцийДокумент
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.уатСтатьиОборотовПоБюджетам.ШаблоныПроводок КАК ТаблицаПроводок
...
Процедура ПереносТранзакций(Отказ, Заголовок)
	...
	НаборДвижений = Движения.уатОборотыБюджетов;
	...
	НаборДвижений.Загрузить(ТабОборотыБюджетов);
	...
	НаборДвижений = Движения.уатБюджетирование;
...
Функция СформироватьЗапросПоОборотам() Экспорт
	...
	|	ИЗ
	|		РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНачала, &ДатаКонца,"+Периодичность+", Сценарий = &Сценарий) КАК ОборотыБюджетовОборотыПлан
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ИЗ
	|		РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНачала, &ДатаКонца,"+Периодичность+", Сценарий = Значение(Справочник.уатСценарииПланирования.ПустаяСсылка)) КАК ОборотыБюджетовОборотыФакт
	|	) КАК ОборотыПланФакт
	// Курсы операций 
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|(ВЫБРАТЬ 
...
Функция СформироватьЗапросПоСчетам()
	...
	|	ИЗ
	|		РегистрБухгалтерии.уатБюджетирование.ОборотыДтКт(&ДатаНачала, &ДатаКонца,"+Периодичность+", , , , , Сценарий = &Сценарий) КАК БюджетированиеОборотыДтКтПлан
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ИЗ
	|		РегистрБухгалтерии.уатБюджетирование.ОборотыДтКт(&ДатаНачала, &ДатаКонца,"+Периодичность+", , , , , Сценарий = Значение(Справочник.уатСценарииПланирования.ПустаяСсылка)) КАК БюджетированиеОборотыДтКтФакт) КАК ОборотыДтКтПланФакт
//  Курсы валюты сценария планирования	
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|(ВЫБРАТЬ 
	|	ПериодПоследнейЗаписиКурсаНаДатуОборота.Дата КАК Дата, 
	...
	|	ИЗ
	|		РегистрБухгалтерии.уатБюджетирование.ОборотыДтКт(&ДатаНачала, &ДатаКонца,"+Периодичность+", , , , , Сценарий = &Сценарий) КАК ОборотыДтКтПлан
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ИЗ
	|		РегистрБухгалтерии.уатБюджетирование.ОборотыДтКт(&ДатаНачала, &ДатаКонца,"+Периодичность+", , , , , Сценарий = Значение(Справочник.уатСценарииПланирования.ПустаяСсылка)) КАК ОборотыДтКтФакт
	|	) КАК Обороты
	|ПО Обороты.Дата >= КурсыВнутр.Период
	|	И (КурсыВнутр.Валюта = &ВалютаСценария)
	|	СГРУППИРОВАТЬ ПО Обороты.Дата) КАК ПериодПоследнейЗаписиКурсаНаДатуОборота
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если АктуализироватьБюджеты Тогда
		...
		НаборДвижений = Движения.уатОборотыБюджетов;	
		НаборДвижений.Загрузить(ТабОборотовДокумента);
		...
		ТабПроводокДокумента.ЗаполнитьЗначения(Новый(Тип("СправочникСсылка.уатСтатьиОборотовПоБюджетам")),"СтатьяОборотов");
		
		НаборДвижений = Движения.уатБюджетирование;
		НаборДвижений.мТаблицаДвижений = ТабПроводокДокумента;
		Движения.уатБюджетирование.ВыполнитьДвижения();
		...
		|ИЗ
		|	РегистрСведений.уатГраницыПериодовБюджетирования КАК ГраницыПериодовБюджетирования
		|
		|ГДЕ
		|	(ГраницыПериодовБюджетирования.Сценарий = &Сценарий) И
		|	(ГраницыПериодовБюджетирования.ЦФО = ЗНАЧЕНИЕ(Справочник.Подразделения.ПустаяСсылка)) И
		|	(ГраницыПериодовБюджетирования.Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка))";
		...
		Если (НЕ Результат.Следующий())
			...
			НаборДвижений = Движения.уатГраницыПериодовБюджетирования;
