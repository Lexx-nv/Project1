...
Процедура ОбновитьКурсДокумента() Экспорт
	...
	Если Сценарий.ИспользоватьКурсыСценария Тогда
		РегистрКурсов=РегистрыСведений.уатКурсыВалютПоСценариям;
...
Процедура ОбновитьКурсИСуммуДокумента(СтараяВалюта) Экспорт
	...
	Сумма= уатОбщегоНазначенияТиповые.ПересчитатьИзВалютыВВалюту(Сумма,
...
Процедура ЗаполнитьЗависимымиОборотами(ТекСтатья,
	...
	ПустойСценарий   = Новый(Тип("СправочникСсылка.уатСценарииПланирования"));
	...
	|ИЗ
	|	РегистрСведений.уатЗависимостиОборотовПоСтатьямБюджетирования.СрезПоследних(&ДатаДвижения, (ИсхСтатьяОборотов = &Статья)
...
Процедура ПроверкаКурсовПоДате(СтруктураДанныхШапки,Отказ,Заголовок)
	...
	|	ИЗ
	|		Документ.уатБюджетнаяОперация.ЗависимыеОбороты КАК БюджетнаяОперацияЗависимыеОбороты
	|	
	|	ГДЕ
	...
	|	ИЗ
	|		Документ.уатБюджетнаяОперация КАК БюджетнаяОперация
	|	
	|	ГДЕ
	...
	уатБюджетирование.ПроверкаКурсовВалютСценарии(ТаблицаВалют,Отказ,Заголовок);
...
Процедура ДвиженияПоРегистрам(СтруктураДанныхШапки, Отказ, Заголовок)
	...
	НаборДвиженийОбороты = Движения.уатОборотыБюджетов;	
	НаборДвиженийОбороты.Загрузить(ТабОборотовДокумента);
	НаборДвиженийОбороты.Записывать = Истина;
	
	НаборДвиженийБюджетирование = Движения.уатБюджетирование;
	...
	НаборДвиженийБюджетирование.мТаблицаДвижений = ТабПроводокДокумента;
    НаборДвиженийБюджетирование.ВыполнитьДвижения();
	НаборДвиженийБюджетирование.Записывать = Истина;
...
Функция СформироватьЗапросПоДаннымДокумента(СтруктураДанныхШапки)
	...
	|	ИЗ
	|		Документ.уатБюджетнаяОперация КАК БюджетнаяОперацияШапка
	...
	|		ИЗ
	|			Документ.уатБюджетнаяОперация.ЗависимыеОбороты
	|		ГДЕ
	|			Документ.уатБюджетнаяОперация.ЗависимыеОбороты.Ссылка = &Ссылка
	|		ОБЪЕДИНИТЬ ВСЕ 
	|		ВЫБРАТЬ
	|			ПериодПланирования Как Дата
	|		ИЗ 
	|			Документ.уатБюджетнаяОперация
	|		ГДЕ
	|			Документ.уатБюджетнаяОперация.Ссылка = &Ссылка) КАК Обороты
