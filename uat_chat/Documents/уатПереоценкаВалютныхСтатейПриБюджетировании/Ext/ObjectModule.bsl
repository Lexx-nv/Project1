...
Процедура ДвиженияПоРегистрам()
	 ...
	 |ИЗ
	 |	РегистрБухгалтерии.уатБюджетирование.Остатки(&ДатаПереоценки, Счет.Валютный = ИСТИНА, , Сценарий = &Сценарий) КАК БюджетированиеОстатки
	 ...
	 Иначе
		 Запрос.Текст=Запрос.Текст+"
		 |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.уатКурсыВалютПоСценариям.СрезПоследних(&ДатаПереоценки,Сценарий=&Сценарий ) КАК КурсыВалютыОстатка
		 |		ПО БюджетированиеОстатки.Валюта = КурсыВалютыОстатка.Валюта
		 |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.уатКурсыВалютПоСценариям.СрезПоследних(&ДатаПереоценки, Валюта = &ВалютаУпрУчета И Сценарий=&Сценарий) КАК КурсыВалютыУпрУчета
		 |		ПО (ИСТИНА)
		 |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.уатКурсыВалютПоСценариям.СрезПоследних(&ДатаПереоценки, Валюта = &ВалютаСценария И Сценарий=&Сценарий) КАК КурсыВалютыСценария
		 |		ПО (ИСТИНА)";
	...
	НаборЗаписейОбороты = Движения.уатОборотыБюджетов;
	ТабОбороты = НаборЗаписейОбороты.Выгрузить();
	ТабОбороты.Очистить();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ (СуммаУпрРазница=0 И СуммаСценарияРазница=0) Тогда
			...
			Движение = Движения.уатБюджетирование.Добавить();
	...
	НаборЗаписейОбороты.Загрузить(ТабОбороты);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ДвиженияПоРегистрам();
	Движения.уатБюджетирование.Записывать  = Истина;
	Движения.уатОборотыБюджетов.Записывать = Истина;
