...
Процедура ЗаполнитьПоИнвентаризацииСервер()
	...
	#Если Клиент Тогда
		...
		Если Объект.Товары.Количество() = 0 Тогда
			Сообщить("В документе """ + Объект.ИнвентаризацияТоваровНаСкладе.Метаданные().Представление() + """ № " + Объект.ИнвентаризацияТоваровНаСкладе.Номер +" отсутствуют товары учетное количество которых превышает фактическое.");
		КонецЕсли;
