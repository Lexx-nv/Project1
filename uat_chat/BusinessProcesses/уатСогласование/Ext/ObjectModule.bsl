...
Процедура ПроверкиПередСтартом(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Предмет) Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Не указан предмет согласования!", Отказ);
		Возврат;
	КонецЕсли;
	...
	Если НЕ Рез.Пустой() Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Выбранный предмет согласования уже на согласовании или согласован!", Отказ);
		Возврат;
	КонецЕсли;
КонецПроцедуры
...
Процедура СогласоватьПараллельноПередСозданиемЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, СтандартнаяОбработка)
	...
	Для Каждого Строка Из Исполнители Цикл
		Задача = Задачи.уатЗадачаИсполнителя.СоздатьЗадачу();
