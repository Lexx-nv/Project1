...
Перем мИмяОтчета;
Перем мНаДату Экспорт; // Дата формирования отчета
Перем мВыведеноПериодов; // Счетчик выведенных в отчет периодов
Перем мГруппировокПоГоризонтали; // Количество группировок в выводе по горизонтали
Перем мЗначимыеГруппировкиГор; // Группировки, выводимые в отчет по горизонтали
...
Функция НастроитьПостроительДляСчетов(НастройкиПостроителя)
	...
	|"+уатБюджетирование.СформироватьТекстЗапросаПоПериодам(Периодичность,"ДанныеСчетов.Период")+"
	...
	|ИЗ
	|	Справочник.уатСчетаБюджета КАК СчетаБюджета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.уатБюджетирование.ОстаткиИОбороты(&ДатаНач, &ДатаКон, "+Периодичность+", , , , Сценарий = &Сценарий) КАК БюджетированиеОстаткиИОбороты
	...
	|			ИЗ
	|				ПланСчетов.уатБюджетирование КАК Бюджетирование
	|					ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.уатБюджетирование.ВидыСубконто КАК БюджетированиеВидыСубконто1
	|					ПО Бюджетирование.Ссылка = БюджетированиеВидыСубконто1.Ссылка И (БюджетированиеВидыСубконто1.НомерСтроки = 1)
	|					ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.уатБюджетирование.ВидыСубконто КАК БюджетированиеВидыСубконто2
	|					ПО Бюджетирование.Ссылка = БюджетированиеВидыСубконто2.Ссылка И (БюджетированиеВидыСубконто2.НомерСтроки = 2)
	|					ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.уатБюджетирование.ВидыСубконто КАК БюджетированиеВидыСубконто3
	...
	ПостроительДляСчетов.Параметры.Вставить("ПустаяСтатья",Новый(Тип("СправочникСсылка.уатСтатьиОборотовПоБюджетам")));
	...
	Для Сч=0 по ПостроительДляСчетов.ДоступныеПоля.Количество()-1 Цикл
		...
		ИначеЕсли Поле.Имя="СтатьяОборотов" Тогда
			МассивТипов=Новый Массив;
			МассивТипов.Добавить(Тип("СправочникСсылка.уатСтатьиОборотовПоБюджетам"));
			Поле.ТипЗначения=Новый ОписаниеТипов(МассивТипов);
			Поле.Отбор=Истина;
		ИначеЕсли Поле.Имя="Номенклатура" Тогда
			МассивТипов=Новый Массив;
			Если Сценарий.ДетализацияПланирования = Перечисления.уатДетализацияПланирования.НоменклатурнаяГруппа Тогда
				МассивТипов.Добавить(Тип("СправочникСсылка.НоменклатурныеГруппы"));
			ИначеЕсли Сценарий.ДетализацияПланирования = Перечисления.уатДетализацияПланирования.МодельТС Тогда
				МассивТипов.Добавить(Тип("СправочникСсылка.уатМоделиТС"));
			ИначеЕсли Сценарий.ДетализацияПланирования = Перечисления.уатДетализацияПланирования.ТипТС Тогда
				МассивТипов.Добавить(Тип("СправочникСсылка.уатТипыТС"));
			ИначеЕсли Сценарий.ДетализацияПланирования = Перечисления.уатДетализацияПланирования.ТС Тогда
...
Функция ПроверитьНаличиеГруппСтатейОборотовДляСтатейБюджетов()
	...
	Запрос = новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ СтатьяОборотов ИЗ Справочник.уатСтатьиБюджета
...
Функция НастроитьПостроительДляСтатей(НастройкиПостроителя)
	...
	Если УровеньГруппыСтатьиОборотовПоБюджетам = -1 Тогда
		ТекстЗАпроса_СтатьиБюджета = "Справочник.уатСтатьиБюджета КАК СтатьиБюджета";
	Иначе
		...
		Для тек_ур = 1 по УровеньГруппыСтатьиОборотовПоБюджетам+1 цикл
			...
			|ИЗ Справочник.уатСтатьиОборотовПоБюджетам
		...
		|ИЗ Справочник.уатСтатьиБюджета КАК СтБюдж
	...
	|"+уатБюджетирование.СформироватьТекстЗапросаПоПериодам(Периодичность,"ОборотыПоСтатьям.Период")+"
	...
	| ИЗ Справочник.уатСтатьиБюджета КАК СтатьиБюджета 
	...
	|ИЗ
	|	РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНач,&ДатаКон,"+Периодичность+",Сценарий=&Сценарий) КАК ОборотыБюджетовОбороты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ "+ТекстЗАпроса_СтатьиБюджета+"
	//Справочник.уатСтатьиБюджета КАК СтатьиБюджета
	...
	ПостроительДляСтатей.Параметры.Вставить("ПустаяСсылкаСтатьяОборотовПоБюджетам",Справочники.уатСтатьиОборотовПоБюджетам.ПустаяСсылка());
...
Функция СформироватьСписокСтатей()
	...
	Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
	             ...
	             |ИЗ
	             |	Справочник.уатСтатьиБюджета КАК СтатьиБюджета
	             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.уатСтатьиОборотовПоБюджетам КАК СтатьиОборотовПоБюджетам
...
Функция СформироватьСписокСчетов()
	...
	Запрос.Текст="ВЫБРАТЬ РАЗЛИЧНЫЕ
	             ...
	             |ИЗ
	             |	Справочник.уатСчетаБюджета КАК СчетаБюджета
	             |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланСчетов.уатБюджетирование КАК Бюджетирование
...
Функция НастроитьПостроительДляСтатейИсточники(НастройкиПостроителя)
	...
	СтруктураЗапроса = уатБюджетирование.СформироватьТекстЗапросаПоОборотам(СтруктураИспользуемых,Сценарий,ДатаНач,ДатаКон, СформироватьСписокСтатей());
...
Функция НастроитьПостроительДляСчетовИсточники(НастройкиПостроителя)
	...
	|	СчетаБюджета.Ссылка КАК СтрокаБюджета,
	|	БюджетированиеОстаткиИОбороты.Период КАК Период,
	|"+уатБюджетирование.СформироватьТекстЗапросаПоПериодам(Периодичность,"БюджетированиеОстаткиИОбороты.Период")+"
	...
	СтруктураЗапроса = уатБюджетирование.СформироватьТекстЗапросаПоОстаткамПериод(СтруктураИспользуемых,Сценарий,ДатаНач,ДатаКон,ПолучитьПериодичностьДляИсточников(), СформироватьСписокСчетов());
	...
	Для Сч=0 по ПостроительДляСчетов.ДоступныеПоля.Количество()-1 Цикл
		...
		ИначеЕсли Поле.Имя="Номенклатура" Тогда
			...
			ИначеЕсли Сценарий.ДетализацияПланирования=Перечисления.уатДетализацияПланирования.ТипТС Тогда
				МассивТипов.Добавить(Тип("СправочникСсылка.уатТипыТС"));
			ИначеЕсли Сценарий.ДетализацияПланирования=Перечисления.уатДетализацияПланирования.ТС Тогда
				МассивТипов.Добавить(Тип("СправочникСсылка.ТС"));
			Иначе
				МассивТипов.Добавить(Тип("СправочникСсылка.Номенклатура"));
			КонецЕсли;
			Поле.ТипЗначения=Новый ОписаниеТипов(МассивТипов);
...
Процедура СоздатьОперацию(Расшифровка, ЭтотОтчет)
	...
	НоваяОперация = Документы.уатБюджетнаяОперация.СоздатьДокумент();
	...
	Если Расшифровка.Свойство("СтатьяОборотов") И ТипЗнч(Расшифровка.СтатьяОборотов)=Тип("СправочникСсылка.уатСтатьиОборотовПоБюджетам") И НЕ Расшифровка.СтатьяОборотов.ЭтоГруппа Тогда
	...
	ИначеЕсли Расшифровка.Свойство("СтатьяБаланса") Тогда
		...
		|			ИЗ
		|				Справочник.уатСтатьиОборотовПоБюджетам.ШаблоныПроводок КАК СтатьиШаблоныПроводок
		|			
		|			ГДЕ
	...
	ИначеЕсли НоваяОперация.Сценарий.ДетализацияПланирования=Перечисления.уатДетализацияПланирования.Номенклатура Тогда
		НоваяОперация.Номенклатура=Справочники.Номенклатура.ПустаяСсылка();
	ИначеЕсли НоваяОперация.Сценарий.ДетализацияПланирования=Перечисления.уатДетализацияПланирования.НоменклатурнаяГруппа Тогда
...
//
// Параметры:
//	Расшифровка           - Структура, значение расшифровки, взятое из табличного документа,
//							и, в случае надобности, дополненное
...
// Обновляет построитель после использования запроса по источникам данных. Объемный текст запроса,
//	полученный при настройке построителя, существенно замедляет интерфейсную работу с полями построителя отчетов.
//
...
Процедура СформироватьТекстЗапросаПоисточникам(НетИсточников)
	...
	|Валюта КАК Валюта,
	|"+уатБюджетирование.СформироватьТекстЗапросаПоПериодам(Периодичность,"Период")+"
...
Процедура СформироватьНастройкиОтчета(МассивОтбора,СтруктураПредставлениеПолей, ОбновлениеПостроителя=Ложь)
	...
	|ИЗ
	|	РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНач,&ДатаКон,"+Периодичность+",Сценарий=&Сценарий) КАК ОборотыБюджетовОбороты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.уатСтатьиБюджета КАК СтатьиБюджета
	...
	|ИЗ
	|	РегистрБухгалтерии.уатБюджетирование.ОстаткиИОбороты(&ДатаНач,&ДатаКон,"+Периодичность+", , , ,Сценарий=&Сценарий) КАК БюджетированиеОстаткиИОбороты
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|				Бюджетирование.Ссылка КАК Ссылка,
	|				Бюджетирование.Валютный КАК Валютный,
	|				Бюджетирование.Количественный КАК Количественный,
	...
	|					ПО Бюджетирование.Ссылка = БюджетированиеВидыСубконто3.Ссылка И (БюджетированиеВидыСубконто3.НомерСтроки = 3)) КАК ВложенныйЗапрос
	|			ПО БюджетированиеОстаткиИОбороты.Счет = ВложенныйЗапрос.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.уатСчетаБюджета КАК СчетаБюджета
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь) Экспорт
	...
	Для Инд=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1  Цикл
		...
		// Если добавлялись одни и те же поля в строки и в колонки, то  у них будет неправильное автогенеренное имя,
		//	оканчивающееся на число
	...
	Для Каждого Реквизит Из Метаданные.Отчеты.уатОтчетПоБюджету.Реквизиты Цикл
		СтруктураНастроекОтчета.Вставить(Реквизит.Имя, ЭтотОбъект[Реквизит.Имя]);
	КонецЦикла;

	Для Каждого ТабличнаяЧасть Из Метаданные.Отчеты.уатОтчетПоБюджету.ТабличныеЧасти Цикл
