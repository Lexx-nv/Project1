...
Перем мМассивШиринКолонок Экспорт; // массив ширин колонок табличного документа для сохранения
									//между формированиями отчета
...
Перем мСтруктураДляОтбораПоКатегориям Экспорт; // содержит связь отборов текста запроса Построителя
												// и значений категорий
...
Перем СоответствиеТиповИнтервальныхПолей Экспорт; // Соответствие типов интервальных полей
...
// Заполняет поля основного реквизита (формируемого отчета)
//
// Параметры:
//	УниверсальныйОтчет - универсальный отчет, на движке которого строится нужный отчет.
//
...
// Процедура заполняет показатели отчета, представление показателей, флаги использования
//
//Параметры:
//	ИмяПоля - Имя поля показателя
//	ПредставлениеПоля - Представление для отображения в отчета
//	ВклПоУмолчанию - Использование показателя по умолчанию
//	ФорматнаяСтрока - строка форматирования показателя
//
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь, УказыватьРасшифровки = Истина) Экспорт
	...
	// Если выбрана периодическая группировка, необходимо установить соответствующее значение элемента
	//	отбора Периодичность, связанного с параметром периодичности итогов виртуальной таблицы регистра.
	...
	// Преобразуем исходный макет путем копирования областей, чтобы он соответствовал количеству выбранных показателей,
	// способу их расположения, количеству и расположению выбранных полей
	...
	Если ВыводитьДетальныеЗаписи Тогда
		...
		// Изменим макет: добавим по одной колонке для каждого независимого поля,
		//	которое будет выводиться на уровне детальных записей
		// на 1(2) меньше количества полей - одно уже есть
		Для Сч=1 По МассивИндексовНезависимыхВыбранныхПолей.Количество()-1 Цикл
		...
		// В добавленных колонках установим параметры областей, чтобы при выводе они заполнялись
		//	значениями соответствующих выбранных полей
		Сч = 0;
		Для Каждого Инд ИЗ МассивИндексовНезависимыхВыбранныхПолей Цикл
			...
			// Определим, можно ли использовать представление поля. Для этого попытаемся разобрать автоматический макет,
			//	область "Детали" построителя
			Попытка
...
//					КонецЕсли;
	...
	ДокументРезультат.ОбластьПечати = ДокументРезультат.Область(1,2,ДокументРезультат.ВысотаТаблицы,ШиринаТаблицы);
