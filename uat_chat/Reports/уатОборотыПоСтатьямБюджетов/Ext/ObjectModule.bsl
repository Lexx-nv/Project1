...
Перем мСтруктураНеСохраняемыеВНастройкеРеквизиты Экспорт; // Структура, содержит реквизиты отчета, не подлежащие 
                   //сохранению
...
Перем мМассивШиринКолонок Экспорт; // массив ширин колонок табличного документа для сохранения между 
                   // формированиями отчета
Перем мИсходныйМакетОтчета Экспорт; // исходный макет, используемый для отчета. По умолчанию "Макет", 
                  // но может быть переопределен
...
Процедура СформироватьНастройкиОтчета(МассивОтбора,СтруктураПредставлениеПолей,ОбновлениеПостроителя=Ложь)
	...
	Если НЕ СтатьяБаланса.Пустая()  Тогда
		...
		| 	ОборотыДт.Номенклатура.* КАК Номенклатура, " + уатБюджетирование.СформироватьТекстЗапросаПоПериодам(Периодичность,"ВЫБОР КОГДА НЕ ОборотыДт.Период Есть NULL Тогда ОборотыДт.Период ИНАЧЕ &ДатаКон КОНЕЦ ")+"
		...
		|		РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНач, &ДатаКон,{&Периодичность},Сценарий = &Сценарий {СтатьяОборотов,ЦФО,Проект, Контрагент,Номенклатура}) КАК ОборотыДт
		|			ПО СтатьиОборотов.Ссылка = ОборотыДт.СтатьяОборотов
		|УПОРЯДОЧИТЬ ПО СтатьиОборотов.Ссылка.Код
		|{УПОРЯДОЧИТЬ ПО
		|СтатьиОборотов.Ссылка.Код,
		|"+СтрЗаменить(Лев(уатБюджетирование.СформироватьТекстЗапросаПоПериодам
				(Периодичность,"ВЫБОР КОГДА НЕ ОборотыДт.Период Есть NULL Тогда ОборотыДт.Период ИНАЧЕ &ДатаКон КОНЕЦ "),
				СтрДлина(уатБюджетирование.СформироватьТекстЗапросаПоПериодам
		...
		| Контрагент.*,
		| Номенклатура.*,
		|" + уатБюджетирование.СформироватьТекстЗапросаПоПериодам(Периодичность,"ВЫБОР КОГДА НЕ ОборотыДт.Период Есть NULL Тогда ОборотыДт.Период ИНАЧЕ &ДатаКон КОНЕЦ ")+"
		| Валюта}";
	...
	Иначе
		...
		|" + уатБюджетирование.СформироватьТекстЗапросаПоПериодам(Периодичность,"ВЫБОР КОГДА НЕ ОборотыПоСтатьям.Период Есть NULL Тогда ОборотыПоСтатьям.Период ИНАЧЕ &ДатаКон КОНЕЦ ")+"
		...
		|ИЗ  Справочник.уатСтатьиОборотовПоБюджетам КАК СтатьиОборотов
		|"+?(ВыводитьНулевыеОбороты,"ЛЕВОЕ СОЕДИНЕНИЕ","ВНУТРЕННЕЕ СОЕДИНЕНИЕ")+" 
		|РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНач, &ДатаКон,{&Периодичность}, Сценарий=&Сценарий) КАК ОборотыПоСтатьям
...
Функция СформироватьСписокСтатей()
	...
	Если ТабОтбор.Количество()>0 Тогда // Есть отбор по статьям
		...
		ПостроительСтатьи.Текст="ВЫБРАТЬ
								...
								|СтатьиОборотовПоБюджетам.Ссылка.* КАК СтатьяОборотов}
		                        |ИЗ
		                        |	Справочник.уатСтатьиОборотовПоБюджетам КАК СтатьиОборотовПоБюджетам
...
Процедура СоздатьОперацию(Расшифровка, ЭтотОтчет)
	...
	ИначеЕсли Сценарий.ДетализацияПланирования=Перечисления.уатДетализацияПланирования.Номенклатура Тогда
		НоваяОперация.Номенклатура=Справочники.Номенклатура.ПустаяСсылка();
	ИначеЕсли Сценарий.ДетализацияПланирования=Перечисления.уатДетализацияПланирования.НоменклатурнаяГруппа Тогда
...
// Процедура заполняет показатели отчета
...
Процедура ВывестиСтроку(Выборка, Знач Индекс, СтруктураПараметров, ДокументРезультат, Расшифровка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ИмяПоказателя Из СтруктураПараметров.Показатели Цикл
			...
			// Если итоги выводятся не по всем уровням, и в колонках - связанное с показателем измерение,
			//	тогда общие итоги не выводим
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь) Экспорт
	...
	Для Инд=0 По ПостроительОтчета.ИзмеренияКолонки.Количество()-1  Цикл
		...
		// Если добавлялись одни и те же поля в строки и в колонки,
		//	то у них будет неправильное автогенеренное имя, оканчивающееся на число
