...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если НомерПоказателя > Показатели.Количество()-1 Тогда
		НомерПоказателя = 0;
	КонецЕсли;
...
Процедура СформироватьОтчет(Диаграмма, ЕстьОшибки = Ложь)	Экспорт
	...
	//	ПостроительОтчета.Параметры.Вставить("ДатаКон", ?(уатОбщегоНазначения.уатЗначениеНеЗаполнено(ДатаКон), ДатаКон,
	//		НачалоДня(ДатаКон + 60*60*24)));
	//Иначе
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(Расшифровка, СтандартнаяОбработка, ЭтотОтчет) Экспорт
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Если Выбор.Значение = 0 Тогда
			...
			Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ИмяОтчетаДляРасшифровкиПоДокументам) Тогда
...
// Процедура транспонирует диаграмму, 
//меняет местами первую группировок серий и первую группировку точек
//
