  ...
  Функция ТекстЗапросаПоСценарию(СтрокаСценарий,СтруктураПараметров,Номер,ПериодДанных, СтруктураГруппировки)
	...
	Если НЕ СтатьяБаланса.Пустая() Тогда
		...
		ТекстЗапроса="
			...
			|	ИЗ
			|		РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНач_"+Номер+", &ДатаКон_"+Номер+","+ПериодДанных+",Сценарий = &Сценарий_"+Номер+") КАК ОборотыБюджетовДебет
			...
			|	ИЗ
			|		РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНач_"+Номер+", &ДатаКон_"+Номер+", "+ПериодДанных+",Сценарий = &Сценарий_"+Номер+") КАК ОборотыБюджетовКредит
			|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
			|				СтатьиШаблоныПроводок.Ссылка КАК Ссылка,
			|				СтатьиШаблоныПроводок.КоэффициентДляСуммы КАК КоэффициентДляСуммы,
			|				СтатьиШаблоныПроводок.КоэффициентДляКоличества КАК КоэффициентДляКоличества
	...
	Иначе
			...
			ТекстЗапроса=ТекстЗапроса+"
			|ИЗ
			|	РегистрНакопления.уатОборотыБюджетов.Обороты(&ДатаНач_"+Номер+", &ДатаКон_"+Номер+","+ПериодДанных+", Сценарий=&Сценарий_"+Номер+") КАК ОборотыБюджетовОбороты
			|СГРУППИРОВАТЬ ПО
			|Сценарий,
			|Период,
			|СтатьяОборотов,
...
Процедура СформироватьНастройкиОтчета(МассивОтбора,СтруктураПредставлениеПолей)
	...
	|"+уатБюджетирование.СформироватьТекстЗапросаПоПериодам(ПериодДанных,"ДанныеСценариев.Период")+"
	|Валюта.* КАК Валюта}";
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь) Экспорт
	...
	Для Каждого Реквизит Из Метаданные.Отчеты.уатСравнительныйАнализОборотовПоСтатьямБюджетов.Реквизиты Цикл
		СтруктураНастроекОтчета.Вставить(Реквизит.Имя, ЭтотОбъект[Реквизит.Имя]);
	КонецЦикла;

	Для Каждого ТабличнаяЧасть Из Метаданные.Отчеты.уатСравнительныйАнализОборотовПоСтатьямБюджетов.ТабличныеЧасти Цикл
