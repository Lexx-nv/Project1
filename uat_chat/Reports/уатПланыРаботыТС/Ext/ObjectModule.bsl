...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	|	Планы.Маршрут КАК Маршрут,
	|	ПРЕДСТАВЛЕНИЕ(Планы.Маршрут),
	...
	|	ВЫБОР
	|		КОГДА Планы.ПараметрВыработки.Временный
	|			ТОГДА -РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0), ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0), СЕКУНДА, Планы.КоличествоОборот), ЧАС) * 3600 / 6000 + Планы.КоличествоОборот / 6000 + РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0), ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0), СЕКУНДА, Планы.КоличествоОборот), ЧАС)
	|		ИНАЧЕ Планы.КоличествоОборот
	...
	|	Планы.ДетализацияПланирования,
	|	Планы.Номенклатура,
	|	Планы.ПараметрВыработки
	|{ВЫБРАТЬ
	|	ПериодПланирования,
	|	Сценарий.*,
	|	Подразделение.*,
	...
	|	ПараметрВыработки.*}
	|ИЗ
	|	РегистрНакопления.уатПланРаботыТС.Обороты(&ДатаНач, &ДатаКон, Регистратор {&Периодичность}, {Сценарий.* КАК Сценарий, Подразделение.* КАК Подразделение, ДетализацияПланирования.* КАК ДетализацияПланирования, ДокументПланирования КАК ДокументПланирования, Номенклатура.* КАК Номенклатура, ПараметрВыработки.* КАК ПараметрВыработки, Заказ.* КАК Заказ, Контрагент.* КАК Контрагент, Договор.* КАК Договор, Маршрут.* КАК Маршрут}) КАК Планы
	|{ГДЕ
	|	Планы.Регистратор КАК Регистратор,
	|	Планы.Период КАК Период,
	|	НАЧАЛОПЕРИОДА(Планы.Период, ДЕНЬ) КАК ПериодДень,
	...
	|	Планы.СтоимостьОборот + Планы.НДСОборот КАК СтоимостьСНДСУпр,
	|		(ВЫБОР
	|				КОГДА Планы.ДетализацияПланирования.ДатаВыбытия <> ДАТАВРЕМЯ(1, 1, 1)
	|					ТОГДА ИСТИНА
	|				КОГДА Планы.ДетализацияПланирования.ДатаВыбытия = ДАТАВРЕМЯ(1, 1, 1)
	|					ТОГДА ЛОЖЬ
	|			КОНЕЦ) КАК ТСВыбыло}	
	...
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("ДетализацияПланирования", "ДетализацияПланирования");
	...
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("ПараметрВыработки", "ПараметрВыработки");
	...
	УниверсальныйОтчет.ДобавитьИзмерениеСтроки("ДетализацияПланирования");
	
	
	// Добавление предопределенных отборов отчета.
	УниверсальныйОтчет.ДобавитьОтбор("Сценарий");
	УниверсальныйОтчет.ДобавитьОтбор("Подразделение");
	УниверсальныйОтчет.ДобавитьОтбор("ДетализацияПланирования");
