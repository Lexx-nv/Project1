...
#Если Клиент Тогда
	...
	Процедура ИнициализироватьПраваИНастройки() Экспорт
		...
		Макет = ПланыВидовХарактеристик.уатПраваИНастройки.ПолучитьМакет(ИмяМакета); 
		...
		Для НомерСтроки = 1 По Макет.ВысотаТаблицы Цикл
			...
			Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(СсылкаПВХправ) Тогда 
				...
				Сообщить("Ошибка при загрузке прав: в строке " + НомерСтроки + " макета <" + ИмяМакета + "> указан элемент отсутствующий в ПВХ ""Права и настройки"""+ " (" + ИмяЭлемПВХ + ")", СтатусСообщения.Важное);
				Продолжить; 
