...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Получим перечень настроек из внешней обработки
	//Реально работает
	//Попытка
	//	ИмяФайла = ПолучитьИмяВременногоФайла();
	//	Обработка = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Настройка системы ОКО");
	//	ДвоичныеДанные = Обработка.ХранилищеВнешнейОбработки.Получить();
	//	ДвоичныеДанные.Записать(ИмяФайла);
	//	Обр = ВнешниеОбработки.Создать(ИмяФайла);
	//	Макет = Обр.ПолучитьМакет("Настройки");
	//	мНастройка = Макет.ПОлучитьОбласть("Пользователи");
	//Исключение
	//КонецПопытки
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПользовательОС = "oko-rmm"  Тогда 
		ПодключитьОбработчикОжидания("ОбработчикСменаТекущейСтрокиРММ", 2, Ложь);
	ИначеЕсли ПользовательОС = "sokotovap" Или ПользовательОС = "oko-obdd" Или ПользовательОС = "oko-admin" Или ПользовательОС = "oko-kpp" Или ПользовательОС = "oko-kpp-2" Тогда 
		//ПодключитьОбработчикОжидания("ОбработчикСменаТекущейСтроки", 2, Ложь);
...
&НаКлиенте
Процедура ОбработчикСменаТекущейСтрокиРММ()
...
Процедура ОпределитьРазмерШрифтовФормы(МассивРазмеров)
	...
	//	ЭтаФорма.Элементы[Строка("ГруппаМалоеОбщее" + х)].Ширина = шРазделов;
	//КонецЦикла;	
	...
	Элементы.ГруппаМалоеОбщее0.Ширина = шРазделов/2 - 2;
	Элементы.ГруппаМалоеОбщее1.Ширина = шРазделов/2 - 2;
	
	Элементы.ГруппаМалоеОбщееЛиния.Ширина = шРазделов/2 - 4;
	Элементы.ГруппаМалоеОбщееРемонт.Ширина = шРазделов/2 - 4;
	Элементы.ГруппаМалоеОбщееГараж.Ширина = шРазделов/2 - 4;
	Элементы.ГруппаМалоеОбщееБСМТС.Ширина = шРазделов/2 - 4;
	
		
	ЭтаФорма.Элементы.НадписьИтого0КолСНарушениями.Шрифт = Новый Шрифт(Элементы.НадписьИтогоОбщее.Шрифт,,шрифтИтогоМалый);
	ЭтаФорма.Элементы.НадписьИтого1КолСНарушениями.Шрифт = Новый Шрифт(Элементы.НадписьИтогоОбщее.Шрифт,,шрифтИтогоМалый);
	...
	Элементы.ПереченьТССНарушениямиТС.Ширина = шГосНомер + 40;
...
Процедура  ЗаполнитьПереченьТСВРемонтеПоПодразделению(Подразделение)
	Отбор = Новый Структура("Подразделение",Подразделение.Наименование);
...
Процедура ЗаполнитьТаблицыПоПодразделению(Подразделение)
	...
	Для Каждого ХХХ Из ПереченьТСВЭксплуатацииПоПодразделению Цикл
		...
		НайдСтроки = ПереченьТСВРемонтеПолный.НайтиСтроки(Новый Структура("ТС", ХХХ.ТС));
		Если НайдСтроки.Количество() > 0 Тогда
			//И если она в ремонте - то она нам не нужна для последующего анализа
			Продолжить;
		КонецЕсли; 
   		
		Если Не ЗначениеЗаполнено(ХХХ.ТС.СистемаМониторинга) Или СокрЛП(ХХХ.ТС.ИДвСистемеНавигации) = "" Тогда //Нет привязки к БСМТС
			нСтрока = ПереченьТСНетДанных.Добавить();
			ЗаполнитьЗначенияСвойств(нСтрока,ХХХ.ТС);
			нСтрока.Причина = "Нет привязки к БСМТС";
			нСтрока.ТС = ХХХ.ТС;
			Продолжить;
		КонецЕсли;	
		
		ТекОбъект = ДанныеБСМТС.Получить(ХХХ.ТС.ИДвСистемеНавигации);
...
Процедура ВидимостьЭлементовФормы(ЭтоПриОткрытии = Ложь)
	...
	Если ЭтоПриОткрытии Тогда //При открытии формы - жестко контролируем окно для пользователя
		...
		Если ПользовательОС = "sokotovap" Или ПользовательОС = "oko-admin" Или ФлагАдминистраторСистемыОКО = Истина Тогда //
	...
	ЭтаФорма.Элементы.Подразделение.Видимость = ((ПереключательРазделы = 0) Или (ПереключательРазделы = 1)  Или (ПереключательРазделы = 4));
	...
	ЭтаФорма.Элементы.НадписьТСВГаражеКол.Заголовок = Строка(ПереченьТСВГараже.Количество() - 1);
	...
	ЭтаФорма.Элементы.НадписьИтого1Кол.Заголовок = Строка(ПереченьТСВГараже.Количество() - 1);
	...
	ЭтаФорма.Элементы.НадписьИтого1Кол1.Заголовок = Строка(ПереченьТСВГараже.Количество() - 1);
	...
	ЭтаФорма.Элементы.НадписьТСВРемонте.Заголовок = "ТС находящиеся в ремонте - " + Строка(ПереченьТСВРемонтеПолный.Количество());
	...
	Элементы.ПереченьТСВРемонтеПолный.ТекущаяСтрока = 0;
	Элементы.ПереченьТСВРемонтеПолный.ВыделенныеСтроки.Очистить();
	
	Элементы.ПереченьТСВРемонте.ТекущаяСтрока = 0;
	Элементы.ПереченьТСВРемонте.ВыделенныеСтроки.Очистить();
	
	Элементы.ПереченьТСВГараже.ТекущаяСтрока = 0;
	Элементы.ПереченьТСВГараже.ВыделенныеСтроки.Очистить();
...
Процедура РаскрасимТаблицыВГаражеИВРемонте()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЦветГаража <> Неопределено тогда
			...
			Оформление.Использование = Истина;
			Поле1 = Оформление.Поля.Элементы.Добавить();
			Поле1.Поле = Новый ПолеКомпоновкиДанных("ПереченьТСНетДанныхГаражныйНомер");
			Поле2 = Оформление.Поля.Элементы.Добавить();
			Поле2.Поле = Новый ПолеКомпоновкиДанных("ПереченьТСНетДанныхГосударственныйНомер");
			Поле3 = Оформление.Поля.Элементы.Добавить();
			Поле3.Поле = Новый ПолеКомпоновкиДанных("ПереченьТСНетДанныхТС");
			Поле4 = Оформление.Поля.Элементы.Добавить();
			Поле4.Поле = Новый ПолеКомпоновкиДанных("ПереченьТСНетДанныхПричина");
			Отбор = Оформление.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПереченьТСНетДанных.Гараж");
			Отбор.ПравоеЗначение = Выборка.Ссылка; 
			Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
			Отбор.Использование = Истина;
			Оформление.Оформление.УстановитьЗначениеПараметра("ЦветФона", ЦветГаража);	
