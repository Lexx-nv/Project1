...
Перем мПоследняяДатаОбновленияРассмотренностиПисем Экспорт; // Переменная хранит дату последнего обновления
															//рассмотренности писем
Перем мДоступныеУчетныеЗаписи Экспорт; // Переменная содержит список доступных учетных для создания писем
Перем мСтруктураДоступныхУчетныхЗаписей Экспорт; // Переменная содержит структуру с данными о доступе пользователя
													// к ученым записям и о предопределенных папках учетных записей
Перем мСоответствияГруппВходящие Экспорт; // Переменная содержит соответствие учетной записи и предопределенной
											// группы писем "Входящие" для учетной записи
Перем мСоответствияГруппИсходящие Экспорт; // Переменная содержит соответствие учетной записи и предопределенной
											// группы писем "Исходящие" для учетной записи
Перем мСоответствияГруппУдаленные Экспорт; // Переменная содержит соответствие учетной записи и предопределенной
											// группы писем "Удаленные" для учетной записи
Перем мСоответствияГруппЧерновики Экспорт; // Переменная содержит соответствие учетной записи и предопределенной
											// группы писем "Черновики" для учетной записи
Перем мСтатистикаРассмотренностиПисем Экспорт; // Переменная содержит соответствие группы писем электронной почты
												// и количества нерассмотренных писем в этой группе
Перем мСтатистикаСохраненныхПисем Экспорт; // Переменная содержит соответствие группы писем электронной почты
											// и количества сохраненных писем в этой группе
Перем мСтатистикаРассмотренностиПисемПоПредметам Экспорт; // Переменная содержит соответствие предмета классификации
															// писем и количества нерассмотренных писем данного предмета
Перем мОтображатьСписокПредметов Экспорт; // Признак отображения списка предметов электронных писем в форме
...
Процедура ОбновитьДоступныеУчетныеЗаписи() Экспорт
	...
	мСтруктураДоступныхУчетныхЗаписей = уатОтношенияСКлиентами.уатПолучитьДоступныеУчетныеЗаписи(глЗначениеПеременной("глТекущийПользователь"));
...
Процедура СобратьСтатистикуРассмотренностиПисем() Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.уатУчетныеЗаписиЭлектроннойПочты) Тогда
		...
		Если мОтображатьСписокПредметов Тогда
			...
			|ИЗ
			|	РегистрНакопления.уатПисьмаЭлектроннойПочты.Остатки КАК ПисьмаЭлектроннойПочтыОстатки
			|
			|ГДЕ
...
Процедура мПоместитьИсходныеДанныеВОбъект(XMLИсточник, ПотокВывода, ФормаОбъекта, ДатаЗаписи)
	...
	Для каждого СтрокаОтвет из ОтветыТаблицы Цикл
		...
		ИначеЕсли	ТипВидКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес	тогда
			...
			МассивПолей	=	РегламентированнаяОтчетность.РазложитьСтрокуВМассивПодстрок(Ответ,	"¤");
		...
		ИначеЕсли	ТипВидКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон	тогда
			...
			//Представление = Представление + ?((Не ПустаяСтрока(СтрокаКИ.Поле2КИ)),(ПроверкаПустойСтроки(Представление,Ложь)
			//	+"(" + СтрокаКИ.Поле2КИ + ")"),"");
			//Представление = Представление + ?((Не ПустаяСтрока(СтрокаКИ.Поле3КИ)),(ПроверкаПустойСтроки(Представление,
			//	ПустаяСтрока(СтрокаКИ.Поле2КИ))+СтрокаКИ.Поле3КИ),"");
			//Представление = Представление + ?((Не ПустаяСтрока(СтрокаКИ.Поле4КИ)),(ПроверкаПустойСтроки(Представление)
			//	+ "доб. " + СтрокаКИ.Поле4КИ), "");
			//СтрокаКИ.ПредставлениеКИ	= Представление
	...
	Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ПотокВывода.Наименование) тогда
		ПотокВывода.Наименование = Фамилия	+ " " + Имя + " " + Отчество;
	КонецЕсли;
	Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ПотокВывода.ДатаРождения) тогда
		ПотокВывода.ДатаРождения = ДатаРождения;
	КонецЕсли;
	Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ПотокВывода.ИНН) тогда
		ПотокВывода.ИНН = Ответ;
	КонецЕсли;
	
	//Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Контрагент) тогда
	//	Контрагент.Записать();
	//	ПотокВывода.ОбъектВладелец = Контрагент.Ссылка;
	//КонецЕсли;
	
	// паспортные данные физлиц !!!!!
	Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВидДокументаУдостоверяющегоЛичность)	или
		 НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СерияДокумента) или
		 НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(НомерДокумента) или
		 НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ДатаВыдачиДокумента) или
		 НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(КемВыданДокумент) или
		 НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(КодПодразделенияДокумента)	тогда
		НаборЗаписейПаспортныхДанных	= ФормаОбъекта.мНаборЗаписейДокументаУдостоверяющегоЛичность;
		//РегистрыСведений.ПаспортныеДанныеФизЛиц.СоздатьНаборЗаписей();
	...
	// гражданство !!!!!
	Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Гражданство) тогда
		НаборЗаписейГражданство	= ФормаОбъекта.мНаборЗаписейГражданство;
		//РегистрыСведений.ГражданствоФизЛиц.СоздатьНаборЗаписей();
	...
	// Семейное положение!!!!!
	Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СемейноеПоложение) тогда
		НаборЗаписейСемейноеПоложение	= ФормаОбъекта.мНаборЗаписейСемейноеПоложение;
		//РегистрыСведений.СемейноеПоложениеФизЛиц.СоздатьНаборЗаписей();
	...
	// воинский учет          !!!!!
	Если	НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ОтношениеКВоинскойОбязанности) или
		НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ОтношениеКВоинскомуУчету) или
		НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Годность) или
		НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Звание) или
		НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(НаличиеМобпредписания) или
		НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Военкомат) или
		НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВУС) тогда
		НаборЗаписейВУ								= ФормаОбъекта.мНаборЗаписейВоинскийУчет;
		//РегистрыСведений.ВоинскийУчет.СоздатьНаборЗаписей();
	...
	//Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ПотокВывода)
	//И НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Контрагент) тогда
	...
	Для каждого	СтрокаКИ	из	КИОбъекта	Цикл
		...
		Если 	
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле1КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле2КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле3КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле4КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле5КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле6КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле7КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле8КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле9КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Поле10КИ)	ИЛИ
			НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ПредставлениеКИ) тогда
			...
			НаборЗаписейКИ	=	ФормаОбъекта.НаборКонтактнойИнформации;
			...
			Для индЗаписи = 0 по КоличествоЗаписейКИ-1 Цикл
				...
				Если ЗаписьКИ.Тип = ТипВидКИ.Тип И ЗаписьКИ.Вид = ТипВидКИ И уатОбщегоНазначения.уатЗначениеНеЗаполнено(ЗаписьКИ.Представление) тогда
...
// Процедура создаёт физическое лицо на основании строки
//
// Параметры:
//	ТекущийПредмет - строка, предмет электронного письма
//
Процедура СоздатьФизлицо(ТекущийПредмет) Экспорт
	...
	ЗапросПоПредмету.УстановитьПараметр("Отправленное", Перечисления.уатСтатусыПисем.Отправленное);
	ЗапросПоПредмету.УстановитьПараметр("Полученное", 	Перечисления.уатСтатусыПисем.Полученное);
	ЗапросПоПредмету.Текст = "ВЫБРАТЬ
	                         |	уатПредметыЭлектронныхПисем.Регистратор КАК ЭП,
	                         |	ВЫБОР КОГДА уатВложенияЭлектронныхПисем.ИмяФайла ПОДОБНО ""%.rps"" ТОГДА ИСТИНА ИНАЧЕ ЛОЖЬ КОНЕЦ КАК ЕстьРезультатОпроса,
	                         |	ВЫБОР КОГДА уатВложенияЭлектронныхПисем.ИмяФайла ПОДОБНО ""%.rps"" ТОГДА уатВложенияЭлектронныхПисем.Хранилище КОНЕЦ КАК Хранилище,
	                         |	уатПредметыЭлектронныхПисем.Регистратор.КомуТЧ.(
	                         |		ВЫБОР КОГДА уатПредметыЭлектронныхПисем.Регистратор.СтатусПисьма = &Отправленное ТОГДА уатПредметыЭлектронныхПисем.Регистратор.КомуТЧ.АдресЭлектроннойПочты ИНАЧЕ """" КОНЕЦ КАК АдресКому
	                         |	) КАК Кому,
	                         |	ВЫБОР КОГДА уатПредметыЭлектронныхПисем.Регистратор.СтатусПисьма = &Полученное ТОГДА уатПредметыЭлектронныхПисем.Регистратор.ОтправительАдресЭлектроннойПочты ИНАЧЕ """" КОНЕЦ КАК АдресОтКого
	                         |ИЗ
	                         |	РегистрСведений.уатПредметыЭлектронныхПисем КАК уатПредметыЭлектронныхПисем
	                         |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.уатВложенияЭлектронныхПисем КАК уатВложенияЭлектронныхПисем
	                         |		ПО уатПредметыЭлектронныхПисем.Регистратор = уатВложенияЭлектронныхПисем.Объект
	                         |
	                         |ГДЕ
	                         |	уатПредметыЭлектронныхПисем.Предмет = &Предмет И
	                         |	(уатВложенияЭлектронныхПисем.ИДФайлаПочтовогоПисьма = """") И
	                         |	(уатВложенияЭлектронныхПисем.Объект.Ссылка ССЫЛКА Документ.уатЭлектронноеПисьмо)";
	...
	Пока ВыборкаЗапроса.Следующий() Цикл
		Если ВыборкаЗапроса.ЕстьРезультатОпроса тогда
			...
			Попытка
				...
				Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ДатаЗаполненияОпроса) тогда
					СтрокаИД 				= ИсходныеДанные.Добавить();
		...
		Пока ВыборкаПолучателей.Следующий() Цикл
			Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВыборкаПолучателей.АдресКому) тогда
				СтрокаИД 				= ИсходныеДанные.Добавить();
			...
			КонецЕсли;
		КонецЦикла;
		Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВыборкаЗапроса.АдресОтКого) тогда
			СтрокаИД 				= ИсходныеДанные.Добавить();
			СтрокаИД.ВидДанных 		= "КИ_адресЭП";
		...
		Если СтрокаИД.ВидДанных = "XML" тогда
			...
			//Попытка
				...
				Если АнкетаИзФайла = Неопределено тогда
					Продолжить;
				КонецЕсли;
				Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ОбъектЗагрузки) тогда
					ОбъектЗагрузки = ОбработкаРезультатовАнкетирования.НайтиОбъектПоАнкетнымДанным(Файл, АнкетаИзФайла);
				КонецЕсли;
				Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ОбъектЗагрузки) тогда
					Ответ = Вопрос("Найдено физлицо, имеющее такое же ФИО и дату рождения, как загружаемое. Возможно такое физлицо уже есть в ИБ. Продолжить загрузку данных?", РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Нет);
				...
				Пока Файл.Прочитать() Цикл
					Если (Файл.ТипУзла  = ТипУзлаXML.НачалоЭлемента) и ((Файл.Имя = "Вопрос")) Тогда
						...
						КонецЕсли;
						мПоместитьИсходныеДанныеВОбъект(Файл, ФЛ, ФормаФЛ, Дата(СтрокаИД.Представление));
					КонецЕсли;
				КонецЦикла;
	...
	Если НЕ ФЛ.ЭтоНовый() тогда
		...
		|
		|ГДЕ
		|	уатЭлектронноеПисьмо.Предмет = &Предмет");
...
// Процедура создаёт опрос на основании строки
//
// Параметры:
//	ТекущийПредмет - строка, предмет электронного письма
//
		...
		Если ВыборкаЗапроса.ЕстьРезультатОпроса тогда
			...
			//Попытка
				...
				Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ОбъектЗагрузки) тогда
					ОбъектЗагрузки = ОбработкаРезультатовАнкетирования.НайтиОбъектПоАнкетнымДанным(Файл, Анкета);
				КонецЕсли;
				Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ДокОпрос) тогда
					Ответ = Вопрос("Найден документ опрос по данной анкете. Возможно такой документ уже есть в ИБ. Продолжить загрузку данных?", РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Нет);
					Если Ответ = КодВозвратаДиалога.Нет тогда
				...
				Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(Анкета) тогда
					Продолжить;
				КонецЕсли;
...
// Восстановим сохраненное значение отображения списка предметов
