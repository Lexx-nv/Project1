...
Перем МенеджерыДляПлановОбмена Экспорт; // Структура менеджеров объектов
...
Перем ФлагКомментироватьОбработкуОбъектов Экспорт; // Признак вывода комментариев в окне сообщений
...
Перем одТипУзлаXML_КонецЭлемента  Экспорт; // Конец элемента узла
Перем одТипУзлаXML_НачалоЭлемента Экспорт; // Начало элемена узла
Перем одТипУзлаXML_Текст          Экспорт; // Текст - Тип узла

Перем ЗначениеПустаяДата Экспорт; // Константа для пустой даты

Перем одСообщения;             // Соответствие. Ключ - код ошибки,  Значение - описание ошибки

Перем мСписокМакетовПравилОбмена Экспорт; // Список макетов для правил обмена
...
Перем ЗагруженныеГлобальныеОбъекты Экспорт; // Структура загруженных глобальных объектов
Перем ЧислоХранимыхЗагруженныхОбъектов Экспорт;  // число хранимых загруженных объектов, 
                                          ...
                                          // очищается
Перем ЗапоминатьЗагруженныеОбъекты Экспорт; // признак запоминания загруженных объектов
...
Перем мОбработкаДляЗагрузкиДанных Экспорт; // Обработка, используемая для загрузки данных (универсальный обмен)

Перем мСоотвествиеПустыхЗначенийТипов;
Перем мСоотвествиеОписаниеТипов;

Перем мБылиПрочитаныПравилаОбменаПриЗагрузке Экспорт; // Признак того, что были прочитаны правила обмена
...
Перем СобытияПослеЗагрузкиПараметров Экспорт; // Структура событий после загрузки параметров
...
// Функция получает строковый номер без префикса
//
...
// Процедура инициализирует протокол обмена
//
...
// Процедура завершает протокол обмена
//
...
// Функция записывает информацию об ошибке в протокол
//
//Возвращаемое значение:
...
// Функция записывает информацию об ошибке обработчика очистки данных
//
//Возвращаемое значение:
//	Строка сообщения об ошибке
//
...
// Функция записывает информацию об ошибке при загрузке обработчика ПКО
//
//Возвращаемое значение:
//	Строка сообщения об ошибке
//
...
// Функция записывает информацию об ошибке при выгрузке обработчика ПКО
//
//Возвращаемое значение:
//	Строка сообщения об ошибке
//
...
// Функция записывает информацию об ошибке обработчика ПКС
//
//Возвращаемое значение:
//	Строка сообщения об ошибке
//
...
// Функция записывает информацию об ошибке обработчика ПВД
//
//Возвращаемое значение:
//	Строка сообщения об ошибке
//
...
// Функция записывает информацию об ошибке обработчика конвертации
//
//Возвращаемое значение:
//	Строка сообщения об ошибке
//
...
// Параметры:
//  ПКО - Строка таблицы значений    - содержит правило конвертации объекта
//  ПравилаКонвертацииСвойствОбъекта - ТаблицаЗначений - содержит правила конвертации свойств
//		или группы свойств объекта из правила ПКО
//  ПКГС - Строка таблицы значений   - содержит правило конвертации группы свойств
//  
...
// Устанавливает значение параметра в таблице
...
// Инициализирует начальные значения параметров.
//Вызывается при загрузке правил обмена.
//
...
// Выполняет запись объектов в информационную базу из очереди незаписанных объектов
...
// Функция получает строку ограничения по дате для запроса
//
//Возвращаемое значение:
//	Строка запроса с условием ограничения по дате
//
...
// Заполняет список типов данных, доступных для удаления
//
...
// Функция обновляет пометки всех родителей у правил выгрузки
//Вызывается при загрузке правил обмена
//Параметры:
//	СтрокиДереваПравилВыгрузки - дерево строк с правилами выгрузки
//	НужноУстанавливатьПометки - если истина, то устанавливает, если ложь, то снимает
//Возвращаемое значение:
//	Истина - если успешно
//
...
//
//Параметры:
//	РезультирующаяСтрокаСИнформациейОбОшибке - строка протокола сообщений
//
...
// Функция выполняет ряд действий перед началом чтения данных
//
//Параметры:
//	СтрокаДанных - данные файла обмена
//Возвращаемое значение:
//	Истина, если выполнено успешно
//
...
// Процедура открывает форму настроек отладки обработчиков выгрузки данных
//
...
// Процедура открывает форму настроек отладки обработчиков загрузки данных
//
...
// Действия, выполняемые после завершения чтения данных
//
...
// Выполняет распаковку файла из архива
//
//Параметры:
//	ИмяФайлаДляРаспаковки - строка имени файла-источника
//
// Возвращаемое значение:
//   Строка – полное имя файла с учетом каталога.
...
// Передает информацию о записи в приемник
//
...
// Передает параметр в приемник перед выгрузкой данных
//
...
// Передает таблицу параметров в приемник перед выгрузкой данных
//
...
// Процедура создает структуру менеджеров объектов метаданных
// и структуру сообщений, выводимых в ходе обмена
//
...
// Функция определяет наличие всех необходимых параметров для подключения к ИБ
//Параметры:
//	СтруктураПодключения - структура - структура с ключами ФайловыйРежим, КаталогИБ, ИмяСервера, ИмяИБНаСервере
//	СтрокаПодключения - строка для пакетного запуска 1С-предприятия
//	СтрокаСообщенияОбОшибке - возвращает строку об ошибке
//Возвращаемое значение:
//	Булево - Истина, если параметров подключения к ИБ достаточно
//
...
// Подключается к ИБ - приемнику
//
// Возвращаемое значение:
//	Обработка-объект УниверсальныйОбменДаннымиXML - если подключение успешно,
//	Неопределено - если подключение неуспешно
//
