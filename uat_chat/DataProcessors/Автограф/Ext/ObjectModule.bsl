...
Функция Алга(ТекстЗапроса,РазобратьJSON0 = Ложь) Экспорт
	...
	//ФаилЗапросаJSON = "C:\Users\1c\Desktop\Обмен_1с\1.txt";
	//ЗапистьВJSON(Отказ,СтрДанные,ФаилЗапросаJSON);
...
//	HTTPЗапрос = Новый HTTPЗапрос("GetStage?schemaID=nvds&IDs="+идАгр+"&SD="+ДатаНачал+"&ED="+ДатаКонеч+"&stageName=*", Заголовки);
//	HTTPЗапрос = Новый HTTPЗапрос("GetTrips?schemaID=nvds&IDs="+идАгр+"&SD="+ДатаНачал+"&ED="+ДатаКонеч+"&tripSplitterIndex=-1", Заголовки);
//	HTTPЗапрос = Новый HTTPЗапрос("EnumParameters?schemaID=nvds&IDs="+идАгр, Заголовки);
	
//	HTTPЗапрос = Новый HTTPЗапрос("/TachoByDriverItem?"+"&SD="+ДатаНачал+"&ED="+ДатаКонеч+"&cardIDs=NULL", Заголовки);
//	HTTPЗапрос = Новый HTTPЗапрос("/EnumDevices/nvds", Заголовки);
//	HTTPЗапрос = Новый HTTPЗапрос("/EnumSchemas", Заголовки);
...
Процедура ПроверитьПробегИзБСМТС(ТБл,ДатаНачала,ДатаОкончания)
	Сообщить(ТекущаяДата());
	//Состояние("Проверка данных в системе GPS...");
	Для каждого Стр из Тбл Цикл
		//Состояние = "Проверка данных GPS : "+сокрЛП(Стр.ГаражныйНомер)+" "+стр.ТС;
		//Сообщить("Проверка данных GPS : "+сокрЛП(Стр.ГаражныйНомер)+" "+стр.ТС);
		Если Стр.ФлагТСсБСМТС = 0 Тогда
			Продолжить;
		КонецЕсли;	
		Стк = глАвтограф.глПробегАТ(Стр.ТС,ДатаНачала,ДатаОкончания);
