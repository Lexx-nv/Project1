Перем мФормаИндикации Экспорт; // Форма, которая будет использоваться как форма индикации
Перем мСписокРеквизитовОбъекта Экспорт; // Список реквизитов объекта
...
//   СписокЗначений
//
Функция ИзСтрокиСРазделителями(СтрокаТекста, Разделитель, НаличиеКавычек, НеУчитыватьТекстСТабуляциями = Ложь) Экспорт

	Если НЕ НеУчитыватьТекстСТабуляциями И ФорматФайла = "ТекстCТабуляциями" Тогда
		НаличиеКавычек = Ложь;
	...
	Для а=1 По СтрДлина(СтрокаТекста) Цикл
		...
		Если Сред(СтрокаТекста,а,1) = Разделитель Тогда
			...
			ИначеЕсли НаименованиеПрограммы = "MSOutlook" И НЕ НеУчитыватьТекстСТабуляциями И ФорматФайла = "ТекстCТабуляциями" Тогда
...
// 
Процедура НачатьНастройкуЗагрузки(ЭтаФорма) Экспорт
	...
	ЭтотОбъект.ПолучитьФорму("ФормаВыполненияЗагрузки", ЭтаФорма).ОткрытьМодально();
	
КонецПроцедуры

// Процедура открывает форму для настройки выгрузки данных.
// 
Процедура НачатьНастройкуВыгрузки(ЭтаФорма) Экспорт

	ЗаполнитьТаблицуПолейВыгрузки();
	ЭтотОбъект.ПолучитьФорму("ФормаВыполненияВыгрузки", ЭтаФорма).ОткрытьМодально();
...
Процедура ЗаписатьВыгружаемуюИнформацию() Экспорт
	...
	Если Метаданные.Справочники[ОпределитьИмяТаблицыОбъекта()].Иерархический Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	И
...
Процедура ЗаписатьЗагружаемуюИнформацию(РодительДляЗагрузки = Неопределено) Экспорт
	...
	Для каждого СтрокаТаблицы Из НастройкаЗагрузкиЗначений Цикл
		...
		Если СтрокаТаблицы.Объект1С = Неопределено ИЛИ СтрокаТаблицы.Объект1С.Пустая() Тогда
			...
			Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(РодительДляЗагрузки) Тогда
			...
			ИначеЕсли ТипОбъекта = "ЛичныеКонтакты" Тогда
				Объект1С.ОбъектВладелец = глЗначениеПеременной("глТекущийПользователь");
