...
Процедура ПриОткрытии(Отказ)
	...
	//ИначеЕсли ПользовательОС = "sokotovap" Или ПользовательОС = "oko-admin" Или ПользовательОС = "oko-kpp" Или ПользовательОС = "oko-kpp-2" Тогда 
	//	//ПодключитьОбработчикОжидания("ОбработчикСменаТекущейСтроки", 2, Ложь);
...
Процедура ЗаполнитьТаблицыПоПодразделению(Подразделение)
	...
	Для Каждого ХХХ Из ПереченьТСВЭксплуатацииПоПодразделению Цикл
		...
		Если ТипЗнч(ТекОбъект) = Тип("Соответствие") Тогда //Данные есть в нужном нам виде
			...
			//Проверим дату и время последних данных
			...
			Если LastData < ДатаСравнения Тогда //Последней дате данных БСМТС более суток
				ТекущееПоложение = ТекОбъект.Получить("Final").Получить("CurrLocation");
				//Сообщить(ТекущееПоложение);
				//Сообщить(ХХХ.ТС.ГаражныйНомер);
				нСтрока = ПереченьТСНетДанных.Добавить();
				ЗаполнитьЗначенияСвойств(нСтрока,ХХХ.ТС);
				нСтрока.Причина = "Данные просрочены - " + Строка(LastData);
				нСтрока.ТС = ХХХ.ТС;
				//Сообщить(ХХХ.ТС.ГаражныйНомер + " - " + Строка(LastData));
				Продолжить;
			КонецЕсли;	
			...
			Если НайдСтроки.Количество() > 0 Тогда //Нашли такой гараж
				нСтрока = ПереченьТСВГараже.Добавить();
				ЗаполнитьЗначенияСвойств(нСтрока,ХХХ.ТС);
				нСтрока.Гараж = НайдСтроки[0].Ссылка;
			Иначе //значит где-то на линии	
...
Процедура ВидимостьЭлементовФормы(ЭтоПриОткрытии = Ложь)
	...
	ЭтаФорма.Элементы.НадписьИтого3Кол.Заголовок = Строка(ПереченьТСНетДанных.Количество());
	...
	ЭтаФорма.Элементы.НадписьИтого3Кол1.Заголовок = Строка(ПереченьТСНетДанных.Количество());
