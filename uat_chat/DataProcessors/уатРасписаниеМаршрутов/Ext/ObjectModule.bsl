...
//
Процедура ЗаполнитьРейсыМаршрута(ТабДокумент, Маршрут, Выход, Индикатор = Ложь)	Экспорт
	
	Если Индикатор Тогда
		ФормаИндикатора = ПолучитьОбщуюФорму("ХодВыполненияОбработкиДанных");
		ФормаИндикатора.КомментарийЗначения = "Заполнение расписания маршрута";
	...
	Если Индикатор Тогда
		КолСтрок = ВыборкаЗапросаПункты.Количество();
	КонецЕсли;	
	...
	Пока ВыборкаЗапросаПункты.Следующий() Цикл
		...
		Если Индикатор Тогда
			Инд = Инд + 1;
			ФормаИндикатора.ЭлементыФормы.Индикатор.Значение = Инд/КолСтрок * 100;
		КонецЕсли;	
	КонецЦикла;
...
Процедура СкопироватьРейс(мВыход, мРейс) Экспорт
	Перем мВремя;
	
	Если ВвестиДату(мВремя, "Продолжительность", ЧастиДаты.Время) Тогда
...
Функция ПересчитатьВремяОтправления(мМаршрут, мВыход, мРейс) Экспорт
	
	Перем мВремя;
	
	Если НЕ ВвестиДату(мВремя, "Укажите временную разницу", ЧастиДаты.Время) Тогда
		Возврат Ложь;
	КонецЕсли;
...
//
Процедура ПечатьМаршрута(мМаршрут, мВыход) Экспорт
