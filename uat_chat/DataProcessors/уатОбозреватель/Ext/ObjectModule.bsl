Перем РесурсVerId Экспорт; // Имя ресурса.
Перем СерверИсточник Экспорт; // Сервер - источник обновления.
Перем СерверИсточникДляПроверкиНаличияОбновления Экспорт; //Сервер - источник проверки наличия обновления.
Перем АдресРесурсовДляПроверкиНаличияОбновления Экспорт; //Адрес ресурсов проверки наличия обновления.
Перем КаталогОбновлений Экспорт; // Каталог обновлений.
...
Функция Проверить(ЭлементыФормы) Экспорт
	...
	ВерсияМетаданныхУАТ = уатОбщегоНазначения.уатНомерВерсииКонфигурации();
	...
	Если НЕ ПустаяСтрока(ИмяФайла) Тогда
		...
		Иначе
			...
			Если стПараметры.Version <> ВерсияМетаданныхУАТ Тогда

				Если Найти(стПараметры.FromVersions,СокрП(ВерсияМетаданныхУАТ)+";")=0 Тогда
			...
			ЭлементыФормы.ТекущаяВерсия.Значение      = ВерсияМетаданныхУАТ;
...
АдресРесурсовДляПроверкиНаличияОбновления = "/buhplace/ITSRepV/V8Update/Configs/UAT_Prof/";
