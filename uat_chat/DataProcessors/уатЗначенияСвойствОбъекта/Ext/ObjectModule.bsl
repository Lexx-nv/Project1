...
Функция ПрочитатьТаблицуСвойствИЗначений()
	...
	Запрос.УстановитьПараметр("НазначениеСвойств",       ?(НазначениеСвойств = Неопределено, уатОбщегоНазначения.уатПолучитьСписокНазначенийСвойствКатегорийОбъектовПоСсылке(ОбъектОтбораЗначений), НазначениеСвойств));
	...
	|	РегистрСведений.уатЗначенияСвойствОбъектов.Значение            КАК Значение
	|
	|ИЗ
	|// Отбираются свойства, предназначенные для заданного типа объектов.
	|	(
	|	ВЫБРАТЬ 
	|		ПланВидовХарактеристик.уатСвойстваОбъектов.Ссылка          КАК Ссылка,
	|		ПланВидовХарактеристик.уатСвойстваОбъектов.Наименование    КАК Наименование,
	|		ПланВидовХарактеристик.уатСвойстваОбъектов.ПометкаУдаления КАК ПометкаУдаления
	|
	|	ИЗ
	|		ПланВидовХарактеристик.уатСвойстваОбъектов
	|
	|	ГДЕ
	|		ПланВидовХарактеристик.уатСвойстваОбъектов.НазначениеСвойства В ( &НазначениеСвойств )
	|
	...
	|ПО
	|	РегистрСведений.уатЗначенияСвойствОбъектов.Свойство = СвойстваОбъектов.Ссылка
	|	И
	|	РегистрСведений.уатЗначенияСвойствОбъектов.Объект = &ОбъектОтбораЗначений
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ОткрытьФормуНовогоСвойства(ФормаВладелец, Элемент = Неопределено, Копирование = Ложь) Экспорт
	...
	ФормаНовогоСвойства = ПланыВидовХарактеристик.уатСвойстваОбъектов.ПолучитьФормуНовогоЭлемента(, ФормаВладелец, );
	...
	Иначе
		...
		НазначениеСвойств = уатОбщегоНазначения.уатПолучитьСписокНазначенийСвойствКатегорийОбъектовПоСсылке(ОбъектОтбораЗначений);
...
Процедура ПроверитьДобавитьНовоеСвойство(Свойство, ТабличноеПоле) Экспорт
	...
	|		ПланВидовХарактеристик.уатСвойстваОбъектов.НазначениеСвойства В ( &НазначениеСвойств )
	|		И
	|		ПланВидовХарактеристик.уатСвойстваОбъектов.Ссылка             = &Свойство
	|
	|	)                                                           КАК СвойстваОбъектов
	|";
...
Функция ЗаписатьЗначенияСвойств() Экспорт
	...
	НаборЗаписейЗначенияСвойств = РегистрыСведений.уатЗначенияСвойствОбъектов.СоздатьНаборЗаписей();
