...
Перем мРазделительАдресов Экспорт; // Символ,который будет использоваться в качестве разделителя
...
Процедура ЗаполнитьСписокГруппРассылки(ГруппыРассылкиСписок) Экспорт
	...
	Иначе
		СписокУчетныхЗаписей = уатОтношенияСКлиентами.уатПолучитьДоступныеУчетныеЗаписи(глЗначениеПеременной("глТекущийПользователь")).Отправка.Скопировать();
	КонецЕсли;
	...
	Запрос.Текст = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	уатГруппыРассылкиУчетныхЗаписейЭлектроннойПочты.ГруппаРассылки.ПометкаУдаления КАК ПометкаУдаления,
	|	уатГруппыРассылкиУчетныхЗаписейЭлектроннойПочты.ГруппаРассылки                 КАК ГруппаРассылки
	|ИЗ
	|	РегистрСведений.уатГруппыРассылкиУчетныхЗаписейЭлектроннойПочты КАК уатГруппыРассылкиУчетныхЗаписейЭлектроннойПочты
	|ГДЕ
	|	уатГруппыРассылкиУчетныхЗаписейЭлектроннойПочты.УчетнаяЗапись В(&СписокУчетныхЗаписей)
	|УПОРЯДОЧИТЬ ПО
	|	ГруппаРассылки ВОЗР
	|";
...
Процедура Объект_ПриОткрытии(ЭлементыФормы) Экспорт
	...
	ЭлементыФормы.СоставГруппыРассылки.Значение.Отбор.ГруппаРассылки.Значение      = Справочники.уатГруппыПочтовойРассылки.ПустаяСсылка();
	...
	Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВидОбщегоОбъекта) Тогда
...
Процедура ДобавитьЭлементы(ЭлементыФормы, СписокДанныхТекущейСтроки, ЭлементФормы, ЭлементУправленияАдрес, СтандартнаяОбработка = Неопределено, ИдентификаторСправочника = Неопределено, Множественный = Ложь) Экспорт
	...
	Если НЕ Множественный Тогда
		...
		Если ВыбранныеСтроки.Количество() = 1 И НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВыбранныеСтроки[0]) Тогда
			...
			Иначе
				Если ПодборОбъектов Тогда
					СписокЭлементов.Добавить(ВыбранныеСтроки[0]);
				Иначе
					Если Адрес = "E-mail не задан" ИЛИ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Адрес) Тогда
						Возврат;
					Иначе
		...
		ИначеЕсли ВыбранныеСтроки.Количество() > 0 Тогда
			Для каждого СтрокаТаблицы Из ВыбранныеСтроки Цикл
				Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтрокаТаблицы) Тогда
					Продолжить;
				КонецЕсли; 
	...
	Если ПодборОбъектов Тогда
		...
		Если ТипЗнч(СписокЭлементов) = Тип("СписокЗначений") Тогда
			...
			Если ИдентификаторСправочника = "ЛичныеКонтакты" ИЛИ ИдентификаторСправочника = "КонтактныеЛицаКонтрагентов" ИЛИ ИдентификаторСправочника = "ПрочиеКонтактныеЛица" Тогда
			Иначе
	...
	Иначе
		...
		Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВыбЭлемент) Тогда
		...
		Иначе
			...
			Если ТипЗнч(СписокЭлементов) = Тип("СписокЗначений") Тогда
				...
				Если ИдентификаторСправочника = "ЛичныеКонтакты" ИЛИ ИдентификаторСправочника = "КонтактныеЛицаКонтрагентов" ИЛИ ИдентификаторСправочника = "ПрочиеКонтактныеЛица" Тогда
					...
					ИначеЕсли ИдентификаторСправочника = "КонтактныеЛицаКонтрагентов" Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Пока ВыборкаДетальныхЗаписей.Следующий() Цикл
					Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(ВыборкаДетальныхЗаписей.АдресЭлектроннойПочты) Тогда
						Продолжить;
					КонецЕсли; 
	...
	Иначе
		...
		Если Множественный Тогда
			...
			Для каждого СтрокаТаблицы Из РезультатыПоиска Цикл
				...
				СтрокаТЧ = ТекущаяТЧ.Добавить();
				СтрокаТЧ.Объект = СтрокаТаблицы.Объект;
				Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтрокаТаблицы.ЗначениеПоУмолчанию) Тогда
		...
		Иначе
			...
			Иначе
				...
				Для каждого СтрокаТаблицы Из ЭлементыФормы.РезультатыПоиска.ВыделенныеСтроки Цикл
					...
					Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(СтрокаТаблицы.ЗначениеПоУмолчанию) И СтрокаТаблицы.ДоступныеАдреса.Количество() = 0 Тогда
						Продолжить;
					КонецЕсли; 
...
Процедура Объект_РезультатыПоискаПриАктивизацииСтроки(Элемент, ЭлементыФормы) Экспорт
	...
	Иначе
		...
		Иначе
			ЭлементФормы.ТолькоПросмотр = Ложь;
			Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(Элемент.ТекущиеДанные.ЗначениеПоУмолчанию) Тогда
				ЭлементФормы.Значение = ЭлементФормы.СписокВыбора[0].Значение;
			Иначе
...
Процедура Объект_ГруппыРассылкиСписокПередНачаломДобавления(Элемент, Отказ, Копирование, ЭтаФорма) Экспорт
	...
	Иначе
		НовыйОбъект = Справочники.уатГруппыПочтовойРассылки.СоздатьЭлемент();
	КонецЕсли;
...
Процедура Объект_ОбработкаЗаписиНовогоОбъекта(Объект, Источник, ЭтаФорма) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникСсылка.уатГруппыПочтовойРассылки") Тогда
...
Процедура Объект_ГруппыРассылкиСписокПередНачаломИзменения(Элемент, Отказ, ЭтаФорма) Экспорт
	...
	Если Элемент.ТекущиеДанные <> Неопределено И НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Элемент.ТекущиеДанные.ГруппаРассылки) Тогда
...
Процедура ДобавитьЭлементыГруппРассылки(ЭлементыФормы, ГруппыРассылкиСписок, Множественный = Ложь) Экспорт
	...
	Если ПодборОбъектов Тогда
		...
		Запрос.Текст = "
		|ВЫБРАТЬ
		|	уатСоставГруппРассылкиЭлектроннойПочты.ОбъектРассылки КАК ОбъектРассылки
		|ИЗ
		|	РегистрСведений.уатСоставГруппРассылкиЭлектроннойПочты КАК уатСоставГруппРассылкиЭлектроннойПочты
		|ГДЕ
		|	уатСоставГруппРассылкиЭлектроннойПочты.ГруппаРассылки В(&ГруппыРассылки)
		|";
	...
	Иначе
		...
		|	РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
		|	ПО
		|	КонтактнаяИнформация.Объект = уатСоставГруппРассылкиЭлектроннойПочты.ОбъектРассылки
		|	И
		|	КонтактнаяИнформация.Тип = &Тип
		|
		|ГДЕ
...
// Процедура обрабатывает событие ПриАктивизацииСтроки списка справочника Контрагенты
//
...
Процедура Объект_ОбъектЭлектроннойПочтыПриИзменении(Элемент, ЭлементТабличноеПоле) Экспорт
	...
	Если уатОбщегоНазначения.уатЗначениеНеЗаполнено(Элемент.Значение) Тогда
...
Процедура Объект_ГруппыРассылкиСписокПриАктивизацииСтроки(Элемент, СоставГруппыРассылки) Экспорт
	...
	Если Элемент.ТекущиеДанные = Неопределено Тогда
		СоставГруппыРассылки.Отбор.ГруппаРассылки.Значение = Справочники.уатГруппыПочтовойРассылки.ПустаяСсылка();
	Иначе
...
Процедура Объект_СоставГруппыРассылкиПередНачаломИзменения(Элемент, Отказ) Экспорт
	...
	Если НЕ уатОбщегоНазначения.уатЗначениеНеЗаполнено(Элемент.ТекущиеДанные.ОбъектРассылки) Тогда
